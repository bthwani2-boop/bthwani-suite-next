# الشركاء والمتاجر — عقد الإغلاق النهائي

```yaml
section_id: partners
canonical_route: /dsh/partners
routes: [/dsh/partners, /dsh/partners/[partnerId], /dsh/partners/stores]
...
قائمة الشركاء، Queue المراجعة، تفاصيل الشريك، الفريق، Commercial Model، المتاجر والجاهزية والنشر والأسطول. يجب جرد كل Tab وDrawer وDetail وWizard وTable وCard وForm وDialog وربطه بالرحلات والـoperationIds، مع N/A مبررة للعناصر غير المنطبقة.
...
إنشاء/استئناف onboarding، claim/review/approve/reject، suspend/terminate، team invite، إضافة متجر، publish/pause، bulk scoped. كل تحكم يملك accessibility name وpermission visibility منفصلة عن business enablement وloading محليًا ومنع double-submit وstale-conflict handling وunknown-result lookup وsuccess readback. لا زر أو Icon شكلي بلا API binding أو أثر.
...
Identity/Workforce references بلا نسخ، DSH partner/store truth، WLT summaries masked عبر facade. يجب توثيق owner لكل حقل وحالة وقرار، operationId/handler/repository/table/event/job/cache consumer، ومنع أي type/status/policy/formatter/fallback محلي يكرر السلطة.
...
Partner كامل من provisioning إلى Store منشور، self-approval، cross-partner IDOR، stale review، partial orchestration، suspension propagation. إضافة إلى ذلك: فتح كل Route مباشرة، refresh/back/session expiry، اختبار كل Tab وFilter وSort وPagination وForm وDialog وButton وIcon بحساب مصرح وغير مصرح، نقر مكرر، فقد الرد بعد commit، قارئ شاشة ولوحة مفاتيح وRTL وzoom 200%.
...
`section_id=partners; registered_routes=PASS; unmapped_pages_tabs_controls_states=0; permission_backend_mismatches=0; parallel_truths=0; automated_tests=PASS; manual_acceptance=PASS; same_sha_evidence=PASS`.
