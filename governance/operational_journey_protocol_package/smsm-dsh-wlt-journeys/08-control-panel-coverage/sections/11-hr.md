# الموارد البشرية — عقد الإغلاق النهائي

```yaml
section_id: hr
canonical_route: /dsh/hr
routes: [/dsh/hr]
...
الأشخاص والملفات المهنية وCaptain/Field provisioning/readiness والتكليفات والورديات والمشرفون والنطاقات والوثائق. يجب جرد كل Tab وDrawer وDetail وWizard وTable وCard وForm وDialog وربطه بالرحلات والـoperationIds، مع N/A مبررة للعناصر غير المنطبقة.
...
create/edit، provision/resume/compensate، assign/end، review documents، suspend/terminate، open Identity/Operations/Finance owner. كل تحكم يملك accessibility name وpermission visibility منفصلة عن business enablement وloading محليًا ومنع double-submit وstale-conflict handling وunknown-result lookup وsuccess readback. لا زر أو Icon شكلي بلا API binding أو أثر.
...
Workforce professional truth مرتبط بـactor_id؛ Identity fields read-only؛ DSH assignments references؛ WLT blockers masked. يجب توثيق owner لكل حقل وحالة وقرار، operationId/handler/repository/table/event/job/cache consumer، ومنع أي type/status/policy/formatter/fallback محلي يكرر السلطة.
...
`section_id=hr; registered_routes=PASS; unmapped_pages_tabs_controls_states=0; permission_backend_mismatches=0; parallel_truths=0; automated_tests=PASS; manual_acceptance=PASS; same_sha_evidence=PASS`.
