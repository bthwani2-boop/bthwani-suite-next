...
pages, mobile journeys, and screen states before product-slice implementation.
...
- `dsh-wlt` is an integration scope or slice family, never a service owner.
- `platform` belongs to `core`.
- Financial truth belongs to `wlt`.
- Shared frontend rows use `surface=shared` and declare consumers.
- Screen and route targets are plans only until implemented by their owning slice.

## Current gate

The normalized inventory does not authorize DSH-001 implementation. Store
...
## Pre-slice gate rules

The following rules from `governance/15_MATRIX_NORMALIZATION_RULES.md` are
enforced as hard gates before any slice execution begins:

- Core identity must not target DSH paths
- SPEC slice segment must match slice_id exactly
- operations-support is not a valid executable capability — split into operations-room or support-queue/support-entry
- WLT-002 must have at least 5 direct WLT logic rows (refund-status)
- WLT-003 must have at least 4 direct WLT logic rows (settlement-status-read)
- DSH-014 must have at least 4 support-entry logic rows (client, partner, captain, field)
- Marketing api_contract must reference DSH-015, not DSH-011
...
- No row may reach READY_FOR_SLICE during the final consistency repair
...
DSH-001 becomes READY_FOR_SLICE only after an independent step approves:
GET /dsh/stores, GET /dsh/stores/{storeId}, store domain, store DB schema, and
store auth/serviceability rules.

## Acceptance condition

Accepted only when schema v2 is present, ownership and slice normalization pass,
all mobile/control-panel targets have screen-state coverage, forbidden paths and
service owners are absent, all pre-slice gate rules pass, and the foundation gate passes.
