...
- Schema changes are forward, reviewable, deterministic, ordered, and tested against the supported upgrade path.
- Applied migration history is not rewritten to make a current state look cleaner.
- Constraints, foreign keys, unique indexes, checks, locking/versioning, idempotency and transaction boundaries encode invariants close to the data when appropriate.
- Destructive change requires proven consumer/data impact, retention requirements, and a forward-safe rollout/rollback plan.
- Seeds/fixtures are development support only and never commercial or production truth.
...
- DSH owns operational commerce/fulfillment truth and only the bounded WLT-backed references/projections allowed by current contracts.
...
- UI state must distinguish applicable loading, empty, offline, forbidden, conflict, partial, error, recovery and success states.
