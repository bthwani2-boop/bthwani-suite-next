...
Requires `control-panel`, role `operator`, permission `operations.read`. The response is operator context-scoped and excludes client identity, address, tokens, idempotency keys and payment-provider payloads.
...
1. Pin the exact DSH commit, operator context, order ID/order number and correlation ID.
2. Read the diagnostic endpoint and compare creation, stuck attempt, outbox and projection counts.
3. Query `dsh_order_create_idempotency` by operator context/client and correlation; never expose the raw idempotency key outside restricted database access.
4. Query `dsh_orders`, `dsh_order_status_events`, `dsh_order_event_outbox` and `dsh_order_truth_audit` by operator context and correlation ID.
...
- A stuck attempt may be investigated only after verifying no order exists for the operator context Checkout Intent. Do not delete the row while a request is active.
...
- operator context, order ID/order number and correlation ID.
