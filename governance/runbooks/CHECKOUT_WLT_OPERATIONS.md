...
Operate and recover the checkout-to-WLT payment-session handoff without creating duplicate payment sessions, duplicating discounts, editing financial truth in DSH, or losing operator context isolation.
...
```sql
SELECT event_key, operator_context_id, checkout_intent_id, payment_session_id,
...
- Rate of `PAYMENT_SESSION_MISMATCH` and operator context mismatch database violations.
...
- Critical: unknown outcome older than 5 minutes or more than 10 concurrent unknown outcomes per operator context.
- Critical: any unapplied receipt older than 30 seconds.
- Critical: any operator context/session mismatch or replay conflict.

Thresholds are operational defaults and require production tuning; they do not change domain truth.

## Security and privacy

- Client routes require the authenticated `client` actor and operator context/client ownership.
...
- Never use a default or synthetic operator context for a new checkout or event.
...
Apply the invariant test to `dsh_runtime` after migrations and seeds:

```powershell
Get-Content services/dsh/database/tests/dsh-910_wlt_event_receipts.sql -Raw |
...
4. Do not drop `dsh_checkout_wlt_event_receipts`, the operator context/session trigger, or observability columns during incident response. They are backward-compatible evidence structures.
...
7. Re-run the dedicated workflow and runtime database invariants on the rollback commit.
...
Technical completion requires all functional slices and FS-01..FS-18 to pass on one commit under `journeys//all-slices`. Final `CLOSED_WITH_EVIDENCE` additionally requires independent Product Owner, QA/device/accessibility, Security/Finance boundary and Release/Production approvals.
