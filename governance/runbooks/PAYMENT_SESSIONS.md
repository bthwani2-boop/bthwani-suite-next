...
1. Every `authorize`, `capture` and provider-status refresh carries operator context, correlation and idempotency identities.
...
2. Enter the operator context and payment-session identifiers.
...
```sql
SELECT id, operator_context_id, provider_reference, captured_at
...
```sql
SELECT id, operator_context_id, payment_session_id, operation, idempotency_key, correlation_id, updated_at
...
```sql
SELECT provider_event_id, operator_context_id, payment_session_id, processing_result, received_at
...
- cross-operator-context payment access;
