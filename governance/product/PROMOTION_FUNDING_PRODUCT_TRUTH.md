# — Promotion Funding Product Truth
...
1. The client applies an eligible governed coupon during checkout. The client never chooses the funding source, split, operator context, or WLT reference.
...
1. `reserve`: DSH calculates the immutable split and asks WLT to reserve it with operator context, idempotency, and correlation assertions.
...
- operator context assertion is mandatory for authenticated promotion-funding service calls.
