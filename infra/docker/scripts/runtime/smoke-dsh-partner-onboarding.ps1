...
$partnerDraft = Invoke-RestMethod "$dshBaseUrl/dsh/field/partners/drafts" -Method Post -Headers $partnerDraftHeaders -ContentType "application/json" -Body $partnerDraftBody -TimeoutSec 10
...
$fieldStore = Invoke-RestMethod "$dshBaseUrl/dsh/field/partners/$($partnerDraft.id)/store" -Headers $fieldHeaders -TimeoutSec 10
...
$updatedStore = Invoke-RestMethod "$dshBaseUrl/dsh/field/partners/$($partnerDraft.id)/store" -Method Patch -Headers $storeProfileHeaders -ContentType "application/json" -Body $storeProfileBody -TimeoutSec 10
...
$settings = Invoke-RestMethod "$dshBaseUrl/dsh/partner/stores/$smokeStoreId/settings" -Method Patch -Headers $settingsHeaders -ContentType "application/json" -Body $settingsBody -TimeoutSec 10
...
$destination = Invoke-RestMethod "$dshBaseUrl/dsh/control-panel/finance/payout-destinations/partner/$($partnerActor.actorId)" -Method Put -Headers $destinationHeaders -ContentType "application/json" -Body $destinationBody -TimeoutSec 10
...
$payoutReadback = Invoke-WebRequest "$dshBaseUrl/dsh/partner/me/finance/payout-destination" -Headers $payoutHeaders -TimeoutSec 10 -SkipHttpErrorCheck
$payoutResult = $null
...
$mutationProbe = Invoke-WebRequest "$dshBaseUrl/dsh/partner/me/finance/payout-destination" -Method Put -Headers $payoutHeaders -ContentType "application/json" -Body '{}' -TimeoutSec 10 -SkipHttpErrorCheck
...
$payoutPartner = Invoke-RestMethod "$dshBaseUrl/dsh/field/partners/$($partnerDraft.id)" -Headers $fieldHeaders -TimeoutSec 10
...
try {
  $visitEvidenceUpload = Invoke-RestMethod "$dshBaseUrl/dsh/field/media/uploads" -Method Post -Headers $fieldHeaders -Form @{
...
$visit = Invoke-RestMethod "$dshBaseUrl/dsh/field/partners/$($partnerDraft.id)/visits" -Method Post -Headers $visitHeaders -ContentType "application/json" -Body $visitBody -TimeoutSec 10
...
try {
  $documentEvidenceUpload = Invoke-RestMethod "$dshBaseUrl/dsh/field/media/uploads" -Method Post -Headers $fieldHeaders -Form @{
    partnerId = [string]$partnerDraft.id
...
$doc = Invoke-RestMethod "$dshBaseUrl/dsh/field/partners/$($partnerDraft.id)/documents" -Method Post -Headers $documentHeaders -ContentType "application/json" -Body $docBody -TimeoutSec 10
...
$submitted = Invoke-RestMethod "$dshBaseUrl/dsh/field/partners/$($partnerDraft.id)/submit" -Method Post -Headers $submitHeaders -ContentType "application/json" -Body $submitBody -TimeoutSec 10
...
$partnerStores = Invoke-RestMethod "$dshBaseUrl/dsh/operator/partners/$($partnerDraft.id)/stores" -Headers $operatorHeaders -TimeoutSec 10
...
function Invoke-PartnerTransition([string] $PartnerId, [string] $ToStatus) {
  $current = Invoke-RestMethod "$dshBaseUrl/dsh/operator/partners/$PartnerId" -Headers $operatorHeaders -TimeoutSec 10
  ...
  $result = Invoke-RestMethod "$dshBaseUrl/dsh/operator/partners/$PartnerId/transition" -Method Post -Headers $headers -ContentType "application/json" -Body $body -TimeoutSec 10
...
$review = Invoke-RestMethod "$dshBaseUrl/dsh/operator/partners/$($partnerDraft.id)/documents/$($doc.id)/review" -Method Patch -Headers $reviewHeaders -ContentType "application/json" -Body $reviewBody -TimeoutSec 10
...
function Invoke-SmokeStoreGovernance([string] $StoreId, [string] $Action, [string] $Value) {
  $detail = Invoke-RestMethod "$dshBaseUrl/dsh/operator/stores/$StoreId" -Headers $operatorHeaders -TimeoutSec 10
  ...
  $result = Invoke-RestMethod "$dshBaseUrl/dsh/operator/stores/$StoreId/governance" -Method Post -Headers $headers -ContentType "application/json" -Body $body -TimeoutSec 10
...
if ($MediaEnabled) {
  ...
  foreach ($storeImage in @(
    ...
    $imageLink = Invoke-RestMethod "$dshBaseUrl/dsh/operator/catalog/assets/$($storeImage.assetId)/link" -Method Post -Headers $operatorHeaders -ContentType "application/json" -Body $imageLinkBody -TimeoutSec 10
...
$assortment = Invoke-RestMethod "$dshBaseUrl/dsh/operator/stores/$smokeStoreId/assortment/$smokeCatalogProductId" -Method Put -Headers $operatorHeaders -ContentType "application/json" -Body $assortmentBody -TimeoutSec 10
...
if ($MediaEnabled) {
  ...
  $publicationWorkspace = Invoke-RestMethod "$dshBaseUrl/dsh/operator/marketing/stores/$smokeStoreId/publication" -Headers $operatorHeaders -TimeoutSec 10
  $publicationHeaders = @{}
  foreach ($key in $operatorHeaders.Keys) {
  ...
  $published = Invoke-RestMethod "$dshBaseUrl/dsh/operator/marketing/stores/$smokeStoreId/publication" -Method Post -Headers $publicationHeaders -ContentType "application/json" -Body $publicationBody -TimeoutSec 10
...
$readiness = Invoke-RestMethod "$dshBaseUrl/dsh/operator/partners/$($partnerDraft.id)/readiness" -Headers $operatorHeaders -TimeoutSec 10
...
$audit = Invoke-RestMethod "$dshBaseUrl/dsh/operator/partners/$($partnerDraft.id)/audit" -Headers $operatorHeaders -TimeoutSec 10
...
$linkedStore = Invoke-RestMethod "$dshBaseUrl/dsh/operator/stores/$smokeStoreId" -Headers $operatorHeaders -TimeoutSec 10
...
if ($MediaEnabled) {
  $publicStore = Invoke-RestMethod "$dshBaseUrl/dsh/stores/$smokeStoreId" -TimeoutSec 10
  ...
  $publicCatalog = Invoke-RestMethod "$dshBaseUrl/dsh/stores/$smokeStoreId/catalog" -TimeoutSec 10
  if (@($publicCatalog.products).Count -lt 1) { throw "Partner Onboarding & Store Publication app-client catalog is empty" }
}

$partnerSelfStatus = Invoke-RestMethod "$dshBaseUrl/dsh/partner/activation/status?storeId=$([uri]::EscapeDataString($smokeStoreId))" -Headers $partnerHeaders -TimeoutSec 10
...
$partnerSelfReadiness = Invoke-RestMethod "$dshBaseUrl/dsh/partner/activation/readiness?storeId=$([uri]::EscapeDataString($smokeStoreId))" -Headers $partnerHeaders -TimeoutSec 10
