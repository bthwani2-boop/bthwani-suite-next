...
if ($WltEnabled) {
  ...
  $cartItem = Invoke-RestMethod "$dshBaseUrl/dsh/client/cart/items" -Method Post -Headers $clientHeaders -ContentType "application/json" -Body $cartBody -TimeoutSec 10
  ...
  $cartReadback = Invoke-RestMethod "$dshBaseUrl/dsh/client/cart?storeId=store-test-grocery" -Headers $clientReadHeaders -TimeoutSec 10
  ...
  $addressList = Invoke-RestMethod "$dshBaseUrl/dsh/client/addresses" -Headers $clientReadHeaders -TimeoutSec 10
  ...
  if ($deliveryAddress.Count -eq 0) {
    ...
    $createdAddress = Invoke-RestMethod "$dshBaseUrl/dsh/client/addresses" -Method Post -Headers $addressHeaders -ContentType "application/json" -Body $addressBody -TimeoutSec 10
  ...
  $checkout = Invoke-RestMethod "$dshBaseUrl/dsh/client/checkout-intents" -Method Post -Headers $checkoutHeaders -ContentType "application/json" -Body $checkoutBody -TimeoutSec 10
  ...
  $operatorCheckout = Invoke-RestMethod "$dshBaseUrl/dsh/operator/checkout-intents" -Headers $operatorHeaders -TimeoutSec 10
...
foreach ($actor in $scopedActors) {
  ...
  $context = Invoke-RestMethod "$dshBaseUrl/dsh/store-context?storeId=$storeId" -Headers $headers -TimeoutSec 10
...
$homeDisc = Invoke-RestMethod "$dshBaseUrl/dsh/home-discovery?limit=50" -TimeoutSec 10 -ErrorAction Stop
...
foreach ($homeStore in $homeStores) {
  ...
  $detail = Invoke-RestMethod "$dshBaseUrl/dsh/stores/$encodedStoreId" -TimeoutSec 10 -ErrorAction Stop
  ...
  $catalog = Invoke-RestMethod "$dshBaseUrl/dsh/stores/$encodedStoreId/catalog" -TimeoutSec 15 -ErrorAction Stop
