...
foreach ($Database in $ManagedDatabases) {
  ...
  if ($LASTEXITCODE -ne 0 -or (($Exists -join "").Trim()) -ne "1") {
    throw "Managed database is missing: $($Database.name)"
  }
  ...
  docker exec @DockerEnv $PostgresContainer pg_dump -U $Database.owner -d $Database.name -Fc --no-owner --no-privileges -f $ContainerDump
  ...
  $Checksum = (Get-FileHash -LiteralPath $HostDump -Algorithm SHA256).Hash.ToLowerInvariant()
...
if (-not [string]::IsNullOrWhiteSpace(($MinioRunning -join ""))) {
  ...
  if (-not $MinioVolumeName) { throw "MinIO is running but its data volume could not be resolved." }

  $MinioBackupFile = Join-Path $BackupDir "minio-volume.tar.gz"
  docker run --rm -v "${MinioVolumeName}:/volume-data:ro" -v "${BackupDir}:/backup-dest" alpine:3.21 `
