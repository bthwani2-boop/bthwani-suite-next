...
$script:WltMigrationProbes = [ordered]@{
  ...
  "wlt-024_payment_session_tenancy.sql"           = "EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'wlt_payment_sessions' AND column_name = 'operator_context_id') AND to_regclass('public.wlt_payment_sessions_tenant_checkout_intent_idx') IS NOT NULL"
  ...
  "wlt-029_payment_session_tenant_lock.sql"       = "EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'wlt_payment_sessions' AND column_name = 'operator_context_id' AND is_nullable = 'NO') AND to_regclass('public.wlt_payment_sessions_tenant_checkout_uq') IS NOT NULL"
  ...
  "wlt-090_jrn036_settlement_commission_governance.sql" = "(to_regclass('public.wlt_settlement_requests') IS NOT NULL OR to_regclass('public.wlt_settlement_requests') IS NOT NULL) AND (to_regclass('public.wlt_commission_policy_versions') IS NOT NULL OR to_regclass('public.wlt_commission_policy_versions') IS NOT NULL) AND (to_regclass('public.wlt_audit_events') IS NOT NULL OR to_regclass('public.wlt_finance_audit_events') IS NOT NULL)"
  "wlt-091_jrn036_adjustment_ledger_identity.sql" = "to_regclass('public.wlt_commission_adjustments_request_hash_idx') IS NOT NULL OR to_regclass('public.wlt_commission_adjustments_tenant_request_hash_idx') IS NOT NULL"
  ...
  "wlt-093_jrn036_mutation_receipts.sql" = "(to_regclass('public.wlt_mutation_receipts') IS NOT NULL OR to_regclass('public.wlt_mutation_receipts') IS NOT NULL) AND (to_regclass('public.wlt_mutation_receipts_aggregate_idx') IS NOT NULL OR to_regclass('public.wlt_mutation_receipts_tenant_aggregate_idx') IS NOT NULL) AND (to_regclass('public.wlt_mutation_receipts_request_hash_idx') IS NOT NULL OR to_regclass('public.wlt_mutation_receipts_tenant_request_hash_idx') IS NOT NULL)"
  ...
  "wlt-098_jrn037_payout_destination_governance.sql" = "(to_regclass('public.wlt_payout_audit_events') IS NOT NULL OR to_regclass('public.wlt_payout_audit_events') IS NOT NULL) AND (to_regclass('public.wlt_payout_outbox') IS NOT NULL OR to_regclass('public.wlt_payout_outbox') IS NOT NULL) AND (to_regclass('public.wlt_payout_reconciliations') IS NOT NULL OR to_regclass('public.wlt_payout_reconciliations') IS NOT NULL)"
  "wlt-099_jrn037_request_hash_scope.sql" = "to_regclass('public.wlt_payout_requests_request_hash_idx') IS NOT NULL OR to_regclass('public.wlt_payout_requests_tenant_request_hash_idx') IS NOT NULL"
  ...
  "wlt-101_jrn037_reconciliation_single_claim.sql" = "EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'wlt_single_reconciliation_claim_trigger' AND NOT tgisinternal)"
  ...
  "wlt-114_payout_destination_request_tenant_key.sql" = "EXISTS (SELECT 1 FROM pg_constraint WHERE conname='wlt_payout_destination_requests_pkey' AND pg_get_constraintdef(oid) LIKE '%operator_context_id%idempotency_key%') AND to_regclass('public.wlt_payout_destination_requests_tenant_owner_idx') IS NOT NULL"
