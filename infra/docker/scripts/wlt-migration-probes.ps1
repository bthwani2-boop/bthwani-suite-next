...
$script:WltMigrationProbes = [ordered]@{
  ...
  "wlt-037_jrn_035_refund_governance.sql"         = "EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'wlt_refunds' AND column_name = 'requested_by_operator_id') AND to_regclass('public.wlt_refunds_session_idempotency_idx') IS NOT NULL AND to_regclass('public.wlt_dsh_outbox_events_refund_event_idx') IS NOT NULL"
