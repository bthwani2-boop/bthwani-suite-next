...
const canonicalMigration = read("core/identity/database/migrations/identity-001_canonical_baseline.sql");
...
test("canonical migration contains pure schema without hardcoded development residue", () => {
  assert.doesNotMatch(canonicalMigration, /INSERT INTO identity_actors/);
  assert.doesNotMatch(canonicalMigration, /INSERT INTO identity_roles/);
  assert.doesNotMatch(canonicalMigration, /BTHWANI_LOCAL_DEV_PASSWORD/);
  assert.match(canonicalMigration, /CREATE TABLE public\.identity_actors/);
  assert.match(canonicalMigration, /CREATE TABLE public\.identity_roles/);
});
