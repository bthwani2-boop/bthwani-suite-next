...
FILE_NAMES_MATCH_SEMANTIC_RESPONSIBILITY
DIRECTORY/PACKAGE_PATHS_MATCH_CANONICAL_OWNERSHIP
...
SEARCH_MISLEADING_NAMES_AND_WRONG_OWNER_PATHS
...
Required final sweeps include materially applicable:

```text
FULL_TRACKED_TREE_CENSUS
FULL_DIRECTORY/FILE/MATERIAL_SYMBOL_CENSUS
FULL_PACKAGE/WORKSPACE/DEPENDENCY_CENSUS
FULL_SEMANTIC_RESPONSIBILITY_SWEEP_INDEPENDENT_OF_NAMES/PATHS
FULL_OWNER/WRITER/READER/CONSUMER_SWEEP
FULL_CANONICAL_CONTAINER/ANTI_FRAGMENTATION_SWEEP
...
FULL_NEGATIVE_SPACE
FULL_EXACT_H_CI_AND_REQUIRED_RUNTIME/RENDERED/DEVICE_PROOF
```

Final coverage must prove zero known material gaps/unknowns, not merely a green build.

## 14. Final fixed-point gates
...
RESPONSIBILITY_LESS_FILES/DIRECTORIES/UNOWNED_PACKAGES=0
DEFECT_ABSORPTION_STRUCTURES_REMAINING=0
UNJUSTIFIED_FRAGMENTATION=0
MISLEADING_NAMES_OR_PATHS=0
UNJUSTIFIED_OVERSIZED_OR_MULTI_RESPONSIBILITY_FILES=0
UNJUSTIFIED_CIRCULAR_OR_CROSS_DOMAIN_COUPLING=0
UNBOUNDED_COMPATIBILITY=0
ORPHAN_REQUIRED_DB/BACKEND/API/CONTRACT/BINDING/SCREEN_TRUTH=0
FRONTEND/BACKEND_SHADOW_BUSINESS_TRUTH=0
...
Only then may the orchestrator emit the completion token defined by `00`.
