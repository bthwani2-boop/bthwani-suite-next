...
function compileSchema(relative) {
  ...
  try { return ajv.compile(schema); } catch (error) {
...
function listFiles(relativeRoot) {
  ...
  const walk = (directory, relative) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(nextAbsolute, nextRelative); else files.push(nextRelative);
    }
  };
...
function requireExactDirectory(relative, allowedFiles = [], allowedDirs = []) {
  ...
  for (const entry of fs.readdirSync(absolute(relative), { withFileTypes: true })) {
    ...
    else if (entry.isFile() && !files.has(entry.name)) violations.push({ file: child, line: 0, message: `UNCLASSIFIED_FILE ${entry.name}` });
  }
}
function registeredToolPolicyFiles(registryPath, registry) {
  const allowed = new Set(["README.md"]);
  if (registry && !Array.isArray(registry.entries)) { violations.push({ file: registryPath, line: 0, message: "AGENT_TOOL_REGISTRY_ENTRIES_REQUIRED" }); return [...allowed]; }
  for (const tool of registry?.entries ?? []) {
    const id = String(tool?.id ?? "<unknown>");
    const policyPath = String(tool?.policy_path ?? "").replaceAll("\\", "/");
    if (path.posix.dirname(policyPath) !== ".agents/tools" || path.posix.extname(policyPath) !== ".md") { violations.push({ file: registryPath, line: 0, message: `TOOL_POLICY_PATH_INVALID ${id}:${policyPath || "<missing>"}` }); continue; }
    if (!exists(policyPath)) { violations.push({ file: registryPath, line: 0, message: `REGISTERED_TOOL_POLICY_MISSING ${id}:${policyPath}` }); continue; }
...
requireExactDirectory("tools/prompting", ["01-diagnose-plan-package.md", "02-execute-verify-close.md", "03-end-to-end-fail-closed.md", "04-journey-multisurface-operational-diagnosis.md", "BTHWANI_CHATGPT_GITHUB_EXECUTION_CARD_ONE_PAGE.md"], ["bthwani-orchestrator"]);
requireExactDirectory("tools/prompting/bthwani-orchestrator", ["00-ORCHESTRATOR.md", "01-CORE-CONTRACT.md", "02-DISCOVERY-DIAGNOSIS.md", "03-DECISIONS-COVERAGE-ANTI-DRIFT.md", "04-PACKAGE-EXECUTION.md", "05-VERIFICATION-CLEANUP-CLOSURE.md", "06-CONCURRENCY-RESUME-RECOVERY.md"], ["contracts", "source-map"]);
requireExactDirectory("tools/prompting/bthwani-orchestrator/contracts", ["OVERVIEW-CONTRACT.md", "SEQUENCE-CONTRACT.md", "DECISION-OUTPUT-CONTRACT.md", "EVIDENCE-CONTRACT.md", "CLOSURE-CONTRACT.md"]);
...
requireExactDirectory("plans/diagnose-implementing/_template", ["00-OVERVIEW.template.md", "SEQUENCE.template.md"]);

for (const required of ["governance/GOVERNANCE.md", "governance/product/PRD.md", "governance/policies/engineering.md", "governance/policies/security.md", "governance/policies/delivery.md"]) if (!/^Status:\s*ACTIVE_CANONICAL\s*$/m.test(readText(required))) violations.push({ file: required, line: 0, message: "ACTIVE_CANONICAL_STATUS_REQUIRED" });

for (const retired of [
  "governance/README.md", "governance/domains", "governance/product/decisions", "governance/policies/product.md", "governance/policies/contracts.md", "governance/policies/data.md", "governance/policies/runtime.md", "governance/policies/release.md", "governance/policies/governance.rego", "governance/github/repository-enforcement.json", "governance/github/repository-enforcement.schema.json", "governance/github/full-verification-policy.json", "governance/27_FULLSTACK_MULTI_SURFACE_JOURNEY_REGISTRY.md", "governance/runbooks", "governance/prompting", "governance/partners", "governance/operational_journey_protocol_package", "tools/diagnose-implementing", "tools/plans", "tools/implementing", "tools/important-scripts", "tools/contracts", "tools/rules", ".agents/README.md", ".agents/AGENT_SYSTEM_UPDATE_POLICY.md", ".agents/AUTHORITY_BOUNDARY.md", ".agents/COMMAND_SAFETY_POLICY.md", ".agents/EVIDENCE_GATE_ROUTER.md", ".agents/GRAPHIFY.md", ".agents/PONYTAIL_RULE.md", ".agents/RESTORE_DECISION.md", ".agents/SKILL_CATALOG.md", ".agents/UPDATE_POLICY.md", ".agents/rules", ".agents/adapters", ".agents/hooks", "plans/diagnose-implementing/new-unit.mjs", "plans/diagnose-implementing/_template/START-HERE.template.md", "plans/diagnose-implementing/_template/MANIFEST.template.json", "plans/diagnose-implementing/_template/GLOBAL-DIAGNOSIS.template.md", "plans/diagnose-implementing/_template/COVERAGE.template.json", "plans/diagnose-implementing/_template/EXECUTION-ORDER.template.json", "plans/diagnose-implementing/_template/CLOSURE.template.md", "plans/diagnose-implementing/_template/unit", "plans/diagnose-implementing/_template/01-DIAGNOSIS.template.md", "plans/diagnose-implementing/_template/02-EXECUTION.template.md", "plans/diagnose-implementing/_template/03-VERIFICATION-CLOSURE.template.md", "tools/prompting/bthwani-orchestrator/contracts/DIAGNOSIS-OUTPUT-CONTRACT.md", "tools/prompting/bthwani-orchestrator/contracts/EXECUTION-PACKAGE-CONTRACT.md", "plans/smsm-dsh-wlt-journeys", "tools/scripts/check-smsm-journey-coverage.mjs"
]) if (exists(retired)) violations.push({ file: retired, line: 0, message: "RETIRED_OR_PARALLEL_PATH_REINTRODUCED" });

for (const required of ["governance/contracts/full-verification-policy.json", "tools/guards/opa/governance.rego", "plans/diagnose-implementing/README.md", "plans/diagnose-implementing/new-package.mjs", "plans/diagnose-implementing/new-sequence.mjs", "plans/diagnose-implementing/validate-package.mjs", "plans/diagnose-implementing/_template/00-OVERVIEW.template.md", "plans/diagnose-implementing/_template/SEQUENCE.template.md", "tools/prompting/bthwani-orchestrator/00-ORCHESTRATOR.md", "tools/prompting/bthwani-orchestrator/source-map/SOURCE-RULE-TRACEABILITY.md", `${sdlcRoot}/README.md`, `${sdlcRoot}/lifecycle.state-machine.yaml`, `${sdlcRoot}/roles-and-authority.yaml`, `${sdlcRoot}/gate-catalog.yaml`, `${sdlcRoot}/artifact-manifest.schema.json`, `${sdlcRoot}/change-impact.schema.json`]) if (!exists(required)) violations.push({ file: required, line: 0, message: "REQUIRED_GOVERNANCE_OR_PLAN_PATH_MISSING" });
...
if (!exists(productContractsRoot)) violations.push({ file: productContractsRoot, line: 0, message: "MISSING_PRODUCT_TRUTH_CONTRACTS_DIRECTORY" });
  ...
  for (const fileName of files) {
    ...
    const contract = readJson(relative, "PRODUCT_TRUTH_CONTRACT"); if (!contract) continue;
    const canonical = contract.schemaVersion === 1; const validate = canonical ? canonicalProductValidator : compatibilityProductValidator;
    ...
    if (capabilityIds.has(contract.capabilityId)) violations.push({ file: relative, line: 0, message: `DUPLICATE_PRODUCT_CAPABILITY_ID ${contract.capabilityId}` }); capabilityIds.add(contract.capabilityId);
    ...
    for (const surface of contract.surfaces) { if (surfaces.has(surface.id)) violations.push({ file: relative, line: 0, message: `DUPLICATE_PRODUCT_SURFACE ${surface.id}` }); surfaces.add(surface.id); for (const actorId of surface.actors) if (!actors.has(actorId)) violations.push({ file: relative, line: 0, message: `SURFACE_REFERENCES_UNKNOWN_ACTOR ${surface.id}:${actorId}` }); if (surface.required && ["app-client", "app-partner", "app-captain", "app-field", "control-panel"].includes(surface.id) && !surface.routesOrScreens?.length) violations.push({ file: relative, line: 0, message: `REQUIRED_UI_SURFACE_HAS_NO_ROUTE_OR_SCREEN ${surface.id}` }); if (surface.required && surface.id === "backend" && !surface.operationIds?.length) violations.push({ file: relative, line: 0, message: "REQUIRED_BACKEND_SURFACE_HAS_NO_OPERATION_ID" }); }
  }
}
...
if (authority) {
  ...
  for (const document of authority.documents) {
    if (paths.has(document.path)) violations.push({ file: authorityPath, line: 0, message: `DUPLICATE_AUTHORITY_DOCUMENT ${document.path}` }); paths.add(document.path);
    ...
    if (document.path.startsWith("governance/") && ["DERIVED_SUPPORT", "HISTORICAL_REFERENCE"].includes(document.classification)) violations.push({ file: document.path, line: 0, message: "DERIVED_OR_HISTORICAL_ARTIFACT_FORBIDDEN_INSIDE_GOVERNANCE" });
    if (["ROOT_AUTHORITY", "ACTIVE_CANONICAL", "CONDITIONAL_CANONICAL"].includes(document.classification)) { activePaths.add(document.path); for (const domain of document.authorityDomains) { if (domainOwners.has(domain) && domainOwners.get(domain) !== document.path) violations.push({ file: authorityPath, line: 0, message: `DUPLICATE_ACTIVE_AUTHORITY_DOMAIN ${domain}: ${domainOwners.get(domain)} <> ${document.path}` }); else domainOwners.set(domain, document.path); } }
  ...
  for (const required of ["governance/GOVERNANCE.md", "governance/product/PRD.md", "governance/product/contracts", "governance/policies/engineering.md", "governance/policies/security.md", "governance/policies/delivery.md", "governance/contracts", "governance/agents", "governance/skills", "governance/guards", "governance/tools/agent-tool-registry.json", "governance/authority/authority-precedence.schema.json", singleOwnerPath, "governance/authority/single-owner-mode.schema.json", canonicalProductSchema, "governance/product/platform-model.yaml", workflowPath, "governance/github/master-protection.ruleset.json", "plans/diagnose-implementing"]) if (!paths.has(required)) violations.push({ file: authorityPath, line: 0, message: `REQUIRED_AUTHORITY_NOT_REGISTERED ${required}` });
  const governanceAuthorityRoots = authority.documents.filter((d) => d.path.startsWith("governance/")).map((d) => d.path);
  for (const relative of listFiles("governance")) if (!governanceAuthorityRoots.some((ownerPath) => relative === ownerPath || relative.startsWith(`${ownerPath}/`))) violations.push({ file: relative, line: 0, message: "UNCLASSIFIED_GOVERNANCE_FILE" });
  for (const relative of listFiles("governance").filter((file) => file.endsWith(".md"))) if (/^Status:\s*ACTIVE_CANONICAL\s*$/m.test(readText(relative)) && !activePaths.has(relative)) { const covered = authority.documents.some((d) => ["ACTIVE_CANONICAL", "CONDITIONAL_CANONICAL"].includes(d.classification) && relative.startsWith(`${d.path}/`)); if (!covered) violations.push({ file: relative, line: 0, message: "UNREGISTERED_ACTIVE_CANONICAL_DOCUMENT" }); }
}
...
if (decisions) {
  ...
  for (const decisionClass of ["fail", "blocked", "pending"]) if (!(decisions.closureRules.forbiddenOpenClasses ?? []).includes(decisionClass)) violations.push({ file: decisionPath, line: 0, message: `CLOSURE_FORBIDDEN_OPEN_CLASS_MISSING ${decisionClass}` });
}
