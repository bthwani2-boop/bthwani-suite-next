...
if (!/method == http\.MethodPost && path == "\/wlt\/settlements"/.test(read(dshClientFile))) violations.push({ file: dshClientFile, line: 0, message: "DSH_SETTLEMENT_WRITE_ALLOWLIST_MISSING" });
...
const genericHandler = requireCommercialText("services/wlt/backend/internal/reference/trusted_operator_context_handler.go", "subscription purchases must use /wlt/commercial/payment-sessions", "GENERIC_PAYMENT_ROUTE_ACCEPTS_SUBSCRIPTION");
if (!genericHandler.includes("input.SubscriptionPurchaseID") || !genericHandler.includes("input.CommercialProductReference")) violations.push({ file: "services/wlt/backend/internal/reference/trusted_operator_context_handler.go", line: 0, message: "GENERIC_PAYMENT_ROUTE_SOURCE_GUARD_MISSING" });
...
for (const file of ["services/dsh/backend/internal/wlt/subscription_purchase.go", "services/dsh/backend/internal/wlt/subscription_payment_bound.go"]) {
  const source = requireCommercialText(file, "/wlt/commercial/payment-sessions", "DSH_SUBSCRIPTION_CLIENT_NOT_USING_COMMERCIAL_ROUTE");
  if (source.includes('"/wlt/payment-sessions"')) violations.push({ file, line: 0, message: "DSH_SUBSCRIPTION_CLIENT_USES_GENERIC_PAYMENT_ROUTE" });
}
