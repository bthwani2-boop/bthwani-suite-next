...
function readWorkflowText() {
  ...
  return fs.readdirSync(dir).filter((name) => /\.ya?ml$/.test(name)).sort().map((name) => fs.readFileSync(path.join(dir, name), "utf8")).join("\n");
}

const catalogRelative = "tools/toolchain/tool-catalog.v5.json";
const activationRelative = "tools/toolchain/tool-activation-policy.json";
...
const activationDocument = readJson(activationRelative, { activations: {} });
...
if (activationDocument.schemaVersion !== 1 || activationDocument.policyId !== "TOOL_ACTIVATION_POLICY" || typeof activationDocument.activations !== "object" || activationDocument.activations === null) {
  violations.push({ file: activationRelative, line: 0, message: "MALFORMED_TOOL_ACTIVATION_POLICY" });
}
...
const workflowEvidenceByTool = {
  codeql: ["github/codeql-action"], sonarqube: ["sonarqube", "sonar-scanner"], gitleaks: ["gitleaks", "guard:secrets"],
  trivy: ["aquasecurity/trivy-action", "security:trivy", "trivy"], "osv-scanner": ["osv-scanner", "security:osv"],
  conftest: ["guard:opa-policies", "conftest"], actionlint: ["guard:workflow-lint", "actionlint"],
  zizmor: ["guard:workflow-security", "zizmor"], pinact: ["guard:actions-pin", "pinact"],
  nx: ["nx:projects", "nx affected", "nx run-many"],
};
const expectedFailureByActivation = { active: "fail", partial: "warn", optional: "manual", missing: "manual", disabled: "manual" };
...
for (const entry of catalog.entries ?? []) {
  ...
  const expectedActivation = activations[entry.id];
  if (!allowedActivation.has(expectedActivation)) {
    violations.push({ file: activationRelative, line: 0, message: `UNMAPPED_OR_INVALID_ACTIVATION ${entry.id}:${String(expectedActivation)}` });
    continue;
  }
  if (entry.activation !== expectedActivation) violations.push({ file: catalogRelative, line: 0, message: `ACTIVATION_MISMATCH ${entry.id}: catalog=${entry.activation} policy=${expectedActivation}` });
  const expectedFailure = expectedFailureByActivation[expectedActivation];
  if (entry.failure_policy !== expectedFailure) violations.push({ file: catalogRelative, line: 0, message: `FAILURE_POLICY_MISMATCH ${entry.id}: activation=${expectedActivation} failure=${entry.failure_policy}` });

  const scriptName = entry.package_script || entry.fulfilled_by || scriptByTool[entry.id] || "";
  if (["active", "partial"].includes(expectedActivation) && scriptName.startsWith("guard:") && !scripts[scriptName]) violations.push({ file: packageRelative, line: 0, message: `TOOL_GUARD_SCRIPT_MISSING ${entry.id} -> ${scriptName}` });
  ...
  if (registeredGuard && expectedActivation === "active" && registeredGuard.exit_level !== "fail") violations.push({ file: registryRelative, line: 0, message: `ACTIVE_TOOL_GUARD_MUST_FAIL ${entry.id} -> ${scriptName}` });
  if (registeredGuard && expectedActivation === "partial" && registeredGuard.exit_level === "fail" && !["axe", "markdownlint-cli2", "opa"].includes(entry.id)) violations.push({ file: registryRelative, line: 0, message: `PARTIAL_TOOL_GUARD_LEVEL_DRIFT ${entry.id} -> ${scriptName}` });
}
for (const toolId of Object.keys(activations)) if (!catalogIds.has(toolId)) violations.push({ file: catalogRelative, line: 0, message: `ACTIVATION_TOOL_NOT_IN_CATALOG ${toolId}` });

if (/@latest\b/.test(installer)) violations.push({ file: installerRelative, line: 0, message: "FLOATING_GO_INSTALL_VERSION_FORBIDDEN" });
...
for (const marker of ['ACTIONLINT_VERSION="v1.7.12"','PINACT_VERSION="v0.1.2"','ZIZMOR_VERSION="1.27.0"','actionlint@${ACTIONLINT_VERSION}','pinact@${PINACT_VERSION}','zizmor==${ZIZMOR_VERSION}']) {
