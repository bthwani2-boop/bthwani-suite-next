...
const readJson = (relative) => {
  if (!exists(relative)) { violations.push({ file: relative, message: "REQUIRED_FILE_MISSING" }); return undefined; }
  try { return JSON.parse(readText(relative)); } catch (error) { violations.push({ file: relative, message: `INVALID_JSON ${error.message}` }); return undefined; }
};
...
const rolesRelative = "governance/contracts/sdlc/roles-and-authority.yaml";
const impactSchemaRelative = "governance/contracts/sdlc/change-impact.schema.json";
const artifactSchemaRelative = "governance/contracts/sdlc/artifact-manifest.schema.json";
const deliveryRelative = "governance/policies/delivery.md";
...
const delivery = readText(deliveryRelative);
...
for (const marker of ["finance", "isolation", "CLOSED_WITH_EVIDENCE", "protected approval"]) if (!delivery.includes(marker)) violations.push({ file: deliveryRelative, message: `DELIVERY_EVIDENCE_RULE_MISSING ${marker}` });
