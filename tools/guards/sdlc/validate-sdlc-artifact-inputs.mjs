...
const stageOrder = ["G0_INTAKE", "G1_PRODUCT_MODEL_APPROVED", "G2_DESIGN_APPROVED", "G3_READY_FOR_IMPLEMENTATION", "G4_IMPLEMENTATION_VERIFIED", "G5_PRODUCT_ACCEPTED", "G6_QA_APPROVED", "G7_SECURITY_APPROVED", "G8_RELEASE_APPROVED", "G9_DEPLOYED", "G10_PRODUCTION_VERIFIED", "CLOSED_WITH_EVIDENCE"];
const getArg = (name) => {
...
function readAndValidate(inputRelative, schemaRelative) {
  ...
  if (!fs.existsSync(inputPath)) { violations.push({ file: inputRelative, message: "MISSING_SDLC_INPUT_FILE" }); return undefined; }
  if (!fs.existsSync(schemaPath)) { violations.push({ file: schemaRelative, message: "MISSING_SDLC_SCHEMA" }); return undefined; }
  try {
    const input = JSON.parse(fs.readFileSync(inputPath, "utf8"));
    const schema = JSON.parse(fs.readFileSync(schemaPath, "utf8"));
    const validate = ajv.compile(schema);
    if (!validate(input)) for (const error of validate.errors ?? []) violations.push({ file: inputRelative, message: `SCHEMA_VIOLATION ${error.instancePath} ${error.message}` });
    return input;
  } catch (error) { violations.push({ file: inputRelative, message: `INVALID_JSON_OR_SCHEMA ${error.message}` }); return undefined; }
}
const atOrAfter = (stage, threshold) => stageOrder.indexOf(stage) >= stageOrder.indexOf(threshold);
...
const artifact = artifactPath ? readAndValidate(artifactPath, "governance/contracts/sdlc/artifact-manifest.schema.json") : undefined;
const impact = impactPath ? readAndValidate(impactPath, "governance/contracts/sdlc/change-impact.schema.json") : undefined;
...
if (artifact && impact) {
  ...
  for (const approval of artifact.approvals ?? []) {
    ...
    if ((highRisk || impact.impacts.ci || impact.impacts.governance || impact.impacts.wltFinance) && artifact.changeAuthor && approval.approver === artifact.changeAuthor) violations.push({ file: artifactPath, message: `PROTECTED_CHANGE_SELF_APPROVAL ${approval.authority}` });
  }

  const requiredScopes = new Set(["static"]);
  if (impact.productImpact === "CHANGED") requiredScopes.add("product");
  for (const [impactName, scope] of [["runtime","runtime"],["visual","visual"],["qa","qa"],["security","security"],["wltFinance","finance"],["operatorContext","isolation"],["governance","governance"],["ci","ci"],["release","release"],["production","production"]])
  ...
  function requireApproval(authority, threshold, label) {
    ...
    if (!requiredApprovals.has(authority)) { violations.push({ file: artifactPath, message: `${label}_REQUIRED_APPROVAL_MISSING ${authority}` }); return; }
    if (passDecision) {
  ...
  if (passDecision) for (const authority of requiredApprovals) {
    const approval = approvalByAuthority.get(authority);
