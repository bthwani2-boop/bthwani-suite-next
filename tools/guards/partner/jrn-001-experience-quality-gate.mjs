...
const controlWorkspace = read("services/dsh/frontend/control-panel/partners/PartnersReviewQueueScreen.tsx");
const controlDetail = read("services/dsh/frontend/control-panel/partners/PartnerDetailUnifiedScreen.tsx");
...
const workspace = read("services/dsh/frontend/shared/partner/partner-workspace.controller.tsx");
...
assert.match(controlWorkspace, /useControlPanelSession/);
assert.match(controlWorkspace, /sessionState\.kind !== "authenticated"/);
assert.match(controlDetail, /role="tablist"/);
assert.match(controlDetail, /role="tab"/);
assert.match(controlDetail, /aria-selected/);
assert.match(controlDetail, /useControlPanelSession/);
...
assert.match(workspace, /const PAGE_SIZE = 50/);
assert.doesNotMatch(field, /fetch\s*\(/);
assert.doesNotMatch(partnerStatus, /fetch\s*\(/);
assert.doesNotMatch(controlWorkspace, /fetch\s*\(/);
assert.doesNotMatch(controlDetail, /fetch\s*\(/);

console.log("JRN-001 FS-14 accessibility, RTL, session authority, performance and weak-network gate passed");
