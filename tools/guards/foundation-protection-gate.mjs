...
const policy = readJson(
  path.relative(repoRoot, policyPath).replaceAll(path.sep, "/"),
);
...
if (failures.length > 0) {
  console.error("Foundation protection gate failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(
  `Foundation protection gate passed: ${policy.requiredFiles.length} files and ${policy.requiredMobileApps.length} mobile apps verified.`,
);
