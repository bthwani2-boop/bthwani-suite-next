...
const includePrefixes = (process.env.FULLSTACK_GUARD_INCLUDE ?? "")
  .split(",")
  .map((value) => value.trim())
  .filter(Boolean);

function isIncluded(file) {
  return includePrefixes.length === 0 || includePrefixes.some((prefix) => file.startsWith(prefix));
}
...
if (includePrefixes.length > 0) {
  console.log(`[${guardId}] affected-surface scope: ${includePrefixes.join(", ")}`);
}
