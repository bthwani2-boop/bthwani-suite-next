...
if (truthRaw) {
  try {
    ...
    if (truth.capabilityId !== "JRN_036_SETTLEMENTS_COMMISSIONS") failures.push("product truth capabilityId mismatch");
    const surfaces = new Set((truth.surfaces ?? []).map((surface) => surface.id));
    for (const requiredSurface of ["app-partner", "app-captain", "app-field", "control-panel", "backend", "database", "shared"]) {
      if (!surfaces.has(requiredSurface)) failures.push(`product truth missing required surface ${requiredSurface}`);
    }
...
const settlementBackend = requireIncludes("services/wlt/backend/internal/settlement/jrn036_evidence_settlement.go", [
...
const commissionBackend = requireIncludes("services/wlt/backend/internal/cod/jrn036_governed_commission.go", [
...
for (const forbiddenCallerAmount of ["feeBasisPoints\"", "netAmount\"", "refundAmount\""]) {
  if (dshSettlement.includes(forbiddenCallerAmount) && forbiddenCallerAmount !== "feeBasisPoints\"") {
...
requireIncludes("services/dsh/frontend/shared/finance-wlt-link/jrn036/jrn036.api.ts", [
