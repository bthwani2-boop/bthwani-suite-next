...
function alignClosureTest(source) {
  ...
  if (!source.includes(scopedAssertion)) {
    throw new Error('slice-closure closure test does not contain the scoped preparation-decision assertion.');
  }
  return source;
}
...
if (writeMode) {
  ...
  console.log(changed ? 'slice-closure contract artifacts and closure assertion synchronized.' : 'slice-closure contract artifacts and closure assertion already synchronized.');
} else {
  if (changed) {
    throw new Error('slice-closure contract artifacts are stale. Run: node tools/contracts/sync-handoff-reasons.mjs --write');
  }
  console.log('slice-closure contract artifacts and closure assertion are synchronized.');
}
