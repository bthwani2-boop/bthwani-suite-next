...
Reconstruct isolated branch `g` into the smallest correct canonical structure that preserves required Product/System/Data truth, restores end-to-end product/data/contract/UI parity, and eliminates unjustified inherited structure.
...
```text
00 START HERE
→ 01 BRANCH-WIDE CENSUS / LEDGERS / AUTHORITY REGISTRY
→ 02 CANONICAL TARGET / WINNER-LOSER / STRUCTURAL DELTA
→ 02A END-TO-END PRODUCT-DATA-CONTRACT-UI PARITY
→ 03 ROOT SELECTION / CANONICAL RECONSTRUCTION EXECUTION
→ 04 ELIMINATION / DELETION / LINE-FILE-DIRECTORY FINISHING
→ 05 REVERIFY / EXACT-HEAD CI / REDIAGNOSE / REPEAT
...
CONNECT EVERY REQUIRED END-TO-END TRUTH CHAIN.
...
## P0 — vertical end-to-end parity mandate

Backend and frontend may be separate layers. They may not become separate product truths.

For every material capability/journey, prove one continuous canonical chain where applicable:

```text
PRODUCT MEANING
→ ACTOR / JOURNEY / STATE / TRANSITION
→ DOMAIN OWNER
...
Any unexplained break is `PARITY_GAP`.

```text
CLEAN_BACKEND + CLEAN_FRONTEND + BROKEN_SEMANTIC_CHAIN = NOT_CLOSED
```

Frontend may own presentation/navigation/transient UI state. It may not independently own backend/domain business truth such as permission, eligibility, serviceability, financial state, order state, allowed actions, pricing/fees, availability, capability decisions, workflow transitions, or business validation.

## Fast garbage deletion law

Deletion ceremony is proportional to risk.

If a line/symbol/file/directory/package/script/workflow/dependency/test/screen/route/binding/API artifact is proven unused, unconsumed, unreachable, non-authoritative, not required by the current product/system or any supported external/runtime consumer, and has no state/contract/runtime/security/migration responsibility, delete it immediately.
...
→ QUICK ALL-CONSUMER / IMPORT / CALLER / ROUTE / REACHABILITY CHECK
→ DELETE_NOW
→ SEARCH REFERENCES AGAIN
→ RUN AFFECTED VERIFICATION
```

Before classifying backend/API/data artifacts unused, search all apps/surfaces, control panel, mobile clients, runtime calls, integrations, events/webhooks, scripts/tools, operations, and supported external consumers.
...
Structural, semantic, ownership, schema, contract, runtime, migration, rename/rehome, parity repair, and root-correct mutations remain forbidden until the complete baseline passes.
...
If deletion reveals semantic, vertical-parity, ownership, contract, data, runtime, or required-consumer change, stop the fast lane and return to the complete baseline.
...
STEP 9  BUILD COMPLETE END-TO-END CAPABILITY/JOURNEY PARITY MATRIX
STEP 10 RESOLVE/CLASSIFY ALL PARITY GAPS INTO THE STRUCTURAL DELTA
STEP 11 SYNTHESIZE/RANK TRUE CAUSAL ROOTS
STEP 12 SELECT HIGHEST PROVEN EXECUTABLE STRUCTURAL ROOT
ONLY THEN FIRST STRUCTURAL/SEMANTIC ROOT MUTATION
```

The baseline fails if any in-scope material artifact, boundary, capability, required screen/API/contract/binding/data authority, or parity break remains unresolved in a way that can change owner, writer, target topology, migration direction, deletion safety, or root ranking.

Before root selection require:
...
If three or more signals cluster around one responsibility—duplicate owners/writers/mappings, wrappers, parallel directories, conflicting defaults, multiple config sources, duplicated state logic, generated/manual repairs, backend/frontend semantic drift, repeated patches, unbounded compatibility—escalate from local repair to structural-root investigation.

`STOP PATCHING DESCENDANTS → RECONSTRUCT ROOT` when a common structural root is proven.

## High-risk topology and integration zones

`core/**` and `shared/**` require heightened justification; non-cross-cutting domain truth must be rehomed.

`infra/**`, `tools/**`, `.github/**`, scripts, workflows, runtime/config, tests, fixtures, mocks, dependencies, database/schema, API/contracts/generated bindings, frontend data layers, components, routes, and screens are first-class structural surfaces and receive the same owner/writer/purpose/reachability/disposition treatment.
...
A structural replacement/duplication/ownership/parity root that removes no losing writer/path/file/directory/wrapper/manual mirror or reconnects no broken consumer chain triggers `CLOSURE_SUSPICION` unless evidence proves no losing/broken structure existed.
