...
function Invoke-ProviderSmoke {
  ...
  if ($null -eq $provider) { throw "no non-mock provider is available for domain-039 smoke" }
...
function Invoke-PlatformP3Smoke {
  ...
  Invoke-ProviderSmoke -AccessToken $operatorToken -CorrelationId $correlationId
  ...
  Write-Host "Platform-control P3 multi-service runtime smoke including domain-039 providers: PASS"
}
