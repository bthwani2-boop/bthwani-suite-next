...
const validation = spawnSync(process.execPath, [path.join(repoRoot, "tools/scripts/check-local-media-contract.mjs"), "--mode", "runtime"], {
  cwd: repoRoot,
  encoding: "utf8",
  stdio: "inherit",
  windowsHide: true,
...
for (const item of manifest.media) {
  ...
  if (binding) {
    const stat = fs.statSync(path.join(mediaRoot, relativePath));
    assets.push(`(${[
      sql(binding.assetId), sql(relativePath), "NULL", sql(item.fileName), sql(item.mimeType), String(stat.size),
  ...
  if (item.entityType === "banner" && ["banner-001", "banner-002"].includes(item.entityLogicalKey)) {
    homeUpdates.push(`UPDATE dsh_home_banners SET image_url=${sql(`/dsh-media/${relativePath}`)}, is_active=TRUE, updated_at=NOW() WHERE id=${sql(item.entityLogicalKey)};`);
  }
  if (item.entityType === "promo" && ["promo-001", "promo-002"].includes(item.entityLogicalKey)) {
    homeUpdates.push(`UPDATE dsh_home_promos SET image_url=${sql(`/dsh-media/${relativePath}`)}, is_active=TRUE, updated_at=NOW() WHERE id=${sql(item.entityLogicalKey)};`);
  }
}
