...
for (const file of listFiles()) {
  if (file === "tools/scripts/contracts/foundation.mjs") continue;
  ...
  if (read(file).includes("contracts/master.openapi.yaml")) violations.push({ file, message: `stale canonical index reference; use ${canonicalIndex}` });
}
...
if (!exists(canonicalIndex)) {
  ...
  if (!/^paths:\s*\{\}\s*$/m.test(content)) violations.push({ file: canonicalIndex, message: "canonical index must own no operations and declare paths: {}" });
  if (scalar(content, "x-bthwani-contract-role") !== "MASTER_INDEX_ONLY") violations.push({ file: canonicalIndex, message: "canonical index must declare x-bthwani-contract-role: MASTER_INDEX_ONLY" });
  if (scalar(content, "x-bthwani-manual-paths-forbidden") !== "true") violations.push({ file: canonicalIndex, message: "canonical index must forbid manually owned paths" });
}
...
for (const entry of entries) {
  ...
  if (!exists(manifest)) violations.push({ file: manifest, message: `indexed context '${entry.name}' has no ownership manifest` });
  ...
  for (const candidate of ownedCandidates) {
    ...
    if (priorOwner && priorOwner !== entry.file) violations.push({ file: candidate.file, message: `contract file has multiple owners: ${priorOwner}, ${entry.file}` });
  ...
  for (const source of operationSources) {
    ...
    for (const operation of parseOpenApiContractContent(read(source.file), source.file)) {
      ...
      if (priorOperation) violations.push({ file: source.file, line: operation.line, message: `duplicate operation '${operationKey}' also defined in ${priorOperation.file}` });
      else operationOwners.set(operationKey, { owner: entry.file, file: source.file });

      if (operation.operationId) {
        const priorId = operationIdOwners.get(operation.operationId);
        if (priorId) violations.push({ file: source.file, line: operation.line, message: `duplicate operationId '${operation.operationId}' also defined in ${priorId}` });
        else operationIdOwners.set(operation.operationId, source.file);
      }
    }
  }
}
...
for (const file of contractFiles) {
  ...
  if (role === "MASTER_INDEX_ONLY" && file !== canonicalIndex) violations.push({ file, message: `MASTER_INDEX_ONLY is reserved for ${canonicalIndex}` });
  const state = scalar(content, "x-bthwani-contract-state");
  if (!state) violations.push({ file, message: "missing x-bthwani-contract-state" });
  else if (!allowedStates.has(state)) violations.push({ file, message: `invalid x-bthwani-contract-state '${state}'` });
}

if (entries.length > 0) {
  const digest = crypto.createHash("sha256");
  for (const file of [canonicalIndex, ...[...ownedFiles.keys()].sort()]) digest.update(`${file}\n${read(file)}\n`);
