...
test("Sonar trusted analysis has one explicit trusted opt-in", () => {
  ...
  const baseline = read(".github/workflows/repository-baseline.yml");
  ...
  assert.match(baseline, /trusted_scan: true/u);
  assert.doesNotMatch(baseline, /head_ref: master|base_ref: master/u);
