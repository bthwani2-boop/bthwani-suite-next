...
for (const [key, dir, slug, androidPackage] of apps) {
  ...
  for (const [dep, expected] of Object.entries(requiredDeps)) {
    if (pkg.dependencies?.[dep] !== expected) {
      fail(`${key}: ${dep} expected ${expected}, found ${pkg.dependencies?.[dep]}`);
    }
  }
  ...
  if (!config.includes(`slug: "${slug}"`)) fail(`${key}: slug mismatch`);
  if (!config.includes(`package: "${androidPackage}"`)) fail(`${key}: android package mismatch`);
  if (config.includes('"web"') || /\bweb\b/.test(config)) fail(`${key}: web is forbidden in Expo mobile config`);
}
