...
function Wait-Providers {
  ...
  for ($Attempt = 1; $Attempt -le 30; $Attempt++) {
    try {
      $Response = Invoke-RestMethod -Uri $Url -TimeoutSec 5 -ErrorAction Stop
      $Items = @($Response.providers); $Kinds = @($Items | ForEach-Object { $_.kind })
...
try {
  & pwsh -NoProfile -ExecutionPolicy Bypass -File $RuntimeScript -Action reset -Profiles $CoreProfiles -Force
  if ($LASTEXITCODE -ne 0) { throw "Core runtime reset failed." }

  Invoke-Compose @("--profile", "providers", "--profile", "platform", "up", "-d", "postgres")
  ...
  $MultiSurfaceScript = Join-Path $RepoRoot "tools/scripts/test-dsh-multisurface-runtime-matrix.ps1"
  if (Test-Path -LiteralPath $MultiSurfaceScript) {
    $Evidence.multiSurfaceJourney = Invoke-EvidenceScript "DSH multi-surface journey" $MultiSurfaceScript
  }
  ...
  if ($Cleanup) {
    try { & pwsh -NoProfile -ExecutionPolicy Bypass -File $RuntimeScript -Action down -Profiles $CoreProfiles } catch { }
