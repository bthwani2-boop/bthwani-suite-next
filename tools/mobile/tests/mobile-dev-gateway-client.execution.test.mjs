...
test("gateway client policy rejects public and loopback gateway origins", () => {
  process.env.EXPO_PUBLIC_BTHWANI_DEV_GATEWAY_BASE_URL = "http://8.8.8.8:58110";
  assert.throws(() => resolveMobileDevGatewayBaseUrl(), /MOBILE_DEV_GATEWAY_BASE_URL_INVALID/);

  process.env.EXPO_PUBLIC_BTHWANI_DEV_GATEWAY_BASE_URL = "http://127.0.0.1:58110";
  assert.throws(() => resolveMobileDevGatewayBaseUrl(), /MOBILE_DEV_GATEWAY_BASE_URL_INVALID/);
});

test("gateway client policy requires a per-run capability for developer sessions", () => {
  process.env.EXPO_PUBLIC_BTHWANI_DEV_GATEWAY_BASE_URL = "http://192.168.1.20:58110";
  delete process.env.EXPO_PUBLIC_BTHWANI_DEV_GATEWAY_TOKEN;
  assert.equal(resolveMobileDevGatewayBaseUrl(), "http://192.168.1.20:58110");
...
test("only signed loopback MinIO URLs are rewritten through the mobile gateway", () => {
  ...
  assert.equal(
    ...
    `http://192.168.1.20:58110/__media/dsh-media/a.jpg?${query}`,
  );
  assert.equal(
