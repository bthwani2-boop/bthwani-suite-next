...
export function PartnerDocumentsScreen() {
  ...
  if (c.statusState.kind === "error") {
    return (
      <View style={styles.center}>
        <Text style={styles.error}>{c.statusState.message}</Text>
        <TouchableOpacity style={styles.retryBtn} onPress={c.reload}>
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {docsState.kind === "success" && docsState.documents.length === 0 && (
        ...
        return (
          <View key={doc.id} style={styles.docCard}>
            <View style={styles.docHeader}>
              ...
              <View style={[styles.statusBadge, { backgroundColor: toneBg(statusTone) }]}>
                <Text style={[styles.statusText, { color: toneFg(statusTone) }]}>{statusLabel}</Text>
              </View>
            </View>
            {doc.rejectionReason ? (
