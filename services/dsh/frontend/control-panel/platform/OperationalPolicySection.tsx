...
export function OperationalPolicySection({
  ...
  }, [canRollback, load, modes, profile, rollbackReason, selectedAudit, selectedZone, zones]);
  ...
  if (!canReadZones) {
    return (
      <CpStatePanel
        ...
        description="لن يطلب هذا القسم الملف التشغيلي أو أنماط التنفيذ قبل تحقق صلاحيات القراءة التشغيلية."
    ...
    <View style={styles.section}>
      <View style={styles.headerRow}>
        <View style={styles.grow}>
          <Text role="titleSm">القرار التشغيلي الموحد</Text>
      ...
      {zones.state.kind === "success" && zoneRows.length > 0 ? (
        <CpTable aria-label="مناطق السياسة التشغيلية">
          <thead>
            <tr>
      ...
      <Card style={styles.card}>
        ...
        {auditRows.length === 0 ? <CpStatePanel role="status" title="لا توجد أحداث للمنطقة المختارة" /> : (
          <CpTable aria-label="تدقيق السياسة التشغيلية">
            ...
            <tbody>
              {auditRows.map((row) => (
                  <tr key={row.id} onClick={canRollback ? () => setSelectedAudit(row) : undefined}>
