...
export function SovereignLeadershipPanel() {
  ...
  const [positionTitle, setPositionTitle] = useState("مدير العمليات");
  ...
  const canSubmit = useMemo(
    () => fullNameAr.trim().length > 0 && phoneE164.trim().length >= 9 && department.trim().length > 1 && positionTitle.trim().length > 0 && !submitting,
    [department, fullNameAr, phoneE164, positionTitle, submitting],
  );
  ...
  const changeClass = (value: LeadershipEmploymentClass) => {
    ...
    if (value === "project_manager") {
      ...
      setPositionTitle("مدير المشروع");
    } else if (value === "coordinator" || value === "executive") {
      setDepartment("platform");
      setPositionTitle(value === "coordinator" ? "منسق المنصة" : "إدارة تنفيذية");
    }
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <section style={{ padding: "20px", border: "1px solid var(--bthwani-control-panel-border)", borderRadius: "12px", display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: "12px" }}>
          ...
          <CpTextInput value={department} onChange={setDepartment} aria-label="رمز القسم" placeholder="operations" />
          <CpTextInput value={positionTitle} onChange={setPositionTitle} aria-label="المسمى الوظيفي" placeholder="مدير العمليات" />
          <CpTextInput value={jobGrade} onChange={setJobGrade} aria-label="الدرجة الوظيفية" placeholder="الدرجة الوظيفية" />
          <CpTextInput value={officeLocation} onChange={setOfficeLocation} aria-label="موقع العمل" placeholder="المقر الرئيسي" />
          ...
          <label style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            <select value={permissionBundle} onChange={(event) => setPermissionBundle(event.target.value as LeadershipPermissionBundle)} style={selectStyle}>
              {Object.entries(BUNDLE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
            </select>
          </label>
          <CpTextInput value={assignmentStartsOn} onChange={setAssignmentStartsOn} aria-label="بداية التكليف" placeholder="YYYY-MM-DD بداية التكليف" />
          <CpTextInput value={assignmentEndsOn} onChange={setAssignmentEndsOn} aria-label="نهاية التكليف" placeholder="YYYY-MM-DD نهاية اختيارية" />
        </div>
