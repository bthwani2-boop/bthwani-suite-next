...
const RELATIONSHIP_TYPES: readonly MasterProductRelationshipType[] = ["substitute", "alternative", "complement"];
...
export function CatalogGovernanceScreen() {
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      ...
      setSelectedAttributeId((current) => current || attributeItems[0]?.id || "");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : String(caught));
    } finally {
      setLoading(false);
  ...
  const loadProductGovernance = async () => {
    if (!productId.trim()) { setError("أدخل معرف المنتج المركزي أولاً."); return; }
    setLoading(true);
    setError(null);
    try {
  ...
  const loadPauses = async () => {
    if (!storeId.trim()) { setError("أدخل معرف المتجر أولاً."); return; }
    setLoading(true);
    setError(null);
    try { setPauses(await fetchOperatorAssortmentPauses(storeId.trim())); }
    catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
    finally { setLoading(false); }
  };

  if (loading && attributes.length === 0 && audit.length === 0) {
    return <CpStatePanel role="status" title="جاري تحميل غرفة حوكمة الكتالوج" description="تُقرأ الحقيقة من DSH." code="LOADING" />;
  }

  return (
    <OperationsRoomFrame>
      <CpPageHeader title="غرفة حوكمة الكتالوج المركزي">
        <p>خصائص المنتجات، البدائل، الإيقاف المؤقت، التدقيق والتراجع المحكوم — بلا كتالوج محلي موازٍ.</p>
      </CpPageHeader>

      {error ? <CpStatePanel role="alert" title="تعذر إكمال العملية" description={error} code="CATALOG_GOVERNANCE_ERROR" /> : null}
      {notice ? <CpStatePanel role="status" title="تم التنفيذ" description={notice} code="SAVED" /> : null}
