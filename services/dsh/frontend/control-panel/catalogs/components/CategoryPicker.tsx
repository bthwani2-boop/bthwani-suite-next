...
export function CategoryPicker({ value, onChange, domains, nodes, label = "اختر الفئة / التصنيف" }: CategoryPickerProps) {
  ...
  return (
    <Box style={{ position: "relative", width: 300, zIndex: 10 }}>
      ...
      />
      {isOpen && (
        <Box style={{ position: "absolute", top: 60, left: 0, right: 0, backgroundColor: "var(--bth-colors-surfaceBase)", borderWidth: 1, borderColor: "var(--bth-colors-borderColor)", zIndex: 20, maxHeight: 250, overflow: "hidden", borderRadius: 8 }}>
          {options.length === 0 && (
