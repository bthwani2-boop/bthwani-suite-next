  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      {assignments.length === 0 ? <CpStatePanel role="status" title="لا توجد إسنادات" description="أنشئ مهمة إدخال متجر من النموذج أعلاه." /> : assignments.map((item) => (
        <section key={item.id} style={{ padding: 20, border: "1px solid var(--bthwani-control-panel-border)", borderRadius: 16, background: "var(--bthwani-control-panel-surface)", boxShadow: "0 8px 24px var(--bthwani-control-panel-border)", display: "flex", flexDirection: "column", gap: 10 }}>
