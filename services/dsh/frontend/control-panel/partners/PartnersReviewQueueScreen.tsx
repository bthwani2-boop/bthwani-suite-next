...
import { getPartnerActivationStatusLabel } from "../../shared/partner";
...
export function PartnersReviewQueueScreen() {
  const identity = useIdentitySession();
  const c = usePartnerAdminController();
  ...
  if (c.selectedPartnerId && c.detailState.kind !== "idle") {
    return (
      <DataTablePageFrame
        dir="rtl"
        header={
          <CpPageHeader title="تفاصيل الشريك">
            <CpButton onClick={() => c.selectPartner(null)}>← العودة للقائمة</CpButton>
          </CpPageHeader>
        }
      >
        <PartnerDetailPanel
          detailState={c.detailState}
          actionState={c.actionState}
          documentActionState={c.documentActionState}
          readinessState={c.readinessState}
          onTransition={(toStatus, reason) => void c.transition(c.selectedPartnerId!, toStatus, reason)}
          onReviewDocument={(docId, decision, reason) => void c.reviewDocument(c.selectedPartnerId!, docId, decision, reason)}
          onLoadReadiness={() => void c.loadPartnerReadiness(c.selectedPartnerId!)}
        />
      </DataTablePageFrame>
    );
  }
      ...
      filters={
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", direction: "rtl" }}>
          {STATUS_FILTER_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              onClick={() => c.setStatusFilter(opt.value)}
              style={{
                padding: "0.375rem 0.875rem",
                borderRadius: "1rem",
                border: "1px solid",
                borderColor: c.statusFilter === opt.value ? "var(--dsh-text-primary)" : "var(--dsh-card-border)",
                backgroundColor: c.statusFilter === opt.value ? "var(--dsh-text-primary)" : "transparent",
                color: c.statusFilter === opt.value ? "white" : "var(--dsh-text-secondary)",
                cursor: "pointer",
                fontSize: "0.875rem",
          ...
          <tbody>
            {c.visibleRows.map((row) => (
              <tr key={row.id} style={{ borderBottom: `1px solid var(--dsh-content-bg)` }}>
                ...
                <td style={{ padding: "0.875rem 1rem" }}>
                  <StatusBadge status={row.activationStatus} />
                </td>
                ...
                <td style={{ padding: "0.875rem 1rem" }}>
                  <button
                    onClick={() => c.selectPartner(row.id)}
...
function StatusBadge({ status }: { status: DshPartnerActivationStatus }) {
  ...
  return (
    <span style={{
      ...
      {getPartnerActivationStatusLabel(status)}
    </span>
  );
}
