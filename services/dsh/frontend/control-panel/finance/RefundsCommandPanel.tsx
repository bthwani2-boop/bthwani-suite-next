...
} from "../../shared/finance-wlt-link/wlt-refund/wlt-refund.queries";
import type { WltRefundResponse } from "../../shared/finance-wlt-link/wlt-refund/wlt-refund.queries";
...
export function RefundsCommandPanel() {
  ...
  const selected = refunds.find((r) => r.id === selectedRefundId) 
    ?? createMutation.data 
    ...
    <div dir="rtl" aria-busy={busy} style={{ display: "grid", gap: "1rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", gap: "1rem" }}>
        <Card style={{ padding: "1rem", display: "grid", gap: "0.7rem", alignContent: "start" }}>
          ...
          {refundsQuery.isSuccess && refunds.length === 0 ? <Text role="body" tone="muted">لا توجد استردادات محمّلة.</Text> : refunds.map((refund) => (
            <button
              key={refund.id}
              type="button"
          ...
          {!selected ? <Text role="body" tone="muted">اختر استردادًا لعرض الإجراءات.</Text> : (
            <>
              ...
              <div role="region" aria-label="سجل تدقيق الاسترداد" style={{ display: "grid", gap: "0.35rem", paddingTop: "0.5rem" }}>
                ...
                {auditQuery.isSuccess ? auditQuery.data?.map((event) => (
