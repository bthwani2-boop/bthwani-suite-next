...
function actionsForStatus(status: string): readonly PayoutAction[] {
  switch (status) {
    ...
    case "processing":
      return [
        { id: "complete", label: "تأكيد الاكتمال والترحيل", tone: "success", run: completePayoutRequest },
      ];
    default:
      return [];
  }
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", flexWrap: "wrap" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", minWidth: "260px" }}>
                    ...
                    <Text role="caption" tone="muted">
                      المستفيد: {request.beneficiaryActorId} ({request.beneficiaryActorType})
                    </Text>
                    <Text role="caption" tone="muted">المبلغ: {formatMoney(request.amountMinorUnits, request.currency)}</Text>
                    {request.providerReference ? (
                      <Text role="caption">مرجع المزود: {request.providerReference}</Text>
                    ) : null}
                    {request.providerStatus ? (
                      <Text role="caption" tone="muted">حالة المزود: {request.providerStatus}</Text>
                    ) : null}
