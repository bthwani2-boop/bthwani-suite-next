...
export type DshOperatorDispatchMapLocation = {
  readonly assignmentId: string;
  readonly orderId: string;
  readonly captainId: string;
  readonly latitude: number;
...
export type DshOperatorDispatchTrackingOverview = {
  readonly alerts: readonly DshDispatchTrackingAlert[];
  readonly total: number;
  readonly locations: readonly DshOperatorDispatchMapLocation[];
...
export async function fetchOperatorDispatchTrackingAlerts(): Promise<DshOperatorDispatchTrackingOverview> {
  const data = await request<DshOperatorDispatchTrackingOverview>(
    "/dsh/operator/dispatch/tracking-alerts",
  );
  return {
    alerts: data.alerts ?? [],
    total: Number.isFinite(data.total) ? data.total : data.alerts?.length ?? 0,
    locations: data.locations ?? [],
    locationsTotal: Number.isFinite(data.locationsTotal)
      ? data.locationsTotal
