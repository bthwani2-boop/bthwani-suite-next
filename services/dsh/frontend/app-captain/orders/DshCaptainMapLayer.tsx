...
const mapDemandZones: readonly MapHeatZone[] = [
  { id: 'demand-1', top: 58, right: 34, size: 164, color: 'rgba(229, 57, 53, 0.20)', label: 'طلب مرتفع' },
  { id: 'demand-2', top: 188, left: 26, size: 118, color: 'rgba(229, 57, 53, 0.14)', label: 'ذروة قريبة' },
...
const mapCaptainZones: readonly MapHeatZone[] = [
  { id: 'captain-1', top: 128, left: 112, size: 132, color: 'rgba(198, 40, 40, 0.14)', label: 'كباتن أكثر' },
  { id: 'captain-2', bottom: 138, left: 154, size: 104, color: 'rgba(198, 40, 40, 0.10)', label: 'تغطية قريبة' },
];
    ...
    <Box style={{ flex: 1, position: 'relative' }}>
      <SurfaceAny tone="inset" padding={0} gap={0} radiusToken="xl" style={{ flex: 1, overflow: 'hidden', borderColor: theme.lineStrong } as any}>
        <Box style={{ flex: 1, backgroundColor: theme.surfaceSecondary, overflow: 'hidden' }}>
          ...
          <Box style={{ position: 'absolute', top: 78, left: 40, width: 7, height: 222, borderRadius: 999, backgroundColor: 'rgba(198, 40, 40, 0.10)' }} />
          <Box style={{ position: 'absolute', top: 136, left: 40, right: 74, height: 7, borderRadius: 999, backgroundColor: 'rgba(198, 40, 40, 0.08)' }} />
          <Box style={{ position: 'absolute', top: 214, right: 58, width: 148, height: 7, borderRadius: 999, backgroundColor: 'rgba(198, 40, 40, 0.08)', transform: [{ rotate: '-18deg' }] }} />
          <Box style={{ position: 'absolute', bottom: 122, left: 92, right: 42, height: 7, borderRadius: 999, backgroundColor: 'rgba(198, 40, 40, 0.06)', transform: [{ rotate: '14deg' }] }} />

          {mapDemandZones.map((zone) => (
            <Box key={zone.id} style={{ position: 'absolute', width: zone.size, height: zone.size, borderRadius: zone.size / 2, backgroundColor: zone.color, borderWidth: borders.hairline, borderColor: 'rgba(229, 57, 53, 0.12)', ...(zone.top != null ? { top: zone.top } : {}), ...(zone.bottom != null ? { bottom: zone.bottom } : {}), ...(zone.left != null ? { left: zone.left } : {}), ...(zone.right != null ? { right: zone.right } : {}) } as any} />
          ))}

          {mapCaptainZones.map((zone) => (
            <Box key={zone.id} style={{ position: 'absolute', width: zone.size, height: zone.size, borderRadius: zone.size / 2, backgroundColor: zone.color, borderWidth: borders.hairline, borderColor: 'rgba(198, 40, 40, 0.12)', ...(zone.top != null ? { top: zone.top } : {}), ...(zone.bottom != null ? { bottom: zone.bottom } : {}), ...(zone.left != null ? { left: zone.left } : {}), ...(zone.right != null ? { right: zone.right } : {}) } as any} />
          ))}
              ...
              <Box layoutDirection="row" align="center" gap={1} style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-start' }}>
                <Box layoutDirection="row" align="center" gap={1} paddingX={1} paddingY={1} radiusToken="pill" background="surfaceRaised" border borderTone="line">
                  <Box layoutDirection="row" align="center" gap={1}>
                    <Box style={{ width: 8, height: 8, borderRadius: 999, backgroundColor: 'rgba(229, 57, 53, 0.95)' }} />
                  </Box>
                  <Box layoutDirection="row" align="center" gap={1}>
                    <Box style={{ width: 8, height: 8, borderRadius: 999, backgroundColor: 'rgba(198, 40, 40, 0.95)' }} />
                  </Box>
                </Box>
              </Box>
            </SurfaceAny>
