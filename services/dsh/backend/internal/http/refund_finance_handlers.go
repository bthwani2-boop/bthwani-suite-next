...
func refundRequestIdentity(r *http.Request, fallbackParts ...string) (string, string) {
	...
	if correlationID == "" { correlationID = "refund-" + digest[:24] }
	if idempotencyKey == "" { idempotencyKey = "refund-" + digest }
...
func (s *protectedStoreServer) writeRefundCommand(w http.ResponseWriter, r *http.Request, path, tenantID, idempotencyKey string, body []byte) {
	...
	correlationID := strings.TrimSpace(r.Header.Get("X-Correlation-ID"))
	if correlationID == "" { correlationID = "refund-" + refundHash(path, tenantID, string(body))[:24] }
...
func (s *protectedStoreServer) refundDecisionCommand(w http.ResponseWriter, r *http.Request, action string) {
	actor, ok := s.requirePermission(w, r, "control-panel", FinancePermissionManage, "operator")
	if !ok { return }
	tenantID, ok := requiredPaymentTenant(w, r); if !ok { return }
	refundID := strings.TrimSpace(r.PathValue("refundId")); if refundID == "" { store.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "refundId is required"); return }
	var input refundCommandDecision
	decoder := json.NewDecoder(http.MaxBytesReader(w, r.Body, 16*1024)); decoder.DisallowUnknownFields()
	if err := decoder.Decode(&input); err != nil { store.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "request body is invalid"); return }
	input.Reason = strings.TrimSpace(input.Reason); if input.Reason == "" { store.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "reason is required"); return }
...
func (s *protectedStoreServer) handleApproveFinanceRefund(w http.ResponseWriter, r *http.Request) { s.refundDecisionCommand(w, r, "approve") }
func (s *protectedStoreServer) handleRejectFinanceRefund(w http.ResponseWriter, r *http.Request) { s.refundDecisionCommand(w, r, "reject") }

// POST /dsh/control-panel/finance/refunds/{refundId}/complete
func (s *protectedStoreServer) handleCompleteFinanceRefund(w http.ResponseWriter, r *http.Request) {
	actor, ok := s.requirePermission(w, r, "control-panel", FinancePermissionManage, "operator"); if !ok { return }
	tenantID, ok := requiredPaymentTenant(w, r); if !ok { return }
	refundID := strings.TrimSpace(r.PathValue("refundId")); if refundID == "" { store.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "refundId is required"); return }
...
// POST /dsh/control-panel/finance/refunds/{refundId}/reconcile
func (s *protectedStoreServer) handleReconcileFinanceRefund(w http.ResponseWriter, r *http.Request) {
	...
	decoder := json.NewDecoder(http.MaxBytesReader(w, r.Body, 32*1024)); decoder.DisallowUnknownFields()
	if err := decoder.Decode(&input); err != nil { store.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "request body is invalid"); return }
	input.ResolutionAction = strings.TrimSpace(input.ResolutionAction); input.EvidenceNote = strings.TrimSpace(input.EvidenceNote); input.ProviderReference = strings.TrimSpace(input.ProviderReference)
	...
	body, _ := json.Marshal(map[string]string{"operatorId": actor.ID, "resolutionAction": input.ResolutionAction, "evidenceNote": input.EvidenceNote, "providerReference": input.ProviderReference})
...
// GET /dsh/control-panel/finance/refunds/{refundId}/audit
func (s *protectedStoreServer) handleFinanceRefundAudit(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { store.SendError(w, http.StatusBadGateway, "WLT_UNAVAILABLE", "WLT refund audit read failed"); return }
	writeWltProxyResponse(w, status, body)
}

func privacyRefunds(body []byte) ([]privacyRefund, error) {
	var envelope refundReadEnvelope
	if err := json.Unmarshal(body, &envelope); err != nil { return nil, err }
	out := make([]privacyRefund, 0, len(envelope.Refunds))
	for _, item := range envelope.Refunds {
		out = append(out, privacyRefund{ID:item.ID, OrderID:item.OrderID, AmountMinorUnits:item.AmountMinorUnits, Currency:item.Currency, Status:item.Status, ResolvedAt:item.ResolvedAt, CreatedAt:item.CreatedAt, UpdatedAt:item.UpdatedAt})
	}
	return out, nil
}
...
func (s *protectedStoreServer) proxyPrivacyRefunds(w http.ResponseWriter, r *http.Request, tenantID, orderID, clientID string) {
	...
	if err != nil { store.SendError(w, http.StatusBadGateway, "WLT_UNAVAILABLE", "WLT refund read failed"); return }
	if status < 200 || status >= 300 { writeWltProxyResponse(w, status, body); return }
	items, err := privacyRefunds(body); if err != nil { store.SendError(w, http.StatusBadGateway, "WLT_INVALID_RESPONSE", "WLT refund response was invalid"); return }
	store.SendJSON(w, http.StatusOK, map[string]any{"refunds": items})
}

// GET /dsh/client/orders/{orderId}/refunds
func (s *protectedStoreServer) handleClientOrderRefunds(w http.ResponseWriter, r *http.Request) {
	actor, ok := s.requireActor(w, r, "client"); if !ok { return }
	tenantID, ok := requiredPaymentTenant(w, r); if !ok { return }
	orderID := strings.TrimSpace(r.PathValue("orderId")); if orderID == "" { store.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "orderId is required"); return }
	var exists bool
	if err := s.db.QueryRowContext(r.Context(), `SELECT EXISTS(SELECT 1 FROM dsh_orders WHERE id=$1::uuid AND tenant_id=$2 AND client_id=$3)`, orderID, tenantID, actor.ID).Scan(&exists); err != nil || !exists {
...
// GET /dsh/partner/orders/{orderId}/refunds
func (s *protectedStoreServer) handlePartnerOrderRefunds(w http.ResponseWriter, r *http.Request) {
	_, storeID, ok := s.partnerStore(w, r); if !ok { return }
	tenantID, ok := requiredPaymentTenant(w, r); if !ok { return }
	orderID := strings.TrimSpace(r.PathValue("orderId")); if orderID == "" { store.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "orderId is required"); return }
	var exists bool
	if err := s.db.QueryRowContext(r.Context(), `SELECT EXISTS(SELECT 1 FROM dsh_orders WHERE id=$1::uuid AND tenant_id=$2 AND store_id=$3)`, orderID, tenantID, storeID).Scan(&exists); err != nil || !exists {
