...
func (s *protectedStoreServer) handleListCatalogApprovals(w http.ResponseWriter, r *http.Request) {
	actor, ok := s.requirePermission(w, r, "control-panel", CatalogApprovalPermissionRead)
	if !ok {
		return
	}
	operatorContextID, ok := requireCatalogApprovalOperatorContext(w, actor)
...
func (s *protectedStoreServer) handleTransitionCatalogApproval(w http.ResponseWriter, r *http.Request) {
	actor, ok := s.requirePermission(w, r, "control-panel", CatalogApprovalPermissionManage)
	if !ok {
		return
	}
	operatorContextID, ok := requireCatalogApprovalOperatorContext(w, actor)
