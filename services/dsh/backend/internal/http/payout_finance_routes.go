...
// require their dedicated finance destination actions at the DSH boundary.
func registerPayoutFinanceRoutes(mux *http.ServeMux, s *protectedStoreServer) {
	...
	verify = s.withPermission("control-panel", "finance.payout_destinations.verify", verify)
	...
	deactivate = s.withPermission("control-panel", "finance.payout_destinations.deactivate", deactivate)
