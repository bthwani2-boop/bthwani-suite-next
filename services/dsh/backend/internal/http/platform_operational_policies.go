...
func writePlatformPolicyError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, platformpolicies.ErrInvalid):
		store.SendError(
			...
			"platform policy input is invalid",
		)
	case errors.Is(err, platformpolicies.ErrNotFound):
		store.SendError(
			...
			"platform policy record was not found",
		)
	case errors.Is(err, platformpolicies.ErrVersionConflict):
		store.SendError(
			...
			http.StatusConflict,
			"VERSION_CONFLICT",
			"platform policy changed; reload before retrying",
		)
	case errors.Is(err, platformpolicies.ErrIdempotencyConflict):
		store.SendError(
			...
			"idempotency key was reused with a different platform policy request",
		)
	default:
		store.SendError(
			w,
			...
			"INTERNAL_ERROR",
			"platform policy operation failed",
		)
	}
}
...
func (s *protectedStoreServer) handleListPlatformZones(
	w http.ResponseWriter,
	r *http.Request,
) {
	...
	if raw := strings.TrimSpace(r.URL.Query().Get("includeInactive")); raw != "" {
		...
		if err != nil {
			store.SendError(
				...
				http.StatusBadRequest,
				"INVALID_QUERY",
				"includeInactive must be boolean",
			)
			return
		}
...
func (s *protectedStoreServer) handleCreatePlatformZone(
	w http.ResponseWriter,
	r *http.Request,
) {
	...
	item, err := platformpolicies.CreateZone(
		r.Context(),
		s.db,
		platformpolicies.CreateZoneInput{
			ID:          body.ID,
			Name:        body.Name,
...
func (s *protectedStoreServer) handleUpdatePlatformZone(
	w http.ResponseWriter,
	r *http.Request,
) {
	actor, ok := s.requirePermission(
	...
	item, err := platformpolicies.UpdateZone(
		r.Context(),
		s.db,
		r.PathValue("zoneId"),
		platformpolicies.UpdateZoneInput{
			Name:            body.Name,
			Description:     body.Description,
...
func (s *protectedStoreServer) handleListPlatformSlaRules(
	w http.ResponseWriter,
	r *http.Request,
) {
	if _, ok := s.requirePermission(
	...
	items, err := platformpolicies.ListSlaRules(
		r.Context(),
		s.db,
		r.URL.Query().Get("zoneId"),
	)
	if err != nil {
		writePlatformPolicyError(w, err)
		return
...
func (s *protectedStoreServer) handleUpsertPlatformSlaRule(
	w http.ResponseWriter,
	r *http.Request,
) {
	actor, ok := s.requirePermission(
	...
	item, err := platformpolicies.UpsertSlaRule(
		r.Context(),
		s.db,
		platformpolicies.UpsertSlaInput{
			ZoneID:          body.ZoneID,
...
func (s *protectedStoreServer) handleGetPlatformCapacity(
	w http.ResponseWriter,
	r *http.Request,
) {
	if _, ok := s.requirePermission(
	...
	item, err := platformpolicies.GetCapacity(
		r.Context(),
		s.db,
		r.URL.Query().Get("zoneId"),
	)
...
func (s *protectedStoreServer) handleUpsertPlatformCapacity(
	w http.ResponseWriter,
	r *http.Request,
) {
	actor, ok := s.requirePermission(
	...
	item, err := platformpolicies.UpsertCapacity(
		r.Context(),
		s.db,
		platformpolicies.UpsertCapacityInput{
			ZoneID:              body.ZoneID,
...
func (s *protectedStoreServer) handleGetPlatformZoneServiceability(
	w http.ResponseWriter,
	r *http.Request,
) {
	if _, ok := s.requirePermission(
	...
	item, err := platformpolicies.GetZoneServiceability(
		r.Context(),
		s.db,
		r.PathValue("zoneId"),
	)
