...
func (s *protectedStoreServer) handleAssignPartnerDelivery(w http.ResponseWriter, r *http.Request) {
	...
	task, err := svc.AssignCourier(r.Context(), orderID, body.StoreCourierID, actor.ID, actor.Role, partnerDeliveryCorrelationID(r, body.CorrelationID))
	if err != nil {
...
// pickup/depart/arrive partner-side actions: resolve the task by orderId,
// decode the common mutation body, and delegate to the given service call.
