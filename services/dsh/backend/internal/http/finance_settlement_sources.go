...
func (s *protectedStoreServer) handleCreateFinanceSettlementFromDeliveredOrders(w http.ResponseWriter, r *http.Request) {
	...
	status, responseBody, err := s.wlt.ExecuteFinanceWrite(r.Context(), "finance.settlements.create", http.MethodPost, "/wlt/settlements", payload, r.Header.Get("X-Correlation-ID"), r.Header.Get("Idempotency-Key"), actor.OperatorContextID)
...
func (s *protectedStoreServer) handleUpsertFinanceSettlementPolicy(w http.ResponseWriter, r *http.Request) {
	...
	status, responseBody, err := s.wlt.ExecuteFinanceWrite(r.Context(), "finance.settlement_policies.upsert", http.MethodPut, path, bytes.Clone(payload), r.Header.Get("X-Correlation-ID"), r.Header.Get("Idempotency-Key"), actor.OperatorContextID)
