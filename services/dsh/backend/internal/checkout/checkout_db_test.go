...
func TestCancelIntentEnqueuesExpireSessionWhenPaymentSessionExistsDBIntegration(t *testing.T) {
	...
	if _, err := AttachWltPaymentSessionIdempotent(db, intent.ID, operatorContextID, clientID, paymentSessionID); err != nil {
		t.Fatalf("AttachWltPaymentSessionIdempotent failed: %v", err)
	}
