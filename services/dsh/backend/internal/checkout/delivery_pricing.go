...
func ResolveDeliveryPricingTx(
	...
	err := tx.QueryRowContext(ctx, `
		SELECT p.store_id,p.fulfillment_mode,p.pricing_mode,p.fee_minor_units,p.currency,p.pricing_config,p.status,p.pricing_source,p.version
		...
		&policy.StoreID, &policy.FulfillmentMode, &policy.PricingMode, &policy.FeeMinorUnits,
		&policy.Currency, &policy.PricingConfig, &policy.Status, &policy.PricingSource, &policy.Version,
	)
	...
	if policy.PricingMode == "free_delivery" {
		policy.FeeMinorUnits = 0
	}
