...
func TestDefaultWorkflowStageMatchesCustomerAndDeliveryReadback(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			stage := defaultStageFor(tt.requestType, tt.status)
			if stage == nil {
				t.Fatalf("default stage is nil; want %q", tt.want)
			}
			if *stage != tt.want {
				t.Fatalf("default stage = %q; want %q", *stage, tt.want)
			}
			rule, ok := stageRulesFor(tt.requestType)[*stage]
			if !ok {
				t.Fatalf("default stage %q is not registered", *stage)
			}
			if !stageMatchesStatus(rule, tt.status) {
				t.Fatalf("default stage %q does not match status %q", *stage, tt.status)
			}
		})
	}
}
