...
func (c *Client) GetPaymentSession(ctx context.Context, sessionID string) (*PaymentSession, error) {
	...
	if strings.TrimSpace(session.ID) == "" ||
		strings.TrimSpace(session.OperatorContextID) == "" ||
		strings.TrimSpace(session.ClientID) == "" ||
		strings.TrimSpace(session.StoreID) == "" ||
		strings.TrimSpace(session.PaymentMethod) == "" ||
		strings.TrimSpace(session.Status) == "" ||
		strings.TrimSpace(session.ProviderReference) == "" ||
		session.AmountMinorUnits <= 0 ||
		strings.TrimSpace(session.Currency) == "" ||
		strings.TrimSpace(session.FinancialPurpose) == "" ||
