...
func (r *OperationRegistry) registerDefaults() {
	...
	r.register(FinanceOperation{ID: "finance.payout_destinations.verify", Type: OperationTypeWrite, RequiredPermission: "administration.approve", Timeout: 15 * time.Second, Idempotent: true})
	r.register(FinanceOperation{ID: "finance.payout_destinations.deactivate", Type: OperationTypeWrite, RequiredPermission: "administration.approve", Timeout: 15 * time.Second, Idempotent: true})
	...
	r.register(FinanceOperation{ID: "finance.cod.read", Type: OperationTypeRead, RequiredPermission: "finance.read", Timeout: 10 * time.Second, Idempotent: true})
	r.register(FinanceOperation{ID: "finance.cod.collect", Type: OperationTypeWrite, RequiredPermission: "finance.manage", Timeout: 15 * time.Second, Idempotent: true})
	r.register(FinanceOperation{ID: "finance.cod.remit", Type: OperationTypeWrite, RequiredPermission: "finance.manage", Timeout: 15 * time.Second, Idempotent: true})
}
