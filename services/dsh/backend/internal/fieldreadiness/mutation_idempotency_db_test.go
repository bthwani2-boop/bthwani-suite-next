...
func TestGovernedEscalationCannotMutateCompletedVisit(t *testing.T) {
	...
	if _, err := CreateGovernedEscalationIdempotent(ctx, db, actor, input, mutation); !errors.Is(err, ErrVisitAlreadyComplete) {
...
func TestRevokedActorCannotExecuteStaleMutations(t *testing.T) {
	...
	_, err := CreateGovernedVisitIdempotent(ctx, db, actor, input, mutation)
