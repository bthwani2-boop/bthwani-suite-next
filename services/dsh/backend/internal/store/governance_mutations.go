	...
	return runMutation(ctx, db, wf, actor, storeID, "field.verification.submit", key, correlationID, input, input.Notes,
		func(tx *sql.Tx, current DshStoreRow) error {
			...
			if fieldAgentID != actor.ID {
				subject, ok := TrustedSubjectFromContext(ctx)
				if !ok {
					return ErrStoreAccessDenied
				}
				permission, ok := matchingObjectPermission(
					subject.Permissions,
					storeID,
					current.PartnerID,
					subject.OperatorContextID,
					"field.visit.cross-actor",
				)
				if !ok || !permissionAllowsCrossActor(permission, "field.visit.cross-actor") {
					return ErrStoreAccessDenied
				}
			}
...
func buildFieldVerificationSnapshots(ctx context.Context, tx *sql.Tx, visitID string) (string, string, string, error) {
	...
	locationSnapshot := map[string]any{"check": nil}
...
func runMutation(
	...
	allowed, err := ActorCanAccessStore(ctx, db, wf, actor, storeID, operation)
	if err != nil {
