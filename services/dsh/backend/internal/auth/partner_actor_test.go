...
func TestProvisionPartnerActorUsesTrustedDSHBoundary(t *testing.T) {
	...
	view, err := client.ProvisionPartnerActor(opctx.WithOperatorContext(context.Background(), "operator-main"), PartnerActorProvisionInput{
...
func TestIssuePartnerActivationForwardsIdempotencyAndCorrelation(t *testing.T) {
	...
	result, err := client.IssuePartnerActivation(
		opctx.WithOperatorContext(context.Background(), "operator-main"),
		"partner-actor-1",
...
func TestSetPartnerStoreAccessUsesTrustedPutBoundary(t *testing.T) {
	...
	client := NewClientWithInternalAccess(server.URL, "dsh-secret", "operator-main")
	err := client.SetPartnerStoreAccess(opctx.WithOperatorContext(context.Background(), "operator-main"), "partner-actor-1", PartnerStoreAccessInput{
...
func TestSetPartnerStoreAccessAllowsBundlelessRevocation(t *testing.T) {
	...
	client := NewClientWithInternalAccess(server.URL, "dsh-secret", "operator-main")
	if err := client.SetPartnerStoreAccess(opctx.WithOperatorContext(context.Background(), "operator-main"), "partner-actor-1", PartnerStoreAccessInput{StoreID: "store-1"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
