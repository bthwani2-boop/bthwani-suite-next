...
func claim(ctx context.Context, db *sql.DB, id string) (*Saga, error) {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE dsh_checkout_payment_sagas SET state='dispatched',attempt_count=attempt_count+1,lease_token=$2::uuid,lease_expires_at=$3,updated_at=NOW() WHERE id=$1::uuid AND state IN ('ready','retry_scheduled','remote_outcome_unknown')`, id, token, lease); err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
		return nil, err
...
func matches(in Input, session *wlt.PaymentSessionDetail) bool {
	return session != nil && session.ID != "" && session.ActorID == in.ClientID && session.StoreID == in.StoreID && session.PaymentMethod == in.PaymentMethod && session.AmountMinorUnits == in.AmountMinorUnits && session.Currency == in.Currency && session.Status != "failed" && session.Status != "expired"
}
...
func Dispatch(ctx context.Context, db *sql.DB, client *wlt.Client, id string) (*Saga, error) {
	...
	if session == nil {
		session, _ = client.GetPaymentSessionByCheckoutIntent(wltCtx, in.CheckoutIntentID)
	}
	if !matches(in, session) {
		...
		if callErr != nil {
			if wlt.IsPaymentSessionOutcomeUnknown(callErr) {
				_, _ = db.ExecContext(ctx, `UPDATE dsh_checkout_payment_sagas SET state='remote_outcome_unknown',last_error=$2,readback_attempt_count=readback_attempt_count+1,updated_at=NOW() WHERE id=$1::uuid`, id, callErr.Error())
			}
