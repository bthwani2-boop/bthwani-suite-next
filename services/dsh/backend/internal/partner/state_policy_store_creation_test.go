...
func actionSet(actions []string) map[string]bool {
	...
	for _, action := range actions { result[action] = true }
	return result
}
...
func TestStoreCreationAuthorityIsSurfaceAndStateBound(t *testing.T) {
	...
	if !field["update_first_store"] { t.Fatal("field onboarding must retain first-store update authority") }
	if field["create_store"] { t.Fatal("field surface must not gain generic branch creation authority") }

	partner := actionSet(AllowedActionsForSurface(StatusPartnerActive, "app-partner"))
	if partner["create_store"] { t.Fatal("app-partner must manage authorized stores, not create ownership") }

	operatorDraft := actionSet(AllowedActionsForSurface(StatusDraft, "control-panel"))
	if !operatorDraft["create_store"] { t.Fatal("control-panel must be able to create an unpublished branch for an eligible partner") }
