...
func CreateOrderTruth(db *sql.DB, input CreateOrderTruthInput) (*OrderTruth, bool, error) {
	...
	if err == nil {
		if existingFingerprint != fingerprint { return nil, false, ErrIdempotencyConflict }
		if existingOrderID.Valid && existingOrderID.String != "" {
			...
			if commitErr := tx.Commit(); commitErr != nil { return nil, false, commitErr }
	...
	if errors.Is(err, sql.ErrNoRows) { return nil, false, fmt.Errorf("%w: checkout intent is inaccessible", ErrConflict) }
	if err != nil { return nil, false, err }
	eligible := checkoutState == "payment_confirmed" || (checkoutState == "payment_pending" && paymentMethod == "cod")
	if !eligible { return nil, false, fmt.Errorf("%w: checkout intent is not eligible for order creation", ErrConflict) }

	rows, err := tx.Query(`
		SELECT product_id, product_name, unit_price, quantity
		FROM dsh_cart_items WHERE cart_id=$1::uuid ORDER BY created_at, id`, cartID)
	if err != nil { return nil, false, err }
	type sourceItem struct { productID, productName string; unitPrice float64; quantity int }
	items := make([]sourceItem, 0)
	for rows.Next() {
		var item sourceItem
		if scanErr := rows.Scan(&item.productID, &item.productName, &item.unitPrice, &item.quantity); scanErr != nil { rows.Close(); return nil, false, scanErr }
		if item.quantity <= 0 || item.unitPrice < 0 { rows.Close(); return nil, false, fmt.Errorf("%w: invalid cart item snapshot", ErrInvalid) }
		items = append(items, item)
	}
	if err = rows.Err(); err != nil { rows.Close(); return nil, false, err }
	rows.Close()
	if len(items) == 0 { return nil, false, fmt.Errorf("%w: checkout cart has no items", ErrInvalid) }
	...
	if err != nil {
		var replayID string
		replayErr := tx.QueryRow(`SELECT id::text FROM dsh_orders WHERE tenant_id=$1 AND checkout_intent_id=$2::uuid`, input.TenantID, input.CheckoutIntentID).Scan(&replayID)
		if replayErr == nil {
			truth, readErr := getOrderTruthTx(tx, replayID, input.TenantID, "client")
			if readErr != nil { return nil, false, readErr }
			if _, bindErr := tx.Exec(`UPDATE dsh_order_create_idempotency SET order_id=$1::uuid, completed_at=NOW() WHERE tenant_id=$2 AND client_id=$3 AND idempotency_key=$4`, replayID, input.TenantID, input.ClientID, input.IdempotencyKey); bindErr != nil { return nil, false, bindErr }
			if commitErr := tx.Commit(); commitErr != nil { return nil, false, commitErr }
			return truth, true, nil
		}
		return nil, false, err
	...
	for _, item := range items {
		snapshot, marshalErr := json.Marshal(map[string]any{"productId": item.productID, "productName": item.productName, "quantity": item.quantity, "unitPrice": item.unitPrice})
		if marshalErr != nil { return nil, false, marshalErr }
	...
	result, err := tx.Exec(`UPDATE dsh_checkout_intents SET state='confirmed', version=version+1, updated_at=NOW() WHERE id=$1::uuid AND tenant_id=$2 AND client_id=$3 AND state IN ('payment_pending','payment_confirmed')`, input.CheckoutIntentID, input.TenantID, input.ClientID)
	if err != nil { return nil, false, err }
	if affected, _ := result.RowsAffected(); affected != 1 { return nil, false, fmt.Errorf("%w: checkout changed concurrently", ErrConflict) }
	if _, err = tx.Exec(`UPDATE dsh_carts SET state='checked_out', version=version+1, updated_at=NOW() WHERE id=$1::uuid AND client_id=$2 AND state='active'`, cartID, input.ClientID); err != nil { return nil, false, err }
	if _, err = tx.Exec(`UPDATE dsh_order_create_idempotency SET order_id=$1::uuid, completed_at=NOW() WHERE tenant_id=$2 AND client_id=$3 AND idempotency_key=$4`, orderID, input.TenantID, input.ClientID, input.IdempotencyKey); err != nil { return nil, false, err }
...
func getOrderTruthTx(tx *sql.Tx, orderID, tenantID, viewerRole string) (*OrderTruth, error) {
	...
	if paymentUpdated.Valid { truth.PaymentProjectionUpdatedAt = &paymentUpdated.Time }
	truth.CurrentOwner = currentOwner(truth.Status)
	truth.AllowedActions = AllowedActions(truth.Status, viewerRole)

	itemRows, err := tx.Query(`SELECT id::text, product_id, product_name, quantity, unit_price, line_total_minor_units, item_snapshot FROM dsh_order_items WHERE order_id=$1::uuid ORDER BY created_at,id`, orderID)
	if err != nil { return nil, err }
	for itemRows.Next() {
		var item OrderTruthItem
		var snapshot []byte
		if err = itemRows.Scan(&item.ID, &item.ProductID, &item.ProductName, &item.Quantity, &item.UnitPrice, &item.LineTotalMinorUnits, &snapshot); err != nil { itemRows.Close(); return nil, err }
		item.Snapshot = json.RawMessage(snapshot)
		truth.Items = append(truth.Items, item)
	}
	if err = itemRows.Err(); err != nil { itemRows.Close(); return nil, err }
	itemRows.Close()

	eventRows, err := tx.Query(`SELECT id::text, event_type, actor_role, from_status, to_status, correlation_id, causation_id, order_version, metadata, created_at FROM dsh_order_status_events WHERE tenant_id=$1 AND order_id=$2::uuid ORDER BY created_at,id`, tenantID, orderID)
	if err != nil { return nil, err }
	for eventRows.Next() {
		var event OrderTruthEvent
		var metadata []byte
		if err = eventRows.Scan(&event.ID, &event.Type, &event.ActorRole, &event.FromStatus, &event.ToStatus, &event.CorrelationID, &event.CausationID, &event.OrderVersion, &metadata, &event.CreatedAt); err != nil { eventRows.Close(); return nil, err }
		event.Metadata = json.RawMessage(metadata)
		truth.StatusTimeline = append(truth.StatusTimeline, event)
	}
	if err = eventRows.Err(); err != nil { eventRows.Close(); return nil, err }
	eventRows.Close()
	return &truth, nil
}
...
func currentOwner(status OrderStatus) string {
	...
	case StatusDelivered, StatusReturnedStore, StatusCancelled:
		return "terminal"
	default:
		if strings.HasPrefix(string(status), "cancelled_") || strings.HasPrefix(string(status), "failed_") { return "terminal" }
		return "operations"
	}
}
...
func AllowedActions(status OrderStatus, viewerRole string) []string {
	...
	switch viewerRole {
	case "client":
		if status == StatusPending { actions = append(actions, "cancel_if_policy_allows") }
		if status == StatusDriverAssigned || status == StatusArrivedStore || status == StatusPickedUp || status == StatusArrivedCustomer { actions = append(actions, "track") }
		if status == StatusDelivered { actions = append(actions, "rate") }
	case "partner":
		switch status {
		case StatusPending: actions = append(actions, "accept", "reject")
		case StatusStoreAccepted: actions = append(actions, "start_preparing")
		case StatusPreparing: actions = append(actions, "revise_estimate", "mark_ready")
		case StatusReadyForPickup: actions = append(actions, "confirm_handoff")
		}
	case "operator":
		actions = append(actions, "view_audit")
		if status == StatusPending || status == StatusStoreAccepted || status == StatusPreparing { actions = append(actions, "cancel_if_policy_allows") }
	}
	return actions
}
