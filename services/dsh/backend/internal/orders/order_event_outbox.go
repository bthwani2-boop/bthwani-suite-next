...
// ownership. Processing rows older than five minutes are reclaimed after a
// crashed worker instead of remaining permanently stuck.
...
func MarkOrderEventPublished(db *sql.DB, outboxID, operatorContextID string) error {
	...
	result, err := db.Exec(`
		...
		WHERE id=$1::uuid AND tenant_id=$2 AND status='processing'`, outboxID, operatorContextID)
	if err != nil {
		return err
	}
	...
	result, err := db.Exec(`
		...
		SET status=CASE WHEN attempt_count >= 12 THEN 'dead_letter' ELSE 'retry' END,
		    next_attempt_at=CASE
		      WHEN attempt_count >= 12 THEN next_attempt_at
...
func GetOrderOutboxRecord(db *sql.DB, operatorContextID, eventID string) (*OrderOutboxRecord, error) {
	...
	err := db.QueryRow(`
		...
		FROM dsh_order_event_outbox WHERE tenant_id=$1 AND event_id=$2::uuid`, operatorContextID, eventID,
	).Scan(
		&record.ID,
		&record.OperatorContextID,
		&record.OrderID,
