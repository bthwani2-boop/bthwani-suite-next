...
func scanCancellation(row *sql.Row) (*Cancellation, error) {
...
func GetCancellation(db *sql.DB, orderID string) (*Cancellation, error) {
	return scanCancellation(db.QueryRow(cancellationSelect+`
		WHERE c.order_id=$1::uuid`, orderID))
}
...
func GetCancellationForContext(db *sql.DB, operatorContextID, orderID string) (*Cancellation, error) {
	...
	return scanCancellation(db.QueryRow(cancellationSelect+`
		...
		WHERE c.order_id=$1::uuid AND (c.operator_context_id=$2 OR ord.operator_context_id=$2)`, orderID, operatorContextID))
}
