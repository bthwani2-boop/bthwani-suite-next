...
func UpsertSlaRule(
	...
	if input.ZoneID == "" ||
		input.Category == "" ||
		len(input.Category) > 120 ||
		input.MaxPrepMins < 1 ||
		input.MaxPrepMins > 1440 ||
		input.MaxDeliveryMins < 1 ||
		input.MaxDeliveryMins > 1440 ||
		input.ExpectedVersion < 0 ||
		!validMutation(mutation) {
		return SlaRule{}, ErrInvalid
	}

	operation := "upsert-sla:" + input.ZoneID + ":" + input.Category
	return withIdempotency(
		ctx,
		db,
		mutation,
		operation,
		input,
		func(tx *sql.Tx) (SlaRule, error) {
			var before SlaRule
			err := tx.QueryRowContext(ctx, `
				SELECT id, zone_id, category, max_prep_mins,
				       max_delivery_mins, version, updated_by, updated_at
				FROM dsh_platform_sla_rules
				WHERE zone_id = $1 AND category = $2
				FOR UPDATE`, input.ZoneID, input.Category).Scan(
				&before.ID,
				&before.ZoneID,
				&before.Category,
				&before.MaxPrepMins,
			...
			if errors.Is(err, sql.ErrNoRows) {
				if input.ExpectedVersion != 0 {
					return SlaRule{}, ErrVersionConflict
				}
				err = tx.QueryRowContext(ctx, `
					INSERT INTO dsh_platform_sla_rules
						(zone_id, category, max_prep_mins,
						 max_delivery_mins, updated_by)
					VALUES ($1, $2, $3, $4, $5)
					RETURNING id, zone_id, category, max_prep_mins,
					          max_delivery_mins, version, updated_by, updated_at`,
					input.ZoneID,
					input.Category,
					input.MaxPrepMins,
...
func UpsertCapacity(
	...
	input.ZoneID = strings.TrimSpace(input.ZoneID)
	if input.ZoneID == "" ||
		input.MaxConcurrentOrders < 1 ||
		input.MaxCaptainsOnline < 0 ||
		input.ThrottleThreshold < 0 ||
		input.ThrottleThreshold > 1 ||
		input.ExpectedVersion < 0 ||
		!validMutation(mutation) {
	...
	return withIdempotency(
		...
		db,
		mutation,
		"upsert-capacity:"+input.ZoneID,
		input,
		func(tx *sql.Tx) (CapacityConfig, error) {
			var before CapacityConfig
			err := tx.QueryRowContext(ctx, `
				SELECT id, zone_id, max_concurrent_orders, max_captains_online,
				       throttle_threshold, version, updated_by, updated_at
				FROM dsh_platform_capacity_configs
			...
			if errors.Is(err, sql.ErrNoRows) {
				if input.ExpectedVersion != 0 {
					return CapacityConfig{}, ErrVersionConflict
				}
				err = tx.QueryRowContext(ctx, `
					INSERT INTO dsh_platform_capacity_configs
						(zone_id, max_concurrent_orders, max_captains_online,
						 throttle_threshold, updated_by)
					VALUES ($1, $2, $3, $4, $5)
					RETURNING id, zone_id, max_concurrent_orders,
					          max_captains_online, throttle_threshold,
					          version, updated_by, updated_at`,
					input.ZoneID,
				...
				err = tx.QueryRowContext(ctx, `
					...
					SET max_concurrent_orders = $2,
					    max_captains_online = $3,
