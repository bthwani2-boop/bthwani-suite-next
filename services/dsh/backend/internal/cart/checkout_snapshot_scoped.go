...
func ComputeCheckoutSnapshotTx(
	...
	hasher := sha256.New()
	_, _ = fmt.Fprintf(hasher, "%s|%s|%s|v%d", cartID, clientID, lockedStoreID, currentVersion)
	for rows.Next() {
		...
		subtotal += lineTotal
		_, _ = fmt.Fprintf(hasher, "|%s:%d:%d:%s", productID, quantity, unitMinorUnits, itemCurrency)
