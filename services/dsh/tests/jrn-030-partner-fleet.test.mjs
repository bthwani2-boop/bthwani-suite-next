...
test("JRN-030 enforces active-store and courier eligibility before issue and redeem", () => {
  ...
  assert.match(courierCodes, /status == "blocked"/);
  assert.match(partnerRoutes, /STORE_INELIGIBLE/);
});

test("JRN-030 expires codes durably and prevents stale transitions", () => {
  assert.match(listModel, /SET status = 'expired', version = version \+ 1/);
  assert.match(courierCodes, /SET status='expired',version=version\+1/);
  assert.match(courierCodes, /WHERE id=\$2 AND store_id=\$3 AND version=\$4/);
...
test("JRN-030 shared brain and all three required surfaces bind to DSH", () => {
  ...
  assert.match(partnerSurface, /إصدار رمز ربط/);
