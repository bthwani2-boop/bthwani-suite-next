--  database security and isolation invariants.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE tgname='trg_dsh_protect_order_snapshot' AND NOT tgisinternal
  ) THEN missing := array_append(missing, 'immutable order snapshot trigger'); END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname='trg_dsh_order_event_outbox' AND NOT tgisinternal
