...
BEGIN
  ...
  IF constraint_count <> 2 THEN
    RAISE EXCEPTION ' expected two governance constraints, found %', constraint_count;
  END IF;
  ...
  IF index_count <> 4 THEN
    RAISE EXCEPTION ' expected four governance indexes, found %', index_count;
  END IF;
END $$;
...
INSERT INTO dsh_store_action_audit (
  ...
  'audit-db-proof', 'operator-', 'operator',
  'store-db-proof', 'operator.visibility.update', '{}'::jsonb,
  '{"isVisible":true}'::jsonb, 'database proof', 'corr-db-proof'
);

INSERT INTO dsh_store_idempotency (
  actor_id, operation, idempotency_key, request_hash, response_body
) VALUES (
  'operator-', 'operator.visibility.update', 'idem-db-proof',
  'hash-db-proof', '{"replayed":false}'::jsonb
);
...
BEGIN
  ...
  WHERE actor_id = 'operator-'
    AND operation = 'operator.visibility.update'
    AND idempotency_key = 'idem-db-proof';

  IF expires_value IS NULL OR expires_value <= created_value THEN
    RAISE EXCEPTION ' idempotency expiry was not populated';
  END IF;
END $$;
...
BEGIN
  BEGIN
    UPDATE dsh_stores SET version = 0 WHERE id = 'store-db-proof';
    RAISE EXCEPTION ' version invariant accepted zero';
  EXCEPTION
  ...
  BEGIN
    INSERT INTO dsh_store_action_audit (
      ...
      'audit-invalid-role', 'unknown-', 'client',
      'store-db-proof', 'invalid', '{}'::jsonb, '{}'::jsonb,
      'must fail', 'corr-invalid-role'
    );
    RAISE EXCEPTION ' audit actor-role invariant accepted client';
  EXCEPTION
    WHEN check_violation THEN NULL;
  END;
END $$;
