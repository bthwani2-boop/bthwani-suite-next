--  payment projection reconciliation invariants.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE tgname='trg_dsh_schedule_payment_projection'
  ...
  INTO function_sql
  FROM pg_proc p
  WHERE p.proname='dsh_apply_order_truth'
  ORDER BY p.oid DESC
  LIMIT 1;
  ...
  IF cardinality(missing) > 0 THEN
    RAISE EXCEPTION ' dsh-905 invariant failure: %', array_to_string(missing, ', ');
  END IF;
END $$;
