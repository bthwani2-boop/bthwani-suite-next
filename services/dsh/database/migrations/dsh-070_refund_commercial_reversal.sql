-- DSH-070: commercial-effect reversal after WLT confirms a refund.
-- Coupon state, loyalty reversal and promotion-funding reversal are recorded in
-- one database transaction. WLT remains the owner of the financial transition.
...
CREATE TABLE IF NOT EXISTS dsh_order_refund_effects (
    ...
    funding_reversal_queued BOOLEAN NOT NULL DEFAULT FALSE,
    processed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

ALTER TABLE dsh_order_refund_effects
    ADD COLUMN IF NOT EXISTS funding_reversal_queued BOOLEAN NOT NULL DEFAULT FALSE;
...
BEGIN
    ...
    VALUES (p_order_id,p_refund_reference,COALESCE(p_reason,''),coupon_changed,loyalty_queued,funding_queued)
...
COMMENT ON FUNCTION dsh_apply_confirmed_refund_effects(UUID,TEXT,TEXT) IS
    'Applies coupon state and atomically queues loyalty and WLT promotion-funding reversal exactly once after WLT confirms refund completion.';
