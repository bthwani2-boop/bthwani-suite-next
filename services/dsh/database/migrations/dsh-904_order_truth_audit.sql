-- order-truth / FS-15: redacted operational audit used by diagnostics and alerts.
BEGIN;
...
CREATE OR REPLACE FUNCTION dsh_validate_audit_metadata()
...
BEGIN
  ...
  IF serialized ~ '(authorization|bearer|idempotency.?key|delivery.?address|wallet.?balance|provider.?payload)' THEN
    RAISE EXCEPTION 'order-truth audit metadata contains a forbidden sensitive key';
  END IF;
  RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS trg_dsh_validate_audit_metadata ON dsh_order_truth_audit;
CREATE TRIGGER trg_dsh_validate_audit_metadata
BEFORE INSERT OR UPDATE ON dsh_order_truth_audit
FOR EACH ROW EXECUTE FUNCTION dsh_validate_audit_metadata();
