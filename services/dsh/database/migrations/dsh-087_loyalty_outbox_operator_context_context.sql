-- DSH-087: propagate OperatorContext context through loyalty earn and reversal events.
--
-- dsh-085 made operator_context_id mandatory for every durable DSH -> WLT event. The
...
COMMENT ON FUNCTION dsh_enqueue_loyalty_earned_on_delivery() IS
    'Atomically enqueues OperatorContext-scoped loyalty earn truth when an order is delivered.';
COMMENT ON FUNCTION dsh_enqueue_loyalty_reversal(UUID,TEXT) IS
    'Creates an idempotent OperatorContext-scoped WLT loyalty reversal after governed refund confirmation.';
