-- DSH-910 /  FS-05, FS-08, FS-13, FS-15
-- Durable, OperatorContext-scoped receipt ledger for WLT payment-session events.
...
CREATE TABLE IF NOT EXISTS dsh_checkout_wlt_event_receipts (
    ...
    CONSTRAINT dsh_checkout_wlt_event_OperatorContext_intent_unique
        UNIQUE (operator_context_id, checkout_intent_id, event_key)
);

CREATE INDEX IF NOT EXISTS idx_dsh_checkout_wlt_event_receipts_intent
    ON dsh_checkout_wlt_event_receipts(operator_context_id, checkout_intent_id, received_at DESC);
CREATE INDEX IF NOT EXISTS idx_dsh_checkout_wlt_event_receipts_session
    ON dsh_checkout_wlt_event_receipts(operator_context_id, payment_session_id, received_at DESC);
...
BEFORE INSERT OR UPDATE OF operator_context_id, checkout_intent_id, payment_session_id
ON dsh_checkout_wlt_event_receipts
