-- order-truth / FS-05: immutable order truth, creation idempotency and event outbox.
BEGIN;
...
DROP TRIGGER IF EXISTS trg_dsh_apply_order_truth ON dsh_orders;
CREATE TRIGGER trg_dsh_apply_order_truth
BEFORE INSERT ON dsh_orders
FOR EACH ROW EXECUTE FUNCTION dsh_apply_order_truth();

CREATE OR REPLACE FUNCTION dsh_protect_order_snapshot()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  IF ROW(NEW.checkout_intent_id, NEW.tenant_id, NEW.store_id, NEW.client_id,
    ...
    RAISE EXCEPTION 'order-truth order truth snapshot is immutable';
  END IF;
  RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS trg_dsh_protect_order_snapshot ON dsh_orders;
CREATE TRIGGER trg_dsh_protect_order_snapshot
BEFORE UPDATE ON dsh_orders
FOR EACH ROW EXECUTE FUNCTION dsh_protect_order_snapshot();
