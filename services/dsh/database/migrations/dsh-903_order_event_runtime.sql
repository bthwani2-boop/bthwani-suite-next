-- order-truth / FS-08: enrich every legacy and governed transition in-database.
BEGIN;

CREATE OR REPLACE FUNCTION dsh_increment_order_version()
...
DROP TRIGGER IF EXISTS trg_dsh_increment_order_version ON dsh_orders;
CREATE TRIGGER trg_dsh_increment_order_version
BEFORE UPDATE OF status ON dsh_orders
FOR EACH ROW EXECUTE FUNCTION dsh_increment_order_version();

CREATE OR REPLACE FUNCTION dsh_enrich_order_event()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  order_row RECORD;
BEGIN
...
DROP TRIGGER IF EXISTS trg_dsh_enrich_order_event ON dsh_order_status_events;
CREATE TRIGGER trg_dsh_enrich_order_event
BEFORE INSERT ON dsh_order_status_events
FOR EACH ROW EXECUTE FUNCTION dsh_enrich_order_event();
...
DROP TRIGGER IF EXISTS trg_dsh_order_event_outbox ON dsh_order_status_events;
CREATE TRIGGER trg_dsh_order_event_outbox
AFTER INSERT ON dsh_order_status_events
FOR EACH ROW EXECUTE FUNCTION dsh_publish_order_event_to_outbox();
