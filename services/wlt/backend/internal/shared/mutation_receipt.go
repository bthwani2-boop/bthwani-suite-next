...
func StoreJrn036MutationReceiptTx(
	...
	if idempotencyKey == "" || requestHash == "" || mutationType == "" || aggregateID == "" {
		return fmt.Errorf("complete settlements-commissions mutation receipt identity is required")
	}
	encoded, err := json.Marshal(response)
	if err != nil {
		return fmt.Errorf("encode settlements-commissions mutation receipt: %w", err)
	}
