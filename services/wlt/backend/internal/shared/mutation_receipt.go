package shared

import (
	"context"
	"database/sql"
	"encoding/json"
	"errors"
	"fmt"
	"strings"
)

var ErrMutationIdempotencyConflict = errors.New("idempotency key was already used with different mutation inputs")

// LoadMutationReceiptTx serializes callers sharing a platform-context-local
// idempotency key, then returns the original canonical response when the exact
// mutation was already committed.
func LoadMutationReceiptTx(
	ctx context.Context,
	tx *sql.Tx,
	idempotencyKey string,
	requestHash string,
) (json.RawMessage, bool, error) {
	operatorContextID, err := RequireOperatorContext(ctx)
	if err != nil {
		return nil, false, err
	}
	idempotencyKey = strings.TrimSpace(idempotencyKey)
	requestHash = strings.TrimSpace(requestHash)
	if idempotencyKey == "" || requestHash == "" {
		return nil, false, fmt.Errorf("idempotencyKey and requestHash are required")
	}
	if len(idempotencyKey) > 200 {
		return nil, false, fmt.Errorf("idempotencyKey must not exceed 200 characters")
	}

	if _, err := tx.ExecContext(
		ctx,
		`SELECT pg_advisory_xact_lock(hashtextextended($1, 0))`,
		operatorContextID+":"+idempotencyKey,
	); err != nil {
		return nil, false, err
	}

	var storedHash string
	var responseText string
	err = tx.QueryRowContext(ctx, `
		SELECT request_hash, response_json::text
		FROM wlt_mutation_receipts
		WHERE operator_context_id = $1 AND idempotency_key = $2`, operatorContextID, idempotencyKey).Scan(&storedHash, &responseText)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, false, nil
	}
	if err != nil {
		return nil, false, err
	}
	if storedHash != requestHash {
		return nil, false, ErrMutationIdempotencyConflict
	}
	return json.RawMessage(responseText), true, nil
}

func StoreMutationReceiptTx(
	ctx context.Context,
	tx *sql.Tx,
	idempotencyKey string,
	requestHash string,
	mutationType string,
	aggregateID string,
	response any,
) error {
	operatorContextID, err := RequireOperatorContext(ctx)
	if err != nil {
		return err
	}
	idempotencyKey = strings.TrimSpace(idempotencyKey)
	requestHash = strings.TrimSpace(requestHash)
	mutationType = strings.TrimSpace(mutationType)
	aggregateID = strings.TrimSpace(aggregateID)
	if idempotencyKey == "" || requestHash == "" || mutationType == "" || aggregateID == "" {
		return fmt.Errorf("complete mutation receipt identity is required")
	}
	encoded, err := json.Marshal(response)
	if err != nil {
		return fmt.Errorf("encode mutation receipt: %w", err)
	}
	_, err = tx.ExecContext(ctx, `
		INSERT INTO wlt_mutation_receipts
		(operator_context_id, idempotency_key, request_hash, mutation_type, aggregate_id, response_json)
		VALUES ($1, $2, $3, $4, $5, $6::jsonb)`,
		operatorContextID,
		idempotencyKey,
		requestHash,
		mutationType,
		aggregateID,
		string(encoded),
	)
	return err
}
