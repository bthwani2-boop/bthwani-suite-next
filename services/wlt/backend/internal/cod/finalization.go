...
func FinalizeCodReservation(ctx context.Context, db *sql.DB, orderID, checkoutIntentID string) (*CodReservation, bool, error) {
	...
	var paymentStatus, paymentMethod, paymentCurrency string
	var cashOnDeliveryAmount int64
	if err := tx.QueryRowContext(ctx, `
		SELECT status, payment_method, currency, COALESCE(cash_on_delivery_amount_minor_units, 0)
		...
		Scan(&paymentStatus, &paymentMethod, &paymentCurrency, &cashOnDeliveryAmount); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
