...
const codReservationCols = `id, operator_context_id, order_id, COALESCE(checkout_intent_id,''), captain_id, amount_minor_units, currency,
 status, idempotency_key, COALESCE(release_reason,''), COALESCE(finalization_ledger_transaction_id,''), created_at, updated_at, resolved_at`
...
func scanCodReservation(row rowScanner) (*CodReservation, error) {
	...
	if err := row.Scan(
		&out.ID, &out.OperatorContextID, &out.OrderID, &out.CheckoutIntentID, &out.CaptainID, &out.AmountMinorUnits, &out.Currency,
		&out.Status, &out.IdempotencyKey, &out.ReleaseReason, &out.FinalizationLedgerTransactionID, &out.CreatedAt, &out.UpdatedAt, &resolvedAt,
	); err != nil {
		return nil, err
	}
	if resolvedAt.Valid {
...
func ReserveCodCapacity(ctx context.Context, db *sql.DB, orderID, checkoutIntentID, captainID string, amountMinorUnits int64, currency, idempotencyKey string) (*CodReservation, bool, error) {
	...
	if orderID == "" || checkoutIntentID == "" || captainID == "" {
		return nil, false, fmt.Errorf("orderId, checkoutIntentId, and captainId are required")
	}
	...
	created, err := scanCodReservation(tx.QueryRowContext(ctx, `
		INSERT INTO wlt_cod_reservations
			(operator_context_id, order_id, checkout_intent_id, captain_id, amount_minor_units, currency, idempotency_key)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		ON CONFLICT (operator_context_id, order_id) DO NOTHING
		RETURNING `+codReservationCols,
		operatorContextID, orderID, checkoutIntentID, captainID, amountMinorUnits, currency, idempotencyKey,
	))
	...
	if existing.CheckoutIntentID != checkoutIntentID || existing.CaptainID != captainID || existing.AmountMinorUnits != amountMinorUnits || existing.Currency != currency {
...
// completed) is rejected: the captain-funded COD exposure has already been
// retired through WLT's finalization ledger transaction, not returned to
// available capacity.
...
func HandleReserveCodCapacity(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		reservation, replayed, err := ReserveCodCapacity(r.Context(), db, input.OrderID, input.CheckoutIntentID, input.CaptainID, input.AmountMinorUnits, input.Currency, idempotencyKey)
		switch {
