...
// CreateCodRecordForTenant creates the WLT-owned COD custody aggregate from a
// tenant-scoped WLT payment session. The caller never supplies amount,
// currency, or tenant ownership.
func CreateCodRecordForTenant(
	ctx context.Context,
	db *sql.DB,
...
func HandleCreateCodRecordTenant(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		record, err := CreateCodRecordForTenant(r.Context(), db, input)
		switch {
		case errors.Is(err, reference.ErrTenantMismatch):
			shared.SendError(w, http.StatusForbidden, "TENANT_MISMATCH", err.Error())
			return
