...
func scanGovernedRefund(row scanner) (*GovernedRefund, error) {
        ...
        if err := row.Scan(
                &out.ID, &out.OperatorContextID, &out.PaymentSessionID, &out.OrderID, &out.ClientID,
...
func ListGovernedRefunds(db *sql.DB, orderID, clientID, operatorContextID string) ([]*GovernedRefund, error) {
...
func CreateGovernedRefund(ctx context.Context, db *sql.DB, input GovernedCreateRefundInput) (*GovernedRefund, bool, error) {
        ...
        created, err := scanGovernedRefund(tx.QueryRowContext(ctx, `
                ...
                input.OperatorContextID, input.PaymentSessionID, input.OrderID, input.ClientID, requestedAmount,
...
func finalizeGovernedRefundSuccess(ctx context.Context, db *sql.DB, refundID, actorID, actorType, providerReference, evidenceReason, correlationID string, allowedFrom []string) (*GovernedRefund, error) {
        ...
        // The refund journal must undo what the capture actually did, so the
        // reversal is derived from the source session's immutable
        // FinancialPurpose (the same fact captureEconomicEffect interpreted)
        // instead of a fixed pair of accounts. Without this, a wallet top-up
        // refund would credit provider_clearing and debit platform_payable while
        // leaving the wallet credited — creating spendable balance out of a
        // refunded card charge.
        var sourcePurpose, sourceClientID string
        if err := tx.QueryRowContext(ctx, `SELECT financial_purpose, client_id FROM wlt_payment_sessions WHERE id=$1`, updated.PaymentSessionID).Scan(&sourcePurpose, &sourceClientID); err != nil {
                return nil, fmt.Errorf("load refund source session accounting facts: %w", err)
        }
        lines, err := RefundEconomicEffect(RefundSourceSession{
                FinancialPurpose: sourcePurpose,
        ...
        if err := dshoutbox.EnqueueRefund(tx, updated.ID, updated.PaymentSessionID, updated.OperatorContextID, updated.OrderID, updated.Reason, correlationID, checkoutPtr, specialPtr); err != nil {
                return nil, err
        }
...
func ListGovernedRefundAudit(db *sql.DB, refundID string) ([]RefundAuditEvent, error) {
        ...
        for rows.Next() {
                ...
                if err := rows.Scan(&item.ID, &item.RefundID, &item.OperatorContextID, &item.EventType, &item.ActorID, &item.ActorType, &item.FromStatus, &item.ToStatus, &item.Reason, &item.CorrelationID, &item.IdempotencyKey, &item.ProviderReference, &item.CreatedAt); err != nil {
                        return nil, err
                }
                out = append(out, item)
        }
...
func HandleListGovernedRefunds(db *sql.DB) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                items, err := ListGovernedRefunds(db, r.URL.Query().Get("orderId"), r.URL.Query().Get("clientId"), r.URL.Query().Get("operatorContextId"))
                if err != nil {
                        shared.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", err.Error())
                        return
                }
