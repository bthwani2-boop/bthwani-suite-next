...
// RegisterDeliveryCollectionRoutes exposes the fulfillment-neutral COD surface.
// Legacy /wlt/cod-records remains available for captain-only compatibility.
func RegisterDeliveryCollectionRoutes(mux *http.ServeMux, db *sql.DB, mutationsEnabled bool) {
	...
	mux.HandleFunc("POST /wlt/delivery-collections", gate(serviceAuth(cod.HandleCreateDeliveryCollection(db))))
