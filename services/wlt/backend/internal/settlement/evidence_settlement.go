...
func normalizeEvidenceSettlementInput(input CreateEvidenceSettlementInput) (CreateEvidenceSettlementInput, time.Time, time.Time, error) {
	...
	if err != nil {
		return input, time.Time{}, time.Time{}, fmt.Errorf("periodStart must use YYYY-MM-DD")
	}
	periodEnd, err := time.Parse("2006-01-02", input.PeriodEnd)
	if err != nil || periodEnd.Before(periodStart) {
		return input, time.Time{}, time.Time{}, fmt.Errorf("periodEnd must use YYYY-MM-DD and be on or after periodStart")
	}
	...
	for index := range input.OrderSources {
		...
		if _, ok := seen[source.OrderID]; ok {
			return input, time.Time{}, time.Time{}, fmt.Errorf("duplicate orderId %s", source.OrderID)
		}
...
func scanGovernedSettlementPolicy(row *sql.Row) (*GovernedSettlementPolicy, error) {
	...
	err := row.Scan(
		...
		&policy.Status,
		&policy.CycleDays,
		&policy.MinimumNetMinorUnits,
		&policy.ChangeReason,
		&policy.UpdatedByOperatorID,
	)
...
func getOrAdoptSettlementPolicyTx(ctx context.Context, tx *sql.Tx, partnerID string) (*GovernedSettlementPolicy, error) {
	...
	const versionQuery = `SELECT partner_id,version,fee_basis_points,currency,status,cycle_days,minimum_net_minor_units,change_reason,updated_by_operator_id
	...
	err = tx.QueryRowContext(ctx, `SELECT fee_basis_points,currency,status,updated_by_operator_id
		FROM wlt_settlement_policies WHERE operator_context_id=$1 AND partner_id=$2`, operatorContextID, partnerID).Scan(&fee, &currency, &status, &operator)
	if errors.Is(err, sql.ErrNoRows) {
	...
	row := tx.QueryRowContext(ctx, `INSERT INTO wlt_settlement_policy_versions
		(operator_context_id,partner_id,version,fee_basis_points,currency,status,cycle_days,minimum_net_minor_units,change_reason,updated_by_operator_id)
...
func UpsertGovernedSettlementPolicy(ctx context.Context, db *sql.DB, partnerID string, input UpsertGovernedSettlementPolicyInput, correlationID string) (*GovernedSettlementPolicy, error) {
	...
	if partnerID == "" || input.OperatorID == "" || input.ChangeReason == "" || correlationID == "" || input.FeeBasisPoints < 0 || input.FeeBasisPoints > 10000 || input.MinimumNetMinorUnits < 0 {
		return nil, fmt.Errorf("valid partnerId, feeBasisPoints, changeReason, operatorId, correlationId and minimum are required")
	}
	if input.Currency == "" {
		input.Currency = "YER"
	}
	...
	if err := tx.QueryRowContext(ctx, `SELECT COALESCE(MAX(version),0)+1
		FROM wlt_settlement_policy_versions WHERE operator_context_id=$1 AND partner_id=$2`, operatorContextID, partnerID).Scan(&version); err != nil {
		return nil, err
	}
	row := tx.QueryRowContext(ctx, `INSERT INTO wlt_settlement_policy_versions
		(operator_context_id,partner_id,version,fee_basis_points,currency,status,cycle_days,minimum_net_minor_units,change_reason,updated_by_operator_id)
...
func CreateEvidenceBackedSettlement(ctx context.Context, db *sql.DB, input CreateEvidenceSettlementInput, correlationID string) (*Settlement, error) {
	...
	input, periodStart, periodEnd, err = normalizeEvidenceSettlementInput(input)
	if err != nil {
		return nil, err
	}
	_ = periodStart
	...
	err = tx.QueryRowContext(ctx, `SELECT settlement_id,request_hash
		FROM wlt_settlement_requests WHERE operator_context_id=$1 AND idempotency_key=$2`, operatorContextID, input.IdempotencyKey).Scan(&existingSettlementID, &existingHash)
	if err == nil {
		if existingHash != requestHash {
			return nil, ErrSettlementIdempotencyConflict
		}
		existing, err := scanSettlement(tx.QueryRowContext(ctx, `SELECT `+settlementCols+` FROM wlt_settlements WHERE operator_context_id=$1 AND id=$2`, operatorContextID, existingSettlementID))
		if err != nil {
			return nil, err
		}
		if err := tx.Commit(); err != nil {
	...
	policy, err := getOrAdoptSettlementPolicyTx(ctx, tx, input.PartnerID)
	if err != nil {
		return nil, err
	}
	...
	type verifiedSource struct {
		source      VerifiedDeliveredOrderSource
		refund      int64
		refundCount int
		basis       int64
	}
	...
	for _, source := range input.OrderSources {
		...
		if err := tx.QueryRowContext(ctx, `SELECT COALESCE(SUM(amount_minor_units),0),COUNT(*)
			FROM wlt_refunds WHERE operator_context_id=$1 AND order_id=$2 AND status='completed'`, operatorContextID, source.OrderID).Scan(&refund, &refundCount); err != nil {
			return nil, err
		}
		if refund > source.GrossAmountMinorUnits {
			return nil, ErrSettlementRefundExceedsGross
		}
		basis := source.GrossAmountMinorUnits - refund
		if basis > 0 {
			gross, err = addPositiveMinorUnits(gross, basis)
			if err != nil {
				return nil, err
			}
		}
	...
	if gross <= 0 {
		return nil, fmt.Errorf("all delivered order value was refunded; no payable settlement basis remains")
	}
	fee, err := settlementFeeFromBasisPoints(gross, policy.FeeBasisPoints)
	if err != nil {
		return nil, err
	}
	net := gross - fee
	if net < policy.MinimumNetMinorUnits {
		return nil, ErrSettlementMinimumNotMet
	}
	positiveCount := 0
	...
	settlement, err := scanSettlement(tx.QueryRowContext(ctx, `INSERT INTO wlt_settlements
		(operator_context_id,partner_id,period_start,period_end,gross_amount,platform_fee,net_amount,currency,order_count)
	...
	for _, item := range verified {
		if item.basis > 0 {
			if _, err := tx.ExecContext(ctx, `INSERT INTO wlt_settlement_source_orders
				(operator_context_id,order_id,settlement_id,partner_id,gross_amount_minor_units,currency,delivered_at)
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO wlt_settlement_source_evidence
			(operator_context_id,order_id,settlement_id,pricing_snapshot_hash,completion_event_id,completion_evidence_hash,cancellation_status,original_gross_minor_units,completed_refund_minor_units,settlement_basis_minor_units,refund_evidence_count)
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO wlt_settlement_requests
		(operator_context_id,idempotency_key,request_hash,settlement_id,partner_id,policy_version)
...
func ListSettlementEvidence(ctx context.Context, db *sql.DB, settlementID string) ([]SettlementSourceEvidenceView, error) {
	...
	rows, err := db.QueryContext(ctx, `SELECT order_id,pricing_snapshot_hash,completion_event_id,completion_evidence_hash,cancellation_status,original_gross_minor_units,completed_refund_minor_units,settlement_basis_minor_units,refund_evidence_count,verified_at::text
		FROM wlt_settlement_source_evidence
		WHERE operator_context_id=$1 AND settlement_id=$2 ORDER BY order_id`, operatorContextID, strings.TrimSpace(settlementID))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	out := []SettlementSourceEvidenceView{}
	for rows.Next() {
		var item SettlementSourceEvidenceView
		if err := rows.Scan(&item.OrderID, &item.PricingSnapshotHash, &item.CompletionEventID, &item.CompletionEvidenceHash, &item.CancellationStatus, &item.OriginalGrossMinorUnits, &item.CompletedRefundMinorUnits, &item.SettlementBasisMinorUnits, &item.RefundEvidenceCount, &item.VerifiedAt); err != nil {
			return nil, err
		}
		out = append(out, item)
	}
...
func HandleCreateEvidenceBackedSettlement(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var input CreateEvidenceSettlementInput
		...
		settlement, err := CreateEvidenceBackedSettlement(r.Context(), db, input, r.Header.Get("X-Correlation-ID"))
		switch {
		case errors.Is(err, ErrSettlementPolicyMissing):
			shared.SendError(w, http.StatusConflict, "SETTLEMENT_POLICY_MISSING", err.Error())
			return
		case errors.Is(err, ErrSettlementEvidenceRequired):
			shared.SendError(w, http.StatusConflict, "SETTLEMENT_EVIDENCE_REQUIRED", err.Error())
			return
		case errors.Is(err, ErrSettlementIdempotencyConflict):
			shared.SendError(w, http.StatusConflict, "IDEMPOTENCY_CONFLICT", err.Error())
			return
		case errors.Is(err, ErrSettlementRefundExceedsGross):
			shared.SendError(w, http.StatusConflict, "REFUND_EXCEEDS_GROSS", err.Error())
			return
		case errors.Is(err, ErrSettlementMinimumNotMet):
			shared.SendError(w, http.StatusConflict, "SETTLEMENT_MINIMUM_NOT_MET", err.Error())
			return
		case err != nil:
...
func HandleListSettlementEvidence(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		shared.SendJSON(w, http.StatusOK, map[string]any{"evidence": evidence})
	}
}
