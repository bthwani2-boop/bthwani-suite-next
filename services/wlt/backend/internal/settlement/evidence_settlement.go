                ...
                if item.basis > 0 {
                        if _, err := tx.ExecContext(ctx, `INSERT INTO wlt_settlement_source_orders
                                ...
                                if errors.As(err, &pqErr) && pqErr.Code == "23505" {
                                        // A different idempotency key replaying an already-settled order
                                        // is a domain conflict, not a raw storage failure: the caller must
                                        // learn which orders are consumed, not that an insert failed.
