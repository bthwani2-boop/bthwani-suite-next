...
func reconcilePayoutWithAuthoritativeStatement(t *testing.T, db *sql.DB, operatorContextID, payoutID, destinationID, externalReference string, amount int64) {
	...
	if err != nil { t.Fatalf("register external provider account: %v", err) }
	...
	if err != nil { t.Fatalf("import authoritative statement: %v", err) }
	...
	if err != nil { t.Fatalf("reconcile payout four-way: %v", err) }
	if reconciliation.Result != "MATCHED" { t.Fatalf("four-way reconciliation result=%s, want MATCHED", reconciliation.Result) }
}
...
func TestManualSettlementReachesCompletion(t *testing.T) {
	...
	if createRes.Code != http.StatusCreated { t.Fatalf("payout create returned %d: %s", createRes.Code, createRes.Body.String()) }
	payoutID := payoutIDFromResponse(t, createRes)

	var available, pending, paid int64
	if err := db.QueryRow(`SELECT available_balance_minor_units,pending_balance_minor_units,paid_total_minor_units
	...
	if err != nil { t.Fatalf("record manual execution: %v", err) }
	...
	if evidence.VerifiedAt != nil { t.Fatal("execution must not verify itself") }
	if got := payoutStatus(t, db, operatorContextID, payoutID); got != "executed" { t.Fatalf("payout status after execution = %q, want executed", got) }
	...
	if err != nil { t.Fatalf("verify manual execution: %v", err) }
	if verified.VerifiedAt == nil { t.Fatal("verification did not stamp verified_at") }
	if got := payoutStatus(t, db, operatorContextID, payoutID); got != "verified" { t.Fatalf("payout status after verification = %q, want verified", got) }
	...
	if completeRes.Code != http.StatusOK { t.Fatalf("completion returned %d: %s", completeRes.Code, completeRes.Body.String()) }
	if got := payoutStatus(t, db, operatorContextID, payoutID); got != "completed" { t.Fatalf("payout status after completion = %q, want completed", got) }
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM wlt_ledger_transactions t JOIN wlt_ledger_lines l ON l.ledger_transaction_id = t.id
		WHERE t.operator_context_id=$1 AND t.transaction_type='payout_completed' AND t.reference_id=$2`, operatorContextID, payoutID).Scan(&lineCount); err != nil { t.Fatal(err) }
	if lineCount != 2 { t.Fatalf("expected a balanced two-line payout_completed transaction, got %d lines", lineCount) }
}
...
func TestRejectedPayoutReleasesPendingReservationWithoutPaidCounter(t *testing.T) {
	...
	if created.Code != http.StatusCreated { t.Fatalf("create returned %d: %s", created.Code, created.Body.String()) }
	...
	var available, pending, paid int64
	if err := db.QueryRow(`SELECT available_balance_minor_units,pending_balance_minor_units,paid_total_minor_units FROM wlt_wallets WHERE operator_context_id=$1 AND actor_type='field' AND actor_id=$2`, operatorContextID, actorID).Scan(&available, &pending, &paid); err != nil { t.Fatal(err) }
	if available != balance || pending != 0 || paid != 0 { t.Fatalf("rejection state available=%d pending=%d paid=%d", available, pending, paid) }
}
...
func TestDuplicateExternalReferenceIsRefused(t *testing.T) {
	...
	for i, idempotencyKey := range []string{"dup-payout-a-" + operatorContextID, "dup-payout-b-" + operatorContextID} {
		...
		if createRes.Code != http.StatusCreated { t.Fatalf("payout create %d returned %d: %s", i, createRes.Code, createRes.Body.String()) }
		payoutID := payoutIDFromResponse(t, createRes)
		if res := approvePayout(t, db, operatorContextID, payoutID, "finance-approver"); res.Code != http.StatusOK { t.Fatalf("approve %d returned %d: %s", i, res.Code, res.Body.String()) }
		var snapshotID string
		if err := db.QueryRow(`SELECT id FROM wlt_approved_payout_snapshots WHERE operator_context_id=$1 AND payout_request_id=$2`, operatorContextID, payoutID).Scan(&snapshotID); err != nil {
		...
		if i == 0 && firstErr != nil { t.Fatalf("first execution should succeed: %v", firstErr) }
	}
	if !errors.Is(firstErr, ErrDuplicateExternalReference) { t.Fatalf("expected the reused external reference to be refused, got %v", firstErr) }
}
