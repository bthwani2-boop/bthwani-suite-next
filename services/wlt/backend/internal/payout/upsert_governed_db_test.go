...
func TestCanonicalPayoutDestinationIdempotencyAndSingleActiveAreOperatorContextLocal(t *testing.T) {
	...
	if otherContext.Code != http.StatusCreated {
		t.Fatalf("cross-OperatorContext same actor/key status = %d, want 201; body=%s", otherContext.Code, otherContext.Body.String())
	}
	otherRef := decodeCanonicalPayoutRef(t, otherContext)
	if otherRef.ID == firstRef.ID || otherRef.ID == secondRef.ID {
	...
	for _, operatorContextID := range []string{operatorContextA, operatorContextB} {
		var activeCount int
		if err := db.QueryRow(`SELECT COUNT(*) FROM wlt_payout_destinations WHERE operator_context_id=$1 AND owner_actor_type='partner' AND owner_actor_id=$2 AND active=true`, operatorContextID, actorID).Scan(&activeCount); err != nil {
			t.Fatal(err)
		}
		if activeCount != 1 {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM wlt_payout_destination_requests WHERE operator_context_id=$1`, operatorContextA).Scan(&aRequestCount); err != nil {
		t.Fatal(err)
	}
	if err := db.QueryRow(`SELECT COUNT(*) FROM wlt_payout_destination_requests WHERE operator_context_id=$1`, operatorContextB).Scan(&bRequestCount); err != nil {
		t.Fatal(err)
	}
	if aRequestCount != 2 || bRequestCount != 1 {
		t.Fatalf("OperatorContext-local request counts are wrong: A=%d B=%d", aRequestCount, bRequestCount)
	}

	var decryptedReference string
	var referenceEncrypted bool
	if err := db.QueryRow(`SELECT pgp_sym_decrypt(destination_reference_encrypted, $3), destination_reference_encrypted IS NOT NULL FROM wlt_payout_destinations WHERE operator_context_id=$1 AND id=$2`, operatorContextA, secondRef.ID, "db-test-encryption-key").Scan(&decryptedReference, &referenceEncrypted); err != nil {
		t.Fatal(err)
	}
