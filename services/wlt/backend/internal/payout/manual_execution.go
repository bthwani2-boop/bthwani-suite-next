...
func scanManualTransferEvidence(row *sql.Row) (*ManualTransferEvidence, error) {
	...
	if verifiedAt.Valid { evidence.VerifiedAt = &verifiedAt.Time }
	return &evidence, nil
}
...
type VerifyManualExecutionInput struct { OperatorID string `json:"-"` }
...
func loadBatchSnapshotFacts(ctx context.Context, tx *sql.Tx, operatorContextID, batchID, snapshotID string) (batchSnapshotFacts, error) {
	...
	var inBatch bool
	if err := tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM wlt_settlement_batch_rows WHERE batch_id=$1 AND approved_snapshot_id=$2)`, batchID, snapshotID).Scan(&inBatch); err != nil { return facts, err }
	if !inBatch { return facts, ErrSnapshotNotInBatch }
	...
	if errors.Is(err, sql.ErrNoRows) { return facts, ErrSnapshotNotInBatch }
	return facts, err
}
...
func advanceBatchExecutionState(ctx context.Context, tx *sql.Tx, operatorContextID, batchID string) error {
	...
	if err := tx.QueryRowContext(ctx, `SELECT b.row_count,(SELECT COUNT(*) FROM wlt_manual_transfer_evidence e WHERE e.batch_id=b.id) FROM wlt_settlement_batches b WHERE b.id=$1 AND b.operator_context_id=$2`, batchID, operatorContextID).Scan(&rowCount, &evidenceCount); err != nil { return err }
	next := "execution_in_progress"
	if evidenceCount >= rowCount { next = "awaiting_verification" }
...
func RecordManualTransferExecution(ctx context.Context, db *sql.DB, batchID string, input RecordManualExecutionInput, correlationID string) (*ManualTransferEvidence, error) {
	...
	if batchStatus != "frozen" && batchStatus != "execution_in_progress" { return nil, ErrBatchNotExecutable }

	facts, err := loadBatchSnapshotFacts(ctx, tx, operatorContextID, batchID, input.ApprovedSnapshotID)
	if err != nil { return nil, err }
	if facts.AmountMinorUnits <= 0 || strings.TrimSpace(facts.Currency) == "" { return nil, fmt.Errorf("approved snapshot contains invalid monetary facts") }
	if input.OperatorID == facts.ApprovedByOperatorID { return nil, fmt.Errorf("%w: the approver cannot execute the transfer they approved", ErrSeparationOfDuties) }
	var payoutStatus string
	if err := tx.QueryRowContext(ctx, `SELECT status FROM wlt_payout_requests WHERE id=$1 AND operator_context_id=$2 FOR UPDATE`, facts.PayoutRequestID, operatorContextID).Scan(&payoutStatus); err != nil { return nil, err }
	if payoutStatus != "approved" { return nil, fmt.Errorf("%w: payout is %s", ErrPayoutNotAwaitingExecution, payoutStatus) }
	...
	if err != nil {
		if strings.Contains(err.Error(), "wlt_manual_transfer_evidence_ref_uq") { return nil, ErrDuplicateExternalReference }
		if strings.Contains(err.Error(), "wlt_manual_transfer_evidence_uq") { return nil, ErrEvidenceAlreadySubmitted }
		return nil, err
	}
	...
	if err != nil { return nil, err }
	if affected, _ := result.RowsAffected(); affected != 1 { return nil, ErrPayoutNotAwaitingExecution }
	if err := advanceBatchExecutionState(ctx, tx, operatorContextID, batchID); err != nil { return nil, err }
...
func VerifyManualTransferExecution(ctx context.Context, db *sql.DB, batchID, evidenceID string, input VerifyManualExecutionInput, correlationID string) (*ManualTransferEvidence, error) {
	...
	if err != nil { return nil, err }
	correlationID = strings.TrimSpace(correlationID)
	if batchID == "" || evidenceID == "" || correlationID == "" { return nil, fmt.Errorf("batchId, evidenceId and X-Correlation-ID are required") }
	tx, err := db.BeginTx(ctx, nil)
	if err != nil { return nil, err }
	defer tx.Rollback() //nolint:errcheck
	...
	if err := tx.QueryRowContext(ctx, `SELECT approved_snapshot_id,executed_by_operator_id,verified_at FROM wlt_manual_transfer_evidence WHERE id=$1 AND batch_id=$2 AND operator_context_id=$3 FOR UPDATE`, evidenceID, batchID, operatorContextID).Scan(&snapshotID, &executedBy, &verifiedAt); err != nil {
		if errors.Is(err, sql.ErrNoRows) { return nil, fmt.Errorf("manual transfer evidence not found") }
		return nil, err
	}
	if verifiedAt.Valid { return nil, ErrEvidenceAlreadyVerified }
	if input.OperatorID == executedBy { return nil, fmt.Errorf("%w: the executor cannot verify their own transfer", ErrSeparationOfDuties) }
	facts, err := loadBatchSnapshotFacts(ctx, tx, operatorContextID, batchID, snapshotID)
	if err != nil { return nil, err }
	if input.OperatorID == facts.ApprovedByOperatorID { return nil, fmt.Errorf("%w: the approver cannot verify the transfer they approved", ErrSeparationOfDuties) }
	...
	if err != nil { return nil, err }
	if affected, _ := result.RowsAffected(); affected != 1 { return nil, fmt.Errorf("%w: payout changed before verification", ErrPayoutNotAwaitingExecution) }
	if err := appendPayoutAudit(ctx, tx, "payout_request", facts.PayoutRequestID, "payout.execution_verified", input.OperatorID, "operator", "", correlationID, map[string]any{"batchId": batchID, "evidenceId": evidence.ID}); err != nil { return nil, err }
	if err := tx.Commit(); err != nil { return nil, err }
	return evidence, nil
}
...
func writeManualExecutionError(w http.ResponseWriter, err error) {
	switch {
	case errors.Is(err, ErrSeparationOfDuties): shared.SendError(w, http.StatusForbidden, "SEPARATION_OF_DUTIES", err.Error())
	case errors.Is(err, ErrDuplicateExternalReference): shared.SendError(w, http.StatusConflict, "DUPLICATE_EXTERNAL_REFERENCE", err.Error())
	case errors.Is(err, ErrEvidenceAlreadySubmitted): shared.SendError(w, http.StatusConflict, "EVIDENCE_ALREADY_SUBMITTED", err.Error())
	case errors.Is(err, ErrEvidenceAlreadyVerified): shared.SendError(w, http.StatusConflict, "EVIDENCE_ALREADY_VERIFIED", err.Error())
	case errors.Is(err, ErrPayoutNotAwaitingExecution), errors.Is(err, ErrBatchNotExecutable): shared.SendError(w, http.StatusConflict, "INVALID_STATUS", err.Error())
	case errors.Is(err, ErrSnapshotNotInBatch): shared.SendError(w, http.StatusNotFound, "NOT_FOUND", err.Error())
	case err != nil && strings.Contains(err.Error(), "not found"): shared.SendError(w, http.StatusNotFound, "NOT_FOUND", err.Error())
	default: shared.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", err.Error())
	}
}
...
func HandleRecordManualTransferExecution(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil { writeManualExecutionError(w, err); return }
...
func HandleVerifyManualTransferExecution(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		decoder := json.NewDecoder(http.MaxBytesReader(w, r.Body, 1024)); decoder.DisallowUnknownFields()
		if err := decoder.Decode(&input); err != nil { shared.SendError(w, http.StatusBadRequest, "INVALID_REQUEST", "request body is invalid"); return }
