...
func TestDestinationMaterialChangeOpensNewUnverifiedVersion(t *testing.T) {
	...
	if code != http.StatusCreated { t.Fatalf("first upsert returned %d", code) }
	if first.DestinationVersion != 1 { t.Fatalf("first destination version must be 1, got %d", first.DestinationVersion) }
	...
	if code != http.StatusOK { t.Fatalf("unchanged upsert returned %d, expected 200", code) }
	if same.ID != first.ID || same.DestinationVersion != 1 { t.Fatalf("unchanged upsert must reuse version 1, got id=%s version=%d", same.ID, same.DestinationVersion) }
	if same.DestinationVerificationStatus != verificationVerified { t.Fatalf("unchanged upsert must preserve verification, got %q", same.DestinationVerificationStatus) }

	changed, code := upsertOfficialWalletDestination(t, db, operatorContextID, actorID, "9876543210", "v2-"+operatorContextID, "corr-v2")
	if code != http.StatusCreated { t.Fatalf("changed upsert returned %d", code) }
	if changed.DestinationVersion != 2 { t.Fatalf("changed destination must open version 2, got %d", changed.DestinationVersion) }
	if changed.DestinationVerificationStatus != verificationUnverified { t.Fatalf("changed destination must require re-verification, got %q", changed.DestinationVerificationStatus) }
	if changed.MaskedDestinationReference == "9876543210" { t.Fatal("destination reference must never be returned unmasked") }

	var activeCount int
	if err := db.QueryRow(`SELECT count(*) FROM wlt_payout_destinations WHERE operator_context_id=$1 AND owner_actor_id=$2 AND active=true`, operatorContextID, actorID).Scan(&activeCount); err != nil { t.Fatal(err) }
	if activeCount != 1 { t.Fatalf("exactly one destination version may be active, got %d", activeCount) }
	var totalCount int
	if err := db.QueryRow(`SELECT count(*) FROM wlt_payout_destinations WHERE operator_context_id=$1 AND owner_actor_id=$2`, operatorContextID, actorID).Scan(&totalCount); err != nil { t.Fatal(err) }
	if totalCount != 2 { t.Fatalf("the superseded version must be preserved, got %d rows", totalCount) }
}
...
func TestPayoutRequestRejectsUnverifiedDestination(t *testing.T) {
	...
	if code != http.StatusCreated { t.Fatalf("upsert returned %d", code) }
	res := executePayoutCreate(t, db, operatorContextID, actorID, "unverified-payout-"+operatorContextID, 25000)
	if res.Code != http.StatusConflict { t.Fatalf("expected an unverified destination to block the payout request, got %d: %s", res.Code, res.Body.String()) }

	var available, pending int64
	if err := db.QueryRow(`SELECT available_balance_minor_units,pending_balance_minor_units FROM wlt_wallets WHERE operator_context_id=$1 AND actor_type='field' AND actor_id=$2`, operatorContextID, actorID).Scan(&available, &pending); err != nil { t.Fatal(err) }
	if available != initialBalance || pending != 0 { t.Fatalf("rejected payout must not reserve funds: available=%d pending=%d", available, pending) }
}
