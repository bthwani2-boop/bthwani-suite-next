...
-- migration carries the same trusted ownership into every JRN-036 settlement
...
ALTER TABLE wlt_jrn036_settlement_policy_versions
  ADD COLUMN IF NOT EXISTS operator_context_id text;

UPDATE wlt_jrn036_settlement_policy_versions version
...
UPDATE wlt_jrn036_settlement_policy_versions
SET operator_context_id = 'legacy-unscoped'
WHERE operator_context_id IS NULL OR btrim(operator_context_id) = '';

ALTER TABLE wlt_jrn036_settlement_policy_versions
  ALTER COLUMN operator_context_id SET NOT NULL,
  DROP CONSTRAINT IF EXISTS wlt_jrn036_settlement_policy_versions_pkey;
ALTER TABLE wlt_jrn036_settlement_policy_versions
  ADD CONSTRAINT wlt_jrn036_settlement_policy_versions_pkey
  PRIMARY KEY (operator_context_id, partner_id, version);
DROP INDEX IF EXISTS wlt_jrn036_settlement_policy_current_idx;
CREATE INDEX wlt_jrn036_settlement_policy_OperatorContext_current_idx
  ON wlt_jrn036_settlement_policy_versions (operator_context_id, partner_id, version DESC);
...
ALTER TABLE wlt_jrn036_settlement_source_evidence
  ADD COLUMN IF NOT EXISTS operator_context_id text;
UPDATE wlt_jrn036_settlement_source_evidence evidence
SET operator_context_id = settlement.operator_context_id
FROM wlt_settlements settlement
WHERE evidence.settlement_id = settlement.id
  AND (evidence.operator_context_id IS NULL OR btrim(evidence.operator_context_id) = '');
UPDATE wlt_jrn036_settlement_source_evidence
SET operator_context_id = 'legacy-unscoped'
WHERE operator_context_id IS NULL OR btrim(operator_context_id) = '';
ALTER TABLE wlt_jrn036_settlement_source_evidence
  ALTER COLUMN operator_context_id SET NOT NULL,
  DROP CONSTRAINT IF EXISTS wlt_jrn036_settlement_source_evidence_pkey;
ALTER TABLE wlt_jrn036_settlement_source_evidence
  ADD CONSTRAINT wlt_jrn036_settlement_source_evidence_pkey
  PRIMARY KEY (operator_context_id, order_id);
DROP INDEX IF EXISTS wlt_jrn036_settlement_completion_event_uidx;
CREATE UNIQUE INDEX wlt_jrn036_settlement_completion_event_OperatorContext_uidx
  ON wlt_jrn036_settlement_source_evidence (operator_context_id, completion_event_id);
CREATE INDEX wlt_jrn036_settlement_evidence_OperatorContext_settlement_idx
  ON wlt_jrn036_settlement_source_evidence (operator_context_id, settlement_id, order_id);

ALTER TABLE wlt_jrn036_settlement_requests
  ADD COLUMN IF NOT EXISTS operator_context_id text;
UPDATE wlt_jrn036_settlement_requests request
SET operator_context_id = settlement.operator_context_id
FROM wlt_settlements settlement
WHERE request.settlement_id = settlement.id
  AND (request.operator_context_id IS NULL OR btrim(request.operator_context_id) = '');
UPDATE wlt_jrn036_settlement_requests
SET operator_context_id = 'legacy-unscoped'
WHERE operator_context_id IS NULL OR btrim(operator_context_id) = '';
ALTER TABLE wlt_jrn036_settlement_requests
  ALTER COLUMN operator_context_id SET NOT NULL,
  DROP CONSTRAINT IF EXISTS wlt_jrn036_settlement_requests_pkey;
ALTER TABLE wlt_jrn036_settlement_requests
  ADD CONSTRAINT wlt_jrn036_settlement_requests_pkey
  PRIMARY KEY (operator_context_id, idempotency_key);
DROP INDEX IF EXISTS wlt_jrn036_settlement_request_hash_uidx;
CREATE UNIQUE INDEX wlt_jrn036_settlement_request_OperatorContext_hash_uidx
  ON wlt_jrn036_settlement_requests (operator_context_id, request_hash);
CREATE INDEX wlt_jrn036_settlement_request_OperatorContext_partner_idx
  ON wlt_jrn036_settlement_requests (operator_context_id, partner_id, created_at DESC);
...
COMMENT ON COLUMN wlt_jrn036_settlement_requests.operator_context_id IS
  'OperatorContext-local idempotency and request evidence identity.';
COMMENT ON COLUMN wlt_jrn036_settlement_source_evidence.operator_context_id IS
