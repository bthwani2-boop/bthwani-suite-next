-- WLT-098 / payouts-destinations: typed payout destinations, destination-bound requests,
...
CREATE TABLE IF NOT EXISTS wlt_payout_audit_events (
...
CREATE INDEX IF NOT EXISTS wlt_payout_audit_aggregate_idx
  ON wlt_payout_audit_events(aggregate_type, aggregate_id, created_at DESC);
CREATE INDEX IF NOT EXISTS wlt_payout_audit_correlation_idx
  ON wlt_payout_audit_events(correlation_id, created_at DESC);

CREATE TABLE IF NOT EXISTS wlt_payout_outbox (
...
CREATE INDEX IF NOT EXISTS wlt_payout_outbox_pending_idx
  ON wlt_payout_outbox(created_at, id)
  WHERE delivered_at IS NULL;

CREATE TABLE IF NOT EXISTS wlt_payout_reconciliations (
...
CREATE INDEX IF NOT EXISTS wlt_payout_reconciliation_request_idx
  ON wlt_payout_reconciliations(payout_request_id, created_at DESC);
...
-- handlers that predate payouts-destinations. It cannot release or move funds itself.
...
DROP TRIGGER IF EXISTS wlt_payout_transition_trigger ON wlt_payout_requests;
CREATE TRIGGER wlt_payout_transition_trigger
...
COMMENT ON TABLE wlt_payout_audit_events IS
  'Append-only payouts-destinations audit for destination, payout and reconciliation lifecycle changes.';
...
COMMENT ON TABLE wlt_payout_reconciliations IS
