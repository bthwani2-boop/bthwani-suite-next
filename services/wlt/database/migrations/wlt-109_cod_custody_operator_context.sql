-- WLT-109: OperatorContext-local COD identity, custody evidence and reconciliation.
--
-- COD records acquired operator_context_id in WLT-102, while order identity, immutable
-- evidence and reconciliation remained global. This migration converges every
-- supporting authority on the COD record's trusted OperatorContext. Immutability and
...
SET operator_context_id = record.operator_context_id
...
ALTER TABLE wlt_cod_custody_evidence
  ...
  ADD CONSTRAINT wlt_cod_custody_evidence_operator_context_idempotency_key
    UNIQUE (operator_context_id, event_type, idempotency_key),
...
SET operator_context_id = reconciliation.operator_context_id
...
COMMENT ON COLUMN wlt_cod_custody_evidence.operator_context_id IS
  'OperatorContext owning immutable COD custody proof and its idempotency identity.';
COMMENT ON COLUMN wlt_cod_reconciliation_cases.operator_context_id IS
  'OperatorContext owning the COD variance workflow.';
COMMENT ON COLUMN wlt_cod_reconciliation_audit_events.operator_context_id IS
  'OperatorContext copied by the reconciliation audit trigger for isolated audit reads.';
