-- WLT-090 / settlements-commissions: evidence-backed settlements and commissions.
--
...
CREATE TABLE IF NOT EXISTS wlt_settlement_requests (
...
CREATE UNIQUE INDEX IF NOT EXISTS wlt_settlement_request_hash_uidx
  ON wlt_settlement_requests (request_hash);

CREATE TABLE IF NOT EXISTS wlt_settlement_source_evidence (
  ...
  CONSTRAINT wlt_settlement_refund_basis_chk CHECK (
    original_gross_minor_units = completed_refund_minor_units + settlement_basis_minor_units
  )
);

CREATE UNIQUE INDEX IF NOT EXISTS wlt_settlement_completion_event_uidx
  ON wlt_settlement_source_evidence (completion_event_id);
CREATE INDEX IF NOT EXISTS wlt_settlement_evidence_settlement_idx
  ON wlt_settlement_source_evidence (settlement_id, order_id);

CREATE TABLE IF NOT EXISTS wlt_settlement_policy_versions (
  partner_id text NOT NULL CHECK (btrim(partner_id) <> ''),
...
CREATE INDEX IF NOT EXISTS wlt_settlement_policy_current_idx
  ON wlt_settlement_policy_versions (partner_id, version DESC);

CREATE TABLE IF NOT EXISTS wlt_commission_policy_versions (
...
CREATE UNIQUE INDEX IF NOT EXISTS wlt_commission_request_hash_uidx
  ON wlt_commission_evidence (request_hash);

CREATE TABLE IF NOT EXISTS wlt_commission_adjustments (
...
CREATE INDEX IF NOT EXISTS wlt_commission_adjustments_commission_idx
  ON wlt_commission_adjustments (commission_id, created_at DESC);

CREATE TABLE IF NOT EXISTS wlt_audit_events (
...
CREATE INDEX IF NOT EXISTS wlt_audit_aggregate_idx
  ON wlt_audit_events (aggregate_type, aggregate_id, created_at DESC);
CREATE INDEX IF NOT EXISTS wlt_audit_correlation_idx
  ON wlt_audit_events (correlation_id, created_at DESC);

-- Backfill the existing WLT-owned fixed field-visit policies into a versioned
-- settlements-commissions policy without changing or trusting any DSH-supplied amount.
