...
CREATE INDEX wlt_jrn037_payout_audit_OperatorContext_aggregate_idx
  ON wlt_jrn037_payout_audit_events
    (operator_context_id, aggregate_type, aggregate_id, created_at DESC);
CREATE INDEX wlt_jrn037_payout_audit_OperatorContext_correlation_idx
  ON wlt_jrn037_payout_audit_events
...
CREATE UNIQUE INDEX wlt_jrn037_payout_outbox_OperatorContext_event_uq
  ON wlt_jrn037_payout_outbox (operator_context_id, payout_request_id, event_type);
DROP INDEX IF EXISTS wlt_jrn037_payout_outbox_pending_idx;
CREATE INDEX wlt_jrn037_payout_outbox_OperatorContext_pending_idx
  ON wlt_jrn037_payout_outbox (operator_context_id, created_at, id)
  WHERE delivered_at IS NULL;
ALTER TABLE wlt_jrn037_payout_outbox
  ADD CONSTRAINT wlt_jrn037_payout_outbox_request_OperatorContext_fk
...
CREATE INDEX wlt_jrn037_payout_reconciliation_OperatorContext_request_idx
  ON wlt_jrn037_payout_reconciliations
    (operator_context_id, payout_request_id, created_at DESC);
CREATE UNIQUE INDEX wlt_jrn037_payout_reconciliation_OperatorContext_single_claim_idx
  ON wlt_jrn037_payout_reconciliations (operator_context_id, payout_request_id)
  WHERE resolved_at IS NULL;
ALTER TABLE wlt_jrn037_payout_reconciliations
  ADD CONSTRAINT wlt_jrn037_payout_reconciliation_request_OperatorContext_fk
  FOREIGN KEY (operator_context_id, payout_request_id)
  REFERENCES wlt_payout_requests (operator_context_id, id)
  ON DELETE RESTRICT;
...
COMMENT ON COLUMN wlt_jrn037_payout_outbox.operator_context_id IS
  'OperatorContext copied from the payout request for isolated notification delivery.';
COMMENT ON COLUMN wlt_jrn037_payout_reconciliations.operator_context_id IS
