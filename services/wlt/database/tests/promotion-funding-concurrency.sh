...
fail() {
  printf ' concurrency proof failed: %s\n' "$1" >&2
  exit 1
}
...
INSERT INTO wlt_promotion_funding_reservations (
  ...
  'concurrency','proof:race','checkout-race','redemption-race',
...
status="$("${PSQL[@]}" -Atc "SELECT status FROM wlt_promotion_funding_reservations WHERE operator_context_id='concurrency' AND external_reference='proof:race'")"
[[ "$status" == "committed" ]] || fail "concurrent transitions did not preserve the first governed state"

event_count="$("${PSQL[@]}" -Atc "SELECT count(*) FROM wlt_promotion_funding_events e JOIN wlt_promotion_funding_reservations r ON r.id=e.reservation_id WHERE r.operator_context_id='concurrency' AND r.external_reference='proof:race' AND e.to_status IN ('committed','released')")"
[[ "$event_count" == "1" ]] || fail "concurrent transitions produced more than one financial event"

printf ' concurrent transition serialization proof passed.\n'
