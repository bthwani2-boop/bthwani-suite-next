...
fail() {
  printf ' database proof failed: %s\n' "$1" >&2
  exit 1
}
...
INSERT INTO wlt_promotion_funding_reservations (
  ...
  'proof','proof:commit','checkout-commit','redemption-commit',
...
INSERT INTO wlt_promotion_funding_reservations (
  ...
  'proof','proof:release','checkout-release','redemption-release',
...
UPDATE wlt_promotion_funding_reservations
 ...
 WHERE operator_context_id='proof' AND external_reference='proof:release';
INSERT INTO wlt_promotion_funding_events (
  reservation_id,event_type,from_status,to_status,order_id,
  idempotency_key,correlation_id,reason
)
...
INSERT INTO wlt_promotion_funding_reservations (
  ...
  'proof','proof:unaudited','checkout-unaudited','redemption-unaudited',
  'coupon-proof','client-proof',1000,0,1000,'YER',
...
UPDATE wlt_promotion_funding_reservations
 ...
 WHERE operator_context_id='proof' AND external_reference='proof:unaudited';
COMMIT;
SQL
then
...
INSERT INTO wlt_promotion_funding_reservations (
  ...
  'proof','proof:stale','checkout-stale','redemption-stale',
  'coupon-proof','client-proof',1000,0,1000,'YER',
...
SELECT id,'committed','reserved','committed','order-stale',
  ...
  FROM wlt_promotion_funding_reservations
 WHERE operator_context_id='proof' AND external_reference='proof:stale';
SQL
...
[[ "$(scalar "SELECT status FROM wlt_promotion_funding_reservations WHERE operator_context_id='proof' AND external_reference='proof:commit'")" == "reversed" ]] || fail "terminal state changed after rejected transition"

printf ' PostgreSQL lifecycle and negative-invariant proof passed.\n'
