...
BEGIN
  ...
  IF missing_columns IS NOT NULL THEN
    RAISE EXCEPTION ' missing wlt_refunds columns: %', missing_columns;
  END IF;
END $$;
...
BEGIN
  ...
  IF status_definition IS NULL
    ...
    RAISE EXCEPTION ' refund status constraint is incomplete: %', status_definition;
  END IF;
  ...
  IF maker_checker_definition IS NULL
    ...
    RAISE EXCEPTION ' maker-checker constraint is missing or incomplete: %', maker_checker_definition;
  END IF;
END $$;
...
BEGIN
  ...
  IF operation_definition IS NULL OR operation_definition NOT LIKE '%refund%' THEN
    RAISE EXCEPTION ' reconciliation operation does not include refund: %', operation_definition;
  END IF;
  ...
  IF receipt_operation_definition IS NULL
    ...
    RAISE EXCEPTION ' receipt operations are incomplete: %', receipt_operation_definition;
  END IF;
  ...
  IF receipt_status_definition IS NULL
    ...
    RAISE EXCEPTION ' receipt lifecycle is incomplete: %', receipt_status_definition;
  END IF;
  ...
  IF receipt_response_definition IS NULL
    ...
    RAISE EXCEPTION ' completed receipt evidence constraint is incomplete: %', receipt_response_definition;
  END IF;
  ...
  IF refund_ref_definition IS NULL
    ...
    RAISE EXCEPTION ' refund reference states are incomplete: %', refund_ref_definition;
  END IF;
END $$;

SELECT ' refund database invariants passed' AS result;
