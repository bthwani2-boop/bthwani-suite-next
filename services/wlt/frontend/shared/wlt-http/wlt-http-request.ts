...
/**
 * POST mutation helper for WLT-owned financial write operations.
 * Mirrors wltFetchJson but sends a JSON body via POST.
 * All WLT mutations must be service-authenticated; DSH surfaces call this
 * only through the governed WLT shared layer.
 */
export async function wltPostJson<T>(
  ...
  try {
    ...
    return { ok: true, data: extract(responseBody) };
  } catch (e) {
    if (e instanceof DOMException && e.name === "TimeoutError") {
      return { ok: false, kind: "network", message: "request timed out" };
    }
