...
function formatMinorUnitsToLabel(
...
function codReferenceToRecord(ref: WltDshCodReference): WltDshFinanceSummaryRecord {
  ...
  return {
    ...
    amountLabel: formatMinorUnitsToLabel(
      ref.amountMinorUnits,
      ref.currency,
      isCollected && !isRemitted ? "-" : "",
    ),
    tone: isCollected && !isRemitted ? "negative" : "neutral",
    statusLabel: isRemitted
      ? "تم الإيداع"
      : isCollected
        ? "عهدة بانتظار الإيداع"
        : "بانتظار إثبات التحصيل",
...
export function WltDshCaptainBridge({
  ...
  const codController = useWltCaptainCodCustodyController(
    Boolean(dshClientId),
    fetchDshCaptainOwnCodRecords,
  );
  ...
  const codOutstandingMinorUnits =
    codController.state.kind === "loaded"
  ...
  const codPendingCollectionCount =
    codController.state.kind === "loaded"
      ...
      : 0;
  const codCollectedOutstandingCount =
    codController.state.kind === "loaded"
      ? codController.state.records.filter(
          (record) => record.status === "collected" && record.remittedAt == null,
        ).length
      : 0;
      ...
      <MobileScrollView fill padding={0} gap={0} contentContainerStyle={{ paddingBottom: 120 }}>
        <Box padding={0} gap={0}>
            ...
            <Box gap={3} style={{ paddingY: 8 }}>
              ...
              {codController.state.kind === "loaded" && codRecords.length > 0 ? (
                <KeyValueList
                  ...
                  items={[
                    {
                      label: "بانتظار إثبات التحصيل",
                      value: codPendingCollectionCount.toLocaleString(),
                      tone: codPendingCollectionCount > 0 ? "info" : "default",
                    },
                    {
                      label: "عهد محصّلة غير مودعة",
                      value: codCollectedOutstandingCount.toLocaleString(),
                      tone: codCollectedOutstandingCount > 0 ? "warning" : "default",
                    },
                    {
          ...
          <ActionStrip
            ...
            onPress={() =>
              setExpandedSection(expandedSection === "earnings" ? null : "earnings")
            }
          >
            <Box gap={3} style={{ paddingY: 8 }}>
              <StateView
          ...
          <ActionStrip
            ...
            onPress={() =>
              setExpandedSection(expandedSection === "settlement" ? null : "settlement")
            }
          >
            <Box gap={3} style={{ paddingY: 8 }}>
              <StateView
