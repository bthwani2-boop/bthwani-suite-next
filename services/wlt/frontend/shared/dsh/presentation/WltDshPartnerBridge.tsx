...
export type WltDshPartnerBridgeProps = {
  ...
  readonly serviceModes?:
    | readonly { id: string; label: string; description: string; enabled: boolean }[]
    | undefined;
  ...
  return (
    <Box padding={4} gap={4}>
      ...
      <PayoutDestinationPanel
        actorType="partner"
        title="وجهة صرف الشريك وطلبات الدفع"
        embedded
      />
      ...
      <View style={styles.actions}>
        {onOpenExpandedWallet ? (
          <Button
            label="عرض المحفظة الكاملة"
            tone="brand"
            onPress={onOpenExpandedWallet}
          />
        ) : null}
        {onOpenSettlementReview ? (
          <Button
            label="مراجعة التسويات المرجعية"
            tone="secondary"
            onPress={onOpenSettlementReview}
          />
        ) : null}
        {onOpenFinancialReport ? (
          <Button
