...
| `fix/master-pr23-pr27-total-closure` | UI files: `services/dsh/frontend/**`, `services/wlt/frontend/**`, `shared/ui-kit/**`, `apps/**` | `KEEP_NOW` | Wide UI formatting, color roles, and boundary cleanup | Same paths | Checked out and staged |
| `fix/master-pr23-pr27-total-closure` | UI helper scripts: `tools/scripts/fix-missing-color-roles.mjs`, `fix-raw-hex.mjs`, `fix-use-client.mjs`, `run-journey-gate.ps1` | `KEEP_NOW` | Wide UI refactoring and journey validation scripts | Same paths | Checked out and staged |
