...
- `PRIVILEGED` — exactly `remediation-analysis.yml` (now named **BThwani Manual Patch Verification**); the only workflow permitted to mutate source, and only inside its own runner, never by pushing. It no longer runs any general auto-repair (mobile source sync, OpenAPI generation, dependency dedupe, lint `--fix`, `go mod tidy`, `gofmt`); it verifies whatever is already at the requested `source_ref` — typically a task branch already repaired through the progressive remediation system — and applies only the pre-existing policy-governed records cleanup. This modernization closed `GAP-0002`.
- `TEMPORARY` — a registered, time-boxed, task-scoped workflow. A `TEMPORARY` entry must declare `taskId`, `contractHash`, `parentBranch`, and an unexpired `expiresAt`; an expired or contract-incomplete entry fails `guard:workflow-temp-files`. A `TEMPORARY` workflow is still read-only, still permission-locked, and still forbidden from mutating source. Temporary workflows are not created by default: they may exist only on a task branch and must be removed before it merges.

Adding a permanent `CORE`-adjacent workflow requires only a new file plus a registry entry — not an edit to either consuming guard, because the guards validate against the registry rather than a hard-coded list of every allowed file.
