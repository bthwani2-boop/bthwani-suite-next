...
test("terminal refusals cannot brick offline capture by exhausting capacity", async () => {
  ...
  await assert.rejects(
    () => enqueueFieldOperation("create_visit", { storeId: "store-blocked" }, "idem-blocked"),
    /capacity exceeded/,
  ...
  const fresh = await enqueueFieldOperation("create_visit", { storeId: "store-new" }, "idem-new");
...
test("evacuation leaves retryable and pending work in place", async () => {
  install();
  const pending = await enqueueFieldOperation("create_visit", { storeId: "s-1" }, "idem-pending");
  const retrying = await enqueueFieldOperation("create_visit", { storeId: "s-2" }, "idem-retry");
...
test("evacuation is a no-op when nothing has failed permanently", async () => {
  install();
  await enqueueFieldOperation("create_visit", { storeId: "s-1" }, "idem-1");
