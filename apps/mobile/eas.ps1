# Shared Android development EAS workflow for one mobile app at a time.
# Public entrypoint: apps/mobile.ps1
...
function Find-Keytool {
    ...
    if ($java -and $java.Source) {
        Add-KeytoolCandidate -List $candidates -Path (Join-Path (Split-Path -Parent $java.Source) 'keytool.exe')
    }
    ...
    if ($env:ProgramFiles) {
        Add-KeytoolCandidate -List $candidates -Path (Join-Path $env:ProgramFiles 'Android\Android Studio\jbr\bin\keytool.exe')
        foreach ($name in @('Java', 'Eclipse Adoptium', 'Microsoft')) {
            $root = Join-Path $env:ProgramFiles $name
            if (Test-Path -LiteralPath $root -PathType Container) {
                Get-ChildItem -LiteralPath $root -Filter keytool.exe -File -Recurse -ErrorAction SilentlyContinue |
                    ForEach-Object { Add-KeytoolCandidate -List $candidates -Path $_.FullName }
            }
        }
    }
    if (${env:ProgramFiles(x86)}) {
        Add-KeytoolCandidate -List $candidates -Path (Join-Path ${env:ProgramFiles(x86)} 'Android\Android Studio\jbr\bin\keytool.exe')
    }
    if ($env:LOCALAPPDATA) {
        Add-KeytoolCandidate -List $candidates -Path (Join-Path $env:LOCALAPPDATA 'Programs\Android Studio\jbr\bin\keytool.exe')
    }
    if ($env:USERPROFILE) {
    ...
    if (-not $match) {
        throw 'keytool was not found. Install a JDK or Android Studio JBR, or set JAVA_HOME.'
    }
...
function Get-FirebaseApiKey {
    ...
    if ($null -eq $client) { throw "$App Firebase client is missing from google-services.json." }
    $key = [string]$client.api_key[0].current_key
    if (-not (Test-ValidGoogleApiKey -Value $key)) { throw "$App Firebase API key is missing or invalid." }
    return $key
}
...
function Stage-Inputs {
    ...
    $firebaseKey = Get-FirebaseApiKey -Config $firebaseConfig

    Import-EnvFile -Path $MobileEnvPath
    $mapsKey = Resolve-ScopedValue -BaseName 'GOOGLE_MAPS_ANDROID_API_KEY'
    if (-not (Test-ValidGoogleApiKey -Value $mapsKey)) {
        throw "Maps key is missing or invalid for $App. Run Initialize first."
    }
    ...
    @(
        '# Generated locally by apps/mobile/eas.ps1.',
...
function Sync-EasDevelopmentEnvironment {
    ...
    Invoke-Checked -Command 'pnpm' -Arguments @(
        ...
        '--force',
        '--non-interactive'
    ) -WorkingDirectory $AppDir -SecretValues @($MapsKey) | Out-Null
}
...
function Get-InputState {
    ...
    $mapsBytes = [System.Text.Encoding]::UTF8.GetBytes($MapsKey)
    $firebaseBytes = [System.Text.Encoding]::UTF8.GetBytes($FirebaseKey)
    return [ordered]@{
        app = $App
        ...
        firebaseKeySha256 = [Convert]::ToHexString([System.Security.Cryptography.SHA256]::HashData($firebaseBytes)).ToLowerInvariant()
        credentialsSha256 = (Get-FileHash -LiteralPath $CredentialsPath -Algorithm SHA256).Hash.ToLowerInvariant()
        mapsKeySha256 = [Convert]::ToHexString([System.Security.Cryptography.SHA256]::HashData($mapsBytes)).ToLowerInvariant()
    }
}
...
function Write-PreflightStamp {
    ...
    $state = Get-InputState -MapsKey $MapsKey -FirebaseKey $FirebaseKey
...
function Assert-PreflightStamp {
    ...
    $current = Get-InputState -MapsKey $MapsKey -FirebaseKey $FirebaseKey
    foreach ($name in @('app', 'commit', 'firebaseFileSha256', 'firebaseKeySha256', 'credentialsSha256', 'mapsKeySha256')) {
...
if ($Mode -eq 'Initialize') {
    ...
    Write-Host "`nPASS: $App initialization completed." -ForegroundColor Green
    exit 0
}
...
if ($Mode -eq 'Preflight') {
    ...
    Write-PreflightStamp -MapsKey $inputs.MapsKey -FirebaseKey $inputs.FirebaseKey
    Write-Host "`nPASS: $App Preflight completed. No build was submitted." -ForegroundColor Green
    exit 0
}

Assert-PreflightStamp -MapsKey $inputs.MapsKey -FirebaseKey $inputs.FirebaseKey
