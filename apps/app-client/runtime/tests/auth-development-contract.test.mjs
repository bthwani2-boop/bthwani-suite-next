...
test("all mobile apps persist a stable Identity installation fingerprint", async () => {
...
test("password login binds sessions to server-owned canonical surfaces", async () => {
  ...
  assert.doesNotMatch(
    repository,
    /createSession\(ctx, actor, fingerprint, "control-panel"\)/,
  );
...
test("quick developer login is local-only and does not bundle privileged secrets", async () => {
  ...
  const mobileSource = [client, partner, field, captain].join("\n");
  ...
  const operatorLogin = main.indexOf(
    "getPasswordToken(LOCAL_ACTORS.operator.username)",
  );
  const readOnlyCheck = main.indexOf("collectReadOnlyReadinessFailures()");

  assert.ok(repairBranch >= 0);
  assert.ok(
    operatorLogin > repairBranch,
    "operator login/session creation must stay inside repair mode",
  );
  assert.ok(
    readOnlyCheck > repairBranch,
    "check mode must use the non-mutating collector",
  );
});
