...
export function CpSelect({ value, onChange, options, style, "aria-label": ariaLabel }: CpSelectProps) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} style={{ ...CP_INPUT_STYLE, ...style }} aria-label={ariaLabel}>
...
export function CpTextInput({ value, onChange, placeholder, type = "text", disabled, "aria-label": ariaLabel }: CpTextInputProps) {
  return (
    <input
      ...
      placeholder={placeholder}
      disabled={disabled}
      style={{ ...CP_INPUT_STYLE, width: "100%" }}
      aria-label={ariaLabel}
    />
  );
}
...
export function CpTable({ style, "aria-label": ariaLabel, children }: CpTableProps) {
  return <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem", ...style }} aria-label={ariaLabel}>{children}</table>;
}

export type CpTableHeaderCellProps = { readonly scope?: "col" | "row"; readonly style?: CSSProperties; readonly children: ReactNode };
const CP_TH_STYLE: CSSProperties = { padding: "0.5rem 0.75rem", textAlign: "start", fontWeight: 600, borderBottom: "2px solid rgba(0,0,0,0.15)", whiteSpace: "nowrap" };
...
const CP_TD_STYLE: CSSProperties = { padding: "0.5rem 0.75rem", borderBottom: "1px solid rgba(0,0,0,0.08)", verticalAlign: "middle" };
...
export function CpKpiCard({ label, value }: CpKpiCardProps) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", minWidth: "9rem", padding: "0.875rem 1rem", border: "1px solid color-mix(in srgb, currentColor 12%, transparent)", borderRadius: "0.875rem", background: "Canvas", boxShadow: "0 0.5rem 1.5rem rgba(0,0,0,0.05)" }}>
...
export function CpKpiStrip({ children }: { readonly children: ReactNode }) {
  return <div style={{ display: "flex", gap: "1.5rem", padding: "0.75rem 0", flexWrap: "wrap" }} role="region" aria-label="مؤشرات">{children}</div>;
}

export function CpPageHeader({ title, children }: { readonly title: string; readonly children?: ReactNode }) {
  return (
    <div style={{ padding: "1rem 1rem 0" }}>
      <h1 style={{ margin: "0 0 0.5rem", fontSize: "1.25rem", fontWeight: 700 }}>{title}</h1>
      {children}
    </div>
  );
}
...
export function CpFilterBar({ label, children }: { readonly label: string; readonly children: ReactNode }) {
  return <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", padding: "0.5rem 1rem", alignItems: "center" }} role="search" aria-label={label}>{children}</div>;
}
...
export function CpStatePanel({ role, title, description, code, children }: { readonly role: "status" | "alert"; readonly title: string; readonly description?: string; readonly code?: string; readonly children?: ReactNode }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "3rem 1rem", gap: "0.5rem", textAlign: "center", minHeight: "12rem" }} role={role} aria-live={role === "status" ? "polite" : undefined}>
      <strong style={{ fontSize: "1.125rem" }}>{title}</strong>
      {description ? <span style={{ fontSize: "0.875rem", opacity: 0.65 }}>{description}</span> : null}
      {code ? <code style={{ fontSize: "0.75rem", opacity: 0.75, fontFamily: "monospace", wordBreak: "break-all", maxWidth: "32rem" }}>{code}</code> : null}
      {children}
    </div>
  );
}
...
export function CpRetryButton({ onClick, children }: { readonly onClick: () => void; readonly children: ReactNode }) {
  return <CpButton onClick={onClick} style={{ marginTop: "0.75rem", padding: "0.375rem 1rem", border: "1px solid currentColor", borderRadius: "0.25rem", background: "transparent", cursor: "pointer", fontSize: "0.875rem" }}>{children}</CpButton>;
}
  ...
  return (
    <div style={{ padding: "1rem", height: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        ...
        <CpButton onClick={onClose} aria-label="إغلاق لوحة التفاصيل" style={{ background: "none", border: "none", cursor: "pointer", fontSize: "1.25rem", lineHeight: 1 }}>×</CpButton>
      </div>
      {children}
    </div>
  );
...
export function CpDetailError({ message }: { readonly message: string }) {
  return (
    <div role="alert">
      ...
      <code style={{ fontSize: "0.75rem", opacity: 0.7, fontFamily: "monospace", wordBreak: "break-all" }}>{message}</code>
    </div>
  );
}
...
export function CpDescriptionRow({ label, children }: { readonly label: string; readonly children: ReactNode }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.125rem" }}>
      <dt style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", opacity: 0.55 }}>{label}</dt>
