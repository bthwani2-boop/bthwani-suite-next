 ...
 * darkGlass) and from `cpScale` (radius / space / type / elevation) so
 * switching appearance mode recolors every screen that uses these primitives
 * without any screen-local styling. Do not re-hardcode colors, shadows, or
 * spacing here or at call sites — extend `cpScale` or the appearance token
 * mapping instead.
 */
...
export function shadowToBoxShadow(shadow: CpAppearanceShadow): string {
  ...
  const styles = useMemo(() => {
    ...
    return WebStyleSheet.create({
      ...
      kpiCard: {
        ...
        gap: cpSpace[1],
        minWidth: "9rem",
        padding: `${cpSpace[3]} ${cpSpace[4]}`,
        border: `1px solid ${cp.kpiBorder}`,
        borderRadius: cpRadius.lg,
        background: cp.kpiSurface,
        boxShadow: cpElevation.raised,
      },
      kpiValue: { fontSize: cpType.display.fontSize, fontWeight: 700, lineHeight: 1, color: tokens.textPrimary },
      kpiLabel: { fontSize: cpType.caption.fontSize, opacity: 0.65, color: tokens.textMuted },
      kpiStrip: { display: "flex", gap: cpSpace[6], padding: `${cpSpace[3]} 0`, flexWrap: "wrap" },
      pageHeader: { padding: `${cpSpace[4]} ${cpSpace[4]} 0` },
      pageHeaderTitle: { margin: `0 0 ${cpSpace[2]}`, fontSize: cpType.title.fontSize, fontWeight: 700, color: tokens.textPrimary },
      filterBar: {
        display: "flex",
        flexWrap: "wrap",
        gap: cpSpace[3],
      ...
      statePanelTitle: { fontSize: cpType.subtitle.fontSize, color: tokens.textPrimary },
      statePanelDesc: { fontSize: cpType.body.fontSize, opacity: 0.65, color: tokens.textMuted },
      statePanelCode: {
      ...
      emptyTableMessage: { padding: `${cpSpace[6]} ${cpSpace[4]}`, fontSize: cpType.body.fontSize, opacity: 0.65, color: tokens.textMuted },
      ...
      inlineCode: { fontSize: cpType.caption.fontSize, fontFamily: "monospace", color: tokens.textPrimary },
      mutedInline: { marginInlineStart: "0.375rem", opacity: 0.55, color: tokens.textMuted },
      detailPanel: { padding: cpSpace[4], height: "100%", background: tokens.surface },
      detailPanelHeader: {
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center",
        marginBottom: cpSpace[4],
      },
      detailPanelTitle: { margin: 0, fontSize: cpType.body.fontSize, fontWeight: 600, color: tokens.textPrimary },
      detailPanelCloseButton: {
        background: "none",
        border: "none",
        cursor: "pointer",
        fontSize: cpType.title.fontSize,
        lineHeight: 1,
        color: tokens.textMuted,
      },
      detailMessage: { opacity: 0.65, color: tokens.textMuted },
      detailErrorTitle: { marginBottom: cpSpace[2], color: tokens.danger },
      detailErrorCode: {
        fontSize: cpType.caption.fontSize,
        opacity: 0.7,
        fontFamily: "monospace",
      ...
      descriptionRowValue: { margin: 0, fontSize: cpType.body.fontSize, color: tokens.textPrimary },
      externalLink: { wordBreak: "break-all", fontSize: cpType.label.fontSize, color: tokens.accent },
