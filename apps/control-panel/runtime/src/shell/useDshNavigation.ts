 ...
 * Sensitive sections default to hidden while the session is restoring or when
 * the matching permission is absent. Navigation uses the same filtered set, so
 * callers cannot route to a concealed section through this hook.
 */
export function useDshNavigation() {
  const router = useRouter();
  const { state } = useControlPanelSession();
  const canReadPlatform = state.kind === "authenticated" &&
