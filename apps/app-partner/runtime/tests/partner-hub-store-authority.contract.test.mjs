...
test("partner hub keeps store status owned by canonical settings readback and rejects stale scope responses", async () => {
  ...
  assert.match(hub, /fetchPartnerStoreCoverageZones\(canonicalStoreId\)/);
  assert.match(hub, /const storeRuntimeRequestSeqRef = React\.useRef\(0\)/);
  assert.match(hub, /const requestSeq = \+\+storeRuntimeRequestSeqRef\.current/);
  assert.match(hub, /if \(!canonicalStoreId \|\| identity\.state\.kind !== "authenticated"\) \{[\s\S]*setStoreRuntime\(\{ kind: "idle" \}\)/);
  assert.match(hub, /requestSeq !== storeRuntimeRequestSeqRef\.current/);
