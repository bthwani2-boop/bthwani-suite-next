...
version: 2026.09.03-h-refoundation-v2
...
This file is routing only. It owns no repository semantics.
...
1. Pin exact live `h`.
2. Load `00` through `05` and every materially applicable `focus/*` owner.
3. Execute repository-wide forensic census → required-truth extraction → CURRENT/CANONICAL models → causal graph → highest correct execution-unit selection → refoundation → migration/cutover → highest-safe deletion → recursive pruning → admission-hole closure → verification/falsification → re-diagnosis.
