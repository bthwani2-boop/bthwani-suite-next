-- PLATFORM-006: database-final boundary for sensitive change-set targets.
...
CREATE OR REPLACE FUNCTION platform_reject_sensitive_change_set_item()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
DROP TRIGGER IF EXISTS platform_sensitive_change_set_item_trigger
    ON platform_change_set_items;

CREATE TRIGGER platform_sensitive_change_set_item_trigger
BEFORE INSERT OR UPDATE OF target_type, target_key, scope_type, scope_id, classification
ON platform_change_set_items
FOR EACH ROW
EXECUTE FUNCTION platform_reject_sensitive_change_set_item();

COMMENT ON FUNCTION platform_reject_sensitive_change_set_item() IS
    'Final database guard rejecting sensitive classifications and existing sensitive variable targets before change-set persistence.';
