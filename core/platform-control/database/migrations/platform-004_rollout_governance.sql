-- PLATFORM-004 / rollout-governance: explicit target scope, health gate, and lifecycle invariants.
...
COMMENT ON CONSTRAINT platform_rollout_target_scope_governed ON platform_rollouts IS
    'rollout-governance rollout scope must use an explicit governed audience, region, city or surface selector.';
COMMENT ON CONSTRAINT platform_rollout_health_gate_governed ON platform_rollouts IS
    'rollout-governance every rollout must require OPERATIONAL health before progression.';
COMMENT ON CONSTRAINT platform_rollout_terminal_state_consistency ON platform_rollouts IS
    'rollout-governance lifecycle timestamps and completed percentage must match the persisted rollout status.';
COMMENT ON FUNCTION platform_prevent_paused_rollout_advance() IS
    'rollout-governance defense in depth: a paused rollout cannot change step, percentage or flag revision until an explicit resume; abort and rollback baseline restoration remain legal.';
