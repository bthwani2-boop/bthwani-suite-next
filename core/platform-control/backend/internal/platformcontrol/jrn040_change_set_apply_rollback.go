...
func (r *Repository) ApplyChangeSetGoverned(ctx context.Context, id, actorID string, roles []string, correlationID string) (ChangeSet, error) {
	...
	for _, item := range items {
		...
		if err := applyGovernedItem(ctx, tx, item); err != nil {
			return ChangeSet{}, err
		}
	}
...
WHERE id = $1::uuid`, id, actorID); err != nil {
	...
	if err := insertAudit(ctx, tx, id, "change_set_applied", actorID, roles, string(ChangeSetApplied), "", correlationID,
		...
		map[string]any{"status": ChangeSetApplied, "appliedItemCount": len(items), "valuesRedacted": true}); err != nil {
		return ChangeSet{}, err
	}
...
WHERE id = $1::uuid`, item.ID, beforeRaw, nextRevision)
	return err
}
...
func (r *Repository) RollbackChangeSetGoverned(ctx context.Context, id, actorID string, roles []string, correlationID, reason string) (ChangeSet, error) {
	...
	for index := len(items) - 1; index >= 0; index-- {
		if err := lockGovernedTarget(ctx, tx, items[index]); err != nil {
			return ChangeSet{}, err
		}
		if err := rollbackGovernedItem(ctx, tx, items[index]); err != nil {
			return ChangeSet{}, err
		}
	}
	if _, err := tx.ExecContext(ctx, `
...
WHERE id = $1::uuid`, id); err != nil {
	...
	if err := insertAudit(ctx, tx, id, "change_set_rolled_back", actorID, roles, string(ChangeSetRolledBack), reason, correlationID,
		map[string]any{"status": status},
		map[string]any{"status": ChangeSetRolledBack, "rolledBackItemCount": len(items), "valuesRedacted": true}); err != nil {
		return ChangeSet{}, err
	}
	if err := tx.Commit(); err != nil {
		return ChangeSet{}, err
...
SET owner_service = $4,
    ...
    revision = revision + 1,
    updated_at = NOW()
WHERE variable_key = $1 AND scope_type = $2 AND scope_id = $3 AND revision = $9`,
			item.TargetKey,
			item.ScopeType,
			item.ScopeID,
...
SET owner_service = $2,
    enabled = $3,
    status = $4,
    targeting_json = $5::jsonb,
    revision = revision + 1,
    updated_at = NOW()
WHERE flag_key = $1 AND revision = $6`,
			item.TargetKey,
			snapshot.OwnerService,
			snapshot.Enabled,
			snapshot.Status,
