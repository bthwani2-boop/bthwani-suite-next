...
func (r *Repository) ValidateChangeSetGoverned(ctx context.Context, id, actorID string, roles []string, correlationID string) (ChangeSet, error) {
	...
	for _, item := range items {
		if err := lockGovernedTarget(ctx, tx, item); err != nil {
			return ChangeSet{}, err
		}
		if err := ensureNoActiveTargetConflict(ctx, tx, id, item); err != nil {
			return ChangeSet{}, err
		}
		...
		if revision != item.ExpectedRevision {
			return ChangeSet{}, fmt.Errorf("%w: %s expected revision %d but current revision is %d", ErrVersionConflict, item.TargetKey, item.ExpectedRevision, revision)
		}
...
SET validated_value_json = $2::jsonb,
    validated_revision = $3,
    validated_at = NOW()
WHERE id = $1::uuid`, item.ID, validatedRaw, revision); err != nil {
			return ChangeSet{}, err
		}
	}
	if _, err := tx.ExecContext(ctx, `
...
WHERE id = $1::uuid`, id); err != nil {
	...
	if err := insertAudit(ctx, tx, id, "change_set_validated", actorID, roles, string(ChangeSetValidated), "", correlationID,
		map[string]any{"status": status},
		map[string]any{"status": ChangeSetValidated, "validatedItemCount": len(items), "valuesRedacted": true}); err != nil {
		return ChangeSet{}, err
	}
...
func (r *Repository) governedTransitionWithPreconditions(
	ctx context.Context,
	id string,
	...
	if err := verifyGovernedPreconditions(ctx, tx, items); err != nil {
		return ChangeSet{}, err
	}
	if next != ChangeSetSubmitted {
		return ChangeSet{}, ErrInvalidTransition
	}
	if _, err := tx.ExecContext(ctx, `
UPDATE platform_change_sets
...
FOR UPDATE`, id).Scan(&proposer, &rawStatus); errors.Is(err, sql.ErrNoRows) {
	...
	if proposer == actorID {
		return ChangeSet{}, ErrMakerCheckerReview
	}
	items, err := loadGovernedChangeSetItemsForUpdate(ctx, tx, id)
	if err != nil {
		return ChangeSet{}, err
...
WHERE id = $1::uuid`, id, actorID, reason); err != nil {
	...
	if err := insertAudit(ctx, tx, id, "change_set_rejected", actorID, roles, string(ChangeSetRejected), reason, correlationID,
		map[string]any{"status": rawStatus}, map[string]any{"status": ChangeSetRejected}); err != nil {
		return ChangeSet{}, err
	}
	if err := tx.Commit(); err != nil {
		return ChangeSet{}, err
...
func verifyGovernedPreconditions(ctx context.Context, tx *sql.Tx, items []ChangeSetItem) error {
	...
	sort.Slice(sortedItems, func(i, j int) bool {
		return governedTargetIdentity(sortedItems[i]) < governedTargetIdentity(sortedItems[j])
	})
	for _, item := range sortedItems {
		if item.ValidatedRevision == nil || item.ItemValidatedAt == nil {
			return fmt.Errorf("%w: %s", ErrValidationSnapshot, item.TargetKey)
		}
		if err := lockGovernedTarget(ctx, tx, item); err != nil {
			return err
		}
		...
		if revision != *item.ValidatedRevision || !jsonEquivalent(snapshot, item.PreconditionSnapshot) {
			return fmt.Errorf("%w: validation snapshot for %s is stale", ErrVersionConflict, item.TargetKey)
		}
	}
	return nil
}
...
SELECT change_set.id::text
FROM platform_change_set_items AS candidate
JOIN platform_change_sets AS change_set ON change_set.id = candidate.change_set_id
WHERE candidate.change_set_id <> $1::uuid
  AND candidate.target_type = $2
  AND candidate.target_key = $3
  AND candidate.scope_type = $4
  AND candidate.scope_id = $5
  AND change_set.status IN ('validated', 'submitted', 'approved')
ORDER BY change_set.created_at, change_set.id
LIMIT 1
FOR UPDATE OF change_set`,
		changeSetID,
		item.TargetType,
		item.TargetKey,
		item.ScopeType,
...
func readGovernedTargetSnapshot(ctx context.Context, tx *sql.Tx, item ChangeSetItem) (any, int64, bool, error) {
	...
	case ChangeTargetVariable:
		...
		var revision int64
		err := tx.QueryRowContext(ctx, `
SELECT owner_service, value_type, classification, status, value_json, revision
FROM platform_variables
WHERE variable_key = $1 AND scope_type = $2 AND scope_id = $3
FOR UPDATE`, item.TargetKey, item.ScopeType, item.ScopeID).Scan(
			&snapshot.OwnerService,
			&snapshot.ValueType,
		...
		if err := json.Unmarshal(raw, &snapshot.Value); err != nil {
			return nil, 0, false, err
		}
	...
	case ChangeTargetFeatureFlag:
		...
		var revision int64
		err := tx.QueryRowContext(ctx, `
SELECT owner_service, status, enabled, targeting_json, revision
FROM platform_feature_flags
WHERE flag_key = $1
FOR UPDATE`, item.TargetKey).Scan(
			&snapshot.OwnerService,
			&snapshot.Status,
			&snapshot.Enabled,
			&targetingRaw,
		...
		if err := json.Unmarshal(targetingRaw, &snapshot.Targeting); err != nil {
			return nil, 0, false, err
		}
...
func jsonEquivalent(left, right any) bool {
	...
	if json.Unmarshal(leftRaw, &leftCanonical) != nil || json.Unmarshal(rightRaw, &rightCanonical) != nil {
		return false
	}
