...
func TestProvidersAuthAcceptsMatchingActiveSaaSTenant(t *testing.T) {
...
func TestProvidersAuthRejectsCrossTenantIdentity(t *testing.T) {
	t.Setenv("BTHWANI_SAAS_MODE", "active")
	t.Setenv("BTHWANI_DEFAULT_TENANT_ID", "tenant-main")
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_ = json.NewEncoder(w).Encode(Identity{
...
func TestProvidersAuthFailsClosedWithoutActiveTenant(t *testing.T) {
	t.Setenv("BTHWANI_SAAS_MODE", "active")
