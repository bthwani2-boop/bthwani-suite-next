...
func (r *Repository) ensureOperationalCore(ctx context.Context, actorID string) (string, error) {
	...
	if _, err := r.db.ExecContext(ctx, `INSERT INTO workforce_provider_operational_core(actor_id) VALUES($1) ON CONFLICT(actor_id) DO NOTHING`, actorID); err != nil {
		return "", err
	}
	if kind == "captain" {
		if _, err := r.db.ExecContext(ctx, `INSERT INTO workforce_captain_activation_core(actor_id) VALUES($1) ON CONFLICT(actor_id) DO NOTHING`, actorID); err != nil {
			return "", err
		}
	}
	return kind, nil
