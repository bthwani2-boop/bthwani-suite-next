...
func (r *Repository) ensureOperationalCore(ctx context.Context, actorID string) (string, error) {
	...
	if kind != "field" && kind != "captain" { return "", ErrInvalidInput }
	if _, err := r.db.ExecContext(ctx, `INSERT INTO workforce_provider_operational_core(actor_id) VALUES($1) ON CONFLICT(actor_id) DO NOTHING`, actorID); err != nil { return "", err }
	if kind == "captain" {
		if _, err := r.db.ExecContext(ctx, `INSERT INTO workforce_captain_activation_core(actor_id) VALUES($1) ON CONFLICT(actor_id) DO NOTHING`, actorID); err != nil { return "", err }
	}
	return kind, nil
}
...
func (r *Repository) OperationalCoreByActorID(ctx context.Context, actorID string) (ProviderOperationalCore, error) {
	...
	if err != nil { return ProviderOperationalCore{}, err }
	...
	err = r.db.QueryRowContext(ctx, `
		...
		FROM workforce_provider_operational_core WHERE actor_id=$1`, actorID, kind).Scan(
...
func (r *Repository) PatchOperationalCore(ctx context.Context, actorID, operatorID string, input OperationalCorePatch) (ProviderOperationalCore, error) {
	kind, err := r.ensureOperationalCore(ctx, actorID)
	if err != nil { return ProviderOperationalCore{}, err }
	if err := validateOperationalCorePatch(kind, input); err != nil { return ProviderOperationalCore{}, err }
	...
	_, err = tx.ExecContext(ctx, `
		...
		WHERE actor_id=$1`, actorID,input.ReferralSourceType,input.ReferralSourceActorID,input.ReferralPartnerID,input.ReferralChannel,
	...
	if err := tx.Commit(); err != nil { return ProviderOperationalCore{}, err }
...
func validateOperationalCorePatch(kind string, input OperationalCorePatch) error {
	if input.ReferralSourceType != nil && !oneOf(*input.ReferralSourceType,"employee","captain","field","partner","advertisement","social_media","public_referral","direct","other") { return ErrInvalidInput }
	if input.IdentityVerificationStatus != nil && !oneOf(*input.IdentityVerificationStatus,"pending","under_review","approved","rejected","expired","needs_resubmission") { return ErrInvalidInput }
	if input.ContractReviewStatus != nil && !oneOf(*input.ContractReviewStatus,"pending","under_review","approved","rejected","needs_resubmission") { return ErrInvalidInput }
	if input.OnboardingStage != nil && !oneOf(*input.OnboardingStage,"basic_profile","documents_pending","documents_review","training_pending","partnerships_review","operations_review","activation_ready","active") { return ErrInvalidInput }
	if input.Captain == nil { return nil }
	if kind != "captain" { return ErrInvalidInput }
	c := input.Captain
...
func oneOf(value string, allowed ...string) bool { for _, item := range allowed { if value == item { return true } }; return false }
...
func EvaluateProviderActivationReadiness(person Person, core ProviderOperationalCore) ActivationReadiness {
	...
	case "captain":
		if person.CaptainProfile==nil || person.CaptainProfile.LicenseStatus!="valid" || !isLicenseNotExpired(person.CaptainProfile.LicenseExpiresAt) { missing=append(missing,"validDrivingLicense") }
		if core.Captain==nil { missing=append(missing,"captainActivationCore") } else {
			c:=core.Captain
	...
	default: missing=append(missing,"supportedProviderKind")
	}
...
func (r *Repository) ActivationReadiness(ctx context.Context, actorID string) (ActivationReadiness,error) {
	person,err:=r.PersonByActorID(ctx,actorID); if err!=nil{return ActivationReadiness{},err}
	core,err:=r.OperationalCoreByActorID(ctx,actorID); if err!=nil{return ActivationReadiness{},err}
...
func (r *Repository) CreateAvailabilityNotice(ctx context.Context, actorID string, input CreateAvailabilityNoticeInput) (AvailabilityNotice,error) {
	if !oneOf(input.NoticeType,"planned_unavailability","immediate_unavailability","short_break","emergency","temporary_restriction") || input.StartsAt.IsZero() || input.EndsAt.IsZero() || !input.EndsAt.After(input.StartsAt) { return AvailabilityNotice{},ErrInvalidInput }
...
func (r *Repository) ListAvailabilityNotices(ctx context.Context, actorID string, limit int) ([]AvailabilityNotice,error) {
	...
	rows,err:=r.db.QueryContext(ctx,`SELECT id::text,actor_id,notice_type,starts_at,ends_at,COALESCE(service_zone_id,''),reason_code,note,status,created_by_actor_id,created_at,updated_at FROM workforce_provider_availability_notices WHERE actor_id=$1 ORDER BY starts_at DESC LIMIT $2`,actorID,limit)
	if err!=nil{return nil,err};defer rows.Close();out:=make([]AvailabilityNotice,0)
	for rows.Next(){var n AvailabilityNotice;if err:=rows.Scan(&n.ID,&n.ActorID,&n.NoticeType,&n.StartsAt,&n.EndsAt,&n.ServiceZoneID,&n.ReasonCode,&n.Note,&n.Status,&n.CreatedByActorID,&n.CreatedAt,&n.UpdatedAt);err!=nil{return nil,err};out=append(out,n)}
	return out,rows.Err()
}
...
func (r *Repository) CreateProviderIncident(ctx context.Context, reporterID string, input CreateProviderIncidentInput) (ProviderIncident,error) {
	input.ActorID=strings.TrimSpace(input.ActorID);input.IncidentCode=strings.TrimSpace(input.IncidentCode);input.Description=strings.TrimSpace(input.Description)
	...
	if input.SourceType==""{input.SourceType="operational"};if input.Severity==""{input.Severity="minor"};if !oneOf(input.Severity,"minor","major","critical"){return ProviderIncident{},ErrInvalidInput};if input.Currency==""{input.Currency="YER"}
	evidence,err:=json.Marshal(nonNil(input.EvidenceMediaRefs));if err!=nil{return ProviderIncident{},err};var id string
	...
	if err!=nil{return ProviderIncident{},err};return r.ProviderIncidentByID(ctx,id,input.ActorID)
}

func (r *Repository) ProviderIncidentByID(ctx context.Context, id, actorID string) (ProviderIncident,error) {
	var incident ProviderIncident;var evidence []byte
	...
	if errors.Is(err,sql.ErrNoRows){return ProviderIncident{},ErrNotFound};if err!=nil{return ProviderIncident{},err}
	if err:=json.Unmarshal(evidence,&incident.EvidenceMediaRefs);err!=nil{return ProviderIncident{},err};if incident.EvidenceMediaRefs==nil{incident.EvidenceMediaRefs=[]string{}}
	return incident,nil
}

func (r *Repository) ListProviderIncidents(ctx context.Context, actorID string, limit int) ([]ProviderIncident,error) {
	if limit<=0 || limit>100{limit=50};rows,err:=r.db.QueryContext(ctx,`SELECT id::text FROM workforce_provider_incidents WHERE actor_id=$1 ORDER BY created_at DESC LIMIT $2`,actorID,limit);if err!=nil{return nil,err};defer rows.Close()
	ids:=make([]string,0);for rows.Next(){var id string;if err:=rows.Scan(&id);err!=nil{return nil,err};ids=append(ids,id)};if err:=rows.Err();err!=nil{return nil,err}
	out:=make([]ProviderIncident,0,len(ids));for _,id:=range ids{item,err:=r.ProviderIncidentByID(ctx,id,actorID);if err!=nil{return nil,err};out=append(out,item)};return out,nil
}
