...
-- actor-bound invitations for the control-panel webapp; they are not phone OTPs.
...
ALTER TABLE identity_activation_challenges
  ...
  ADD CONSTRAINT identity_activation_challenges_surface_check
    CHECK (surface IN ('app-field', 'app-captain', 'app-client', 'app-partner', 'control-panel', 'webapp'));
...
COMMENT ON COLUMN identity_activation_challenges.surface IS
  'Single target surface. webapp is the control-panel first-login surface for employees.';
