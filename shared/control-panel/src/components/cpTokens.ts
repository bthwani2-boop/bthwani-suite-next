...
export function useCpTokens(): CpTokensResult {
  ...
  const styles = useMemo<CpWebStyles>(() => {
    ...
    return WebStyleSheet.create({
      ...
      retryButton: { marginTop: cpSpace[3], padding: `0.375rem ${cpSpace[4]}`, border: `1px solid ${tokens.textPrimary}`, borderRadius: cpRadius.sm, background: "transparent", cursor: "pointer", fontSize: cpType.body.fontSize, color: tokens.textPrimary },
