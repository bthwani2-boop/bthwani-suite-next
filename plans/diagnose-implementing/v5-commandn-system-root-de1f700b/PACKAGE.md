...
LATEST_RECONCILED_SHA: de1f700b94196117c07de6cf5c3131ab4af087b6
...
|---|---|---|---|---|---|---|---|---|---|
| RC-002 | WLT contains the intended wlt-937 payment-session funding-rail integrity migration, but the current immutable manifest omits it; ordered migration authority cannot discover or verify the database invariant. | OP-FINANCE-MIGRATION | EVD-RC-002 | NONE | CON-WLT-MANIFEST,CON-WLT-MIGRATION-RUNNER | Restores WLT financial migration discoverability and unlocks schema, upgrade-path, integrity, and finance evidence. | 1 | DEEPENED_ENOUGH_TO_RANK | READY |
...
| CLEANUP | CLN-BOUNDARY-DRIFT | RC-001 | Remove the contract/binding drift and ensure no undocumented parallel route remains. | PLANNED | EVD-VERIFICATION-PLAN |
| CLEANUP | CLN-MIGRATION-DRIFT | RC-002 | Remove the manifest/source divergence without deleting or rewriting the financial migration. | PLANNED | EVD-VERIFICATION-PLAN |

## Frontier

| Work | RC | Depends on | Blocks / unlocks | Conflict | Owner | Parallel | State | Evidence |
|---|---|---|---|---|---|---|---|---|
| WORK-WLT-MIGRATION-MANIFEST | RC-002 | NONE | WLT migration discovery, ordering, financial integrity, and journey gate. | wlt-migration-authority | task-owner | NO | READY | EVD-VERIFICATION-PLAN |
| WORK-CONTRACT-CUTOVER | RC-001 | WORK-WLT-MIGRATION-MANIFEST | DSH OpenAPI, composed bundle, backend binding, Workforce client, and dispatch readback verification. | dsh-contract-boundary | task-owner | NO | WAITING | EVD-VERIFICATION-PLAN |
...
| EVD-IMPLEMENTATION | Root treatment implemented on the task candidate. | pending final implementation check | TASK_HEAD | task branch | MISSING | Invalidates on any source write |
| EVD-CONSUMERS | All required contract, generated/bound, backend, and Workforce consumers reconcile on the task candidate. | pending consumer and binding checks | TASK_HEAD | task branch | MISSING | Invalidates on any consumer/contract write |
| EVD-CLEANUP | Undocumented route drift and any superseded binding artifacts are removed or correctly reconciled. | pending cleanup and negative-space check | TASK_HEAD | task branch | MISSING | Invalidates on any source write |
| EVD-VERIFICATION | Exact task candidate passes final affected verification. | pending final read-only verification | SELF | task branch | MISSING | Invalidates on any source write |
| EVD-GOVERNANCE | Governance/self-guard evidence passes on the exact task candidate. | pending governance-schema and registered guard verification | SELF | task branch | MISSING | Invalidates on governance/source write |
| EVD-FINAL-ADVERSARIAL | Final negative-space/adversarial checks pass on the exact task candidate. | pending final adversarial verification | SELF | task branch | MISSING | Invalidates on any source write |
