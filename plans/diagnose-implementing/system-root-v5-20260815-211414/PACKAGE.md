...
| EVD-RUNTIME | Candidate-bound runtime/product smoke for WLT-backed financial ownership and affected DSH surfaces | runtime:smoke; runtime:wlt:smoke; runtime:wlt:provider:smoke with local-dsh context and container-provided secrets; WireMock journal | SELF | Docker local runtime, DSH/WLT/Identity/Postgres healthy; migrations sha 6f63ad96a54a3dd61a28e945947311bc5e49de75 | PASS | invalidate on runtime or candidate change |
| EVD-FIELD-RUNTIME-READBACK-HOLD | Authenticated Field payout-destination route is registered and fail-closed, but the local runtime has no governed active Field destination fixture; a successful 2xx owner/masking readback is therefore not yet proven | local Workforce Field activation plus GET /dsh/field/me/finance/payout-destination; response HTTP 404 PAYOUT_DESTINATION_NOT_FOUND | SELF | Docker local runtime, Field actor field-yPmrHDMkOxkj, local operator lacks finance.manage; no destination fixture | BLOCKED | invalidate on a governed Field destination fixture or finance-manager runtime journey |
...
- Field destination runtime readback: HOLD; authenticated route returns the governed 404 because no active local Field destination fixture exists
