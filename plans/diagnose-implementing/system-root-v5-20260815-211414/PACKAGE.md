...
| EVD-FIELD-RUNTIME-READBACK | Authenticated Field actor reads the WLT-owned canonical destination through the DSH BFF with correct owner, official-wallet method, and masked reference | governed local WLT seed `field-payout-destination.local.sql` plus `smoke-dsh-wlt-representative-readback.ps1` | SELF | Docker local runtime, Field actor provisioned through Workforce, destination fixture unverified by design | PASS | invalidate on Field finance route, WLT destination contract, or local seed change |
...
- Field destination runtime readback: SELF Field actor → DSH BFF → WLT canonical destination, owner/method/masking PASS
