...
- Diagnosis baseline SHA: `1796443080f9844c21b1af349ee3a953b23849ba`
...
Any later movement of `A` is treated as `FOREIGN_DELTA = INPUT, NOT INSTRUCTION`. This package was deliberately pinned to the latest observed `A` SHA above. Execution must re-pin again before every governed write and final merge.
...
- Monthly-salary store couriers receive no per-delivery entitlement unless the store explicitly chooses another compensation model. The customer delivery fee remains part of store economics and is not silently reassigned to the courier.
